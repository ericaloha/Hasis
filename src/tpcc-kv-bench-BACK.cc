...
void Consumer_HistoryTree(ProAndCon* itemQ_,GlobalBuffer* gb_, const int plog_flag_){
  ...
  Historyfk1Tree = new BPlusHistoryfk1Tree(db1.c_str(), log1.c_str(), chpt1.c_str(), bg_id++, true);
  // load
  ...
  Historyfk2Tree = new BPlusHistoryfk2Tree(db2.c_str(), log2.c_str(), chpt2.c_str(), bg_id++, true);
  // load
...
void dispatcher_MVCC_TEST(){
  ...
  int tp_cnt=0;
  auto t3 = std::chrono::steady_clock::now();
  cmdLine = tpch_gen_.gentpch_cmd_orderline_TEST();
  while((*cmdLine)[0]!='-' && (*cmdLine)[0]!='1')
  //while(getline(cmd_file_test,cmdLine))
  {
    ...
    cmdLine = tpcc_gen_.gentpcc_cmd_Orderline_TEST();
    if((tp_cnt+1)%500000 == 0){
      auto t4 = std::chrono::steady_clock::now();
      auto tmp1 = std::chrono::duration_cast<std::chrono::microseconds>(t4 - t3).count()/1000;
      std::cout<<"finish test: "<<tp_cnt<<" tp requests, exe time: "<<tmp1<<" ;\n";
      t3 = t4;
