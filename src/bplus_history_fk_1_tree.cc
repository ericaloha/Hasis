...
// switch
const int async_mode_history_fk_1_ = 0; // 0 for sync IO; 1 for async IO
const int kAIOBatchSize_item = 20;
const int kOrder_history_fk_1_ = 700; //how many record a page can contain
const int kOrder_history_fk_1_Plog = kOrder_history_fk_1_ / 10;
// const int kOrder_history_fk_1_ = 50; //for key
const int kOrder_history_fk_1_Index = 2000; // for index

static_assert(kOrder_history_fk_1_ >= 3,
              "The order of B+Tree should be greater than or equal to 3.");


const int kMaxKeySize_history_fk_1_ = 48;
const int kMaxLSNSize_history_fk_1_ = 8;
// const int kMaxValueSize_history_fk_1_ = 256;
const int kMaxValueSize_history_fk_1_col1_ = 16;
const int kMaxValueSize_history_fk_1_col2_ = 16;
const int kMaxValueSize_history_fk_1_col3_ = 20;
const int kMaxValueSize_history_fk_1_col4_ = 8;
const int kMaxValueSize_history_fk_1_col5_ = 24; 
const int kMaxValueSize_history_fk_1_col_all = 16+16+20+8+24;
const off_t kMetaOffset_history_fk_1_ = 0;

int kMaxPages_history_fk_1_item = 1000;     // 100M
const int kMaxFreePages_history_fk_1_ = 1100; 
// const int kMaxPages_history_fk_1_item = 80000; //1G

typedef char Key[kMaxKeySize_history_fk_1_];
typedef char Value1[kMaxValueSize_history_fk_1_col1_];
typedef char Value2[kMaxValueSize_history_fk_1_col2_];
typedef char Value3[kMaxValueSize_history_fk_1_col3_];
typedef char Value4[kMaxValueSize_history_fk_1_col4_];
typedef char Value5[kMaxValueSize_history_fk_1_col5_];
typedef char Value_all[kMaxValueSize_history_fk_1_col_all]; //for log record
...
const int innerKeyoffset_history_fk_1_ = 0;
const int innercoloffset_history_fk_1_ = 0;

const int kMaxUniqueKInGLogSeg=3000;
  


struct BPlusHistoryfk1Tree::Monitor{
  Monitor(){
    bg_id =0;
    plog_insert = 0;
    leaf_insert = 0;
...
struct BPlusHistoryfk1Tree::Meta
{
...
struct BPlusHistoryfk1Tree::Index
{
  ...
  void UpdateIndex(off_t of, off_t lsn, const char *k)
  {
    offset = of;
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_history_fk_1_);
  }
...
//hkc-modify-2
struct BPlusHistoryfk1Tree::Record
{
  Key key;
  Value1 value_1;
  Value2 value_2;
  ...
  void UpdateKV(const char *k, const char *v1, const char *v2, const char *v3, const char *v4,const char *v5, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_history_fk_1_);
    strncpy(value_1, v1, kMaxValueSize_history_fk_1_col1_);
    strncpy(value_2, v2, kMaxValueSize_history_fk_1_col2_);
    strncpy(value_3, v3, kMaxValueSize_history_fk_1_col3_);
    strncpy(value_4, v4, kMaxValueSize_history_fk_1_col4_);
    strncpy(value_5, v5, kMaxValueSize_history_fk_1_col5_);
  }
  void UpdateKey(const char *k, off_t lsn)
  {
    cur_lsn = lsn;
...
//hkc-modify-3
struct BPlusHistoryfk1Tree::PAXKeyRecord
{
  Key key;
  off_t cur_lsn;
...
//hkc-modify-4
struct BPlusHistoryfk1Tree::PAXColumn1Record
{
  Value1 value_1;
...
//hkc-modify-5
struct BPlusHistoryfk1Tree::PAXColumn2Record
{
  Value2 value_2;
...
//hkc-modify-6
struct BPlusHistoryfk1Tree::PAXColumn3Record
{
  Value3 value_3;
...
//hkc-modify-7
struct BPlusHistoryfk1Tree::PAXColumn4Record
{
  Value4 value_4;
...
//hkc-modify-8
struct BPlusHistoryfk1Tree::PAXColumn5Record
{
  Value5 value_5;
...
struct BPlusHistoryfk1Tree::Per_page_Log
{
  Key key;
  Value_all value_all;
  off_t cur_lsn;
  ...
  void UpdateKV(const char *k, const char *v, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_history_fk_1_);
    strncpy(value_all, v, kMaxValueSize_history_fk_1_col_all);
  }
  void UpdateKey(const char *k, off_t lsn)
  {
    cur_lsn = lsn;
...
struct BPlusHistoryfk1Tree::LogRecord
{
  LogRecord() : page_offset(0), log_lsn(0), log_offset(0), next_log_offset(0), page_index(0)
  {
    memset(key, 0, kMaxKeySize_history_fk_1_);
    memset(value, 0, kMaxValueSize_history_fk_1_col_all);
  }
  ...
  char key[kMaxKeySize_history_fk_1_];
  char value[kMaxValueSize_history_fk_1_col_all];

  void UpdateKV(const char *k, const char *v)
  {
    strncpy(key, k, kMaxKeySize_history_fk_1_);
    strncpy(value, v, kMaxValueSize_history_fk_1_col_all);
  }
  void UpdateKey(const char *k) { strncpy(key, k, kMaxKeySize_history_fk_1_); }
  void UpdateValue(const char *v) { strncpy(value, v, kMaxValueSize_history_fk_1_col_all); }
};

struct BPlusHistoryfk1Tree::Checkpoint
{
...
struct BPlusHistoryfk1Tree::Node
{
...
struct BPlusHistoryfk1Tree::IndexNode : BPlusHistoryfk1Tree::Node
{
  IndexNode() = default;
  ~IndexNode() = default;
  ...
  const char *Key(int index) const
  {
    //assert(count > 0);
    //assert(index >= 0);
    //assert(index <= kOrder_history_fk_1_Index);
  ...
  Index indexes[kOrder_history_fk_1_Index + 1];
};

//hkc-modify-9
struct BPlusHistoryfk1Tree::LeafNode : BPlusHistoryfk1Tree::Node
{
  LeafNode() = default;
  ~LeafNode() = default;
  ...
  {
    //assert(count > 0);
    char* all = (char*)malloc(sizeof(char)*kMaxValueSize_history_fk_1_col_all);
    int off =0;
    memcpy(&all[off],records[index].value_1,kMaxValueSize_history_fk_1_col1_);
    off+=kMaxValueSize_history_fk_1_col1_;
    memcpy(&all[off],records[index].value_2,kMaxValueSize_history_fk_1_col2_);
    off+=kMaxValueSize_history_fk_1_col2_;
    memcpy(&all[off],records[index].value_3,kMaxValueSize_history_fk_1_col3_);
    off+=kMaxValueSize_history_fk_1_col3_;
    memcpy(&all[off],records[index].value_4,kMaxValueSize_history_fk_1_col4_);
    off+=kMaxValueSize_history_fk_1_col4_;
    memcpy(&all[off],records[index].value_5,kMaxValueSize_history_fk_1_col5_);
    off+=kMaxValueSize_history_fk_1_col5_;
    return all;
  }
  ...
  {
    //assert(index >= 0);
    //assert(index < kOrder_history_fk_1_);
  ...
  off_t offset_v4;
  off_t offset_v5;
  BPlusHistoryfk1Tree::Record records[kOrder_history_fk_1_];
  // BPlusHistoryfk1Tree::Record Per_page_logs[kOrder_history_fk_1_/2];
...
//hkc-modify-10
struct BPlusHistoryfk1Tree::Plog : BPlusHistoryfk1Tree::Node
{
  Plog() = default;
  ~Plog() = default;
  ...
  {
    //assert(index >= 0);
    //assert(index < kOrder_history_fk_1_Plog);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateKV(index, k,v1,v2,v3,v4, v5, lsn);
  }
  ...
  // BPlusHistoryfk1Tree::Record records[kOrder_history_fk_1_/2];
  BPlusHistoryfk1Tree::Record records[kOrder_history_fk_1_Plog];
};

class BPlusHistoryfk1Tree::BlockCache
{
  struct Node;
...
public:
  BlockCache(bool enable_backthd)
  {
    ...
    // pre_allocate
    // 2. init free list
    char *init_ptr = (char *)malloc(kMaxFreePages_history_fk_1_ * kLeafPageSize); // i.e., for each page
    memset(init_ptr, 0, kMaxFreePages_history_fk_1_ * kLeafPageSize);

    char *flag_addr = init_ptr;
    for (int i = 0; i < kMaxFreePages_history_fk_1_; i++)
    {
...
void RowToColumn_Cache(char* initial_page, char* converted_page){
  ...
  row_leaf->offset_v5 = (4+40+8+12+12+16+8)*1024;
  ...
  //convert row as column and fit in sub pages
    ...
    /*
      const int kMaxKeySize_history_fk_1_ = 16; 16K
      const int kMaxValueSize_history_fk_1_col1_ = 16; 16K
      const int kMaxValueSize_history_fk_1_col2_ = 24; 24K
      const int kMaxValueSize_history_fk_1_col3_ = 8; 8K
      const int kMaxValueSize_history_fk_1_col4_ = 50; 50K
    */
  ...
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&buf[k_of],&(row_leaf->records[i].key),kMaxKeySize_history_fk_1_);
    k_of+=kMaxKeySize_history_fk_1_;
    memcpy(&buf[lsn_of],&(row_leaf->records[i].cur_lsn),sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&buf[v1_of],&(row_leaf->records[i].value_1),kMaxValueSize_history_fk_1_col1_);
    v1_of+=kMaxValueSize_history_fk_1_col1_;
    memcpy(&buf[v2_of],&(row_leaf->records[i].value_2),kMaxValueSize_history_fk_1_col2_);
    v2_of+=kMaxValueSize_history_fk_1_col2_;
    memcpy(&buf[v3_of],&(row_leaf->records[i].value_3),kMaxValueSize_history_fk_1_col3_);
    v3_of+=kMaxValueSize_history_fk_1_col3_;
    memcpy(&buf[v4_of],&(row_leaf->records[i].value_4),kMaxValueSize_history_fk_1_col4_);
    v4_of+=kMaxValueSize_history_fk_1_col4_;
    memcpy(&buf[v5_of],&(row_leaf->records[i].value_5),kMaxValueSize_history_fk_1_col5_);
    v5_of+=kMaxValueSize_history_fk_1_col5_;
  }
...
void ColumnToRow_Cache(char* column_page, char* row_page){
  ...
  off_t v5_of = row_leaf->offset_v5;
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&(row_leaf->records[i].key),&buf[k_of],kMaxKeySize_history_fk_1_);
    k_of+=kMaxKeySize_history_fk_1_;
    memcpy(&(row_leaf->records[i].cur_lsn),&buf[lsn_of],sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&(row_leaf->records[i].value_1),&buf[v1_of],kMaxValueSize_history_fk_1_col1_);
    v1_of+=kMaxValueSize_history_fk_1_col1_;
    memcpy(&(row_leaf->records[i].value_2),&buf[v2_of],kMaxValueSize_history_fk_1_col2_);
    v2_of+=kMaxValueSize_history_fk_1_col2_;
    memcpy(&(row_leaf->records[i].value_3),&buf[v3_of],kMaxValueSize_history_fk_1_col3_);
    v3_of+=kMaxValueSize_history_fk_1_col3_;
    memcpy(&(row_leaf->records[i].value_4),&buf[v4_of],kMaxValueSize_history_fk_1_col4_);
    v4_of+=kMaxValueSize_history_fk_1_col4_;
    memcpy(&(row_leaf->records[i].value_5),&buf[v5_of],kMaxValueSize_history_fk_1_col5_);
    v5_of+=kMaxValueSize_history_fk_1_col5_;
  }
  ...
  void Put(T *block, int is_plog)
  {
    ...
    if (async_mode_history_fk_1_ == 0)
    {
      if (size_lru > kMaxPages_history_fk_1_item * 0.9)
      {
        KickLRUbyUring();
      }
      if (mtr_cache->plog_flag != 1 && mtr_cache->FLU_disabled == 0 && size_flu > kMaxPages_history_fk_1_item * 0.5)
      {
        KickLFUbyUring();
      }
    }
  ...
  void SetMtr(BPlusHistoryfk1Tree::Monitor *mtr_1){
    mtr_cache = mtr_1;
  }
    ...
    for (i = 0; i < Queue_depth; i++)
    {
      ...
      if(debug_flag==1){
        ...
        if( (leaf->count!=row_leaf_test->count) || 
            ...
            (leaf->sha_plog_offset!=row_leaf_test->sha_plog_offset) || //hkc-ap-08
...
class BPlusHistoryfk1Tree::InternalCache
{
  struct Node;
...
public:
  ...
  void SetMtr(BPlusHistoryfk1Tree::Monitor *mtr_2){
    mtr_internal = mtr_2;
  }
...
//hkc-ap-10*
void BPlusHistoryfk1Tree::ScanAP_impl(off_t &left, off_t &right, std::string &value){
...
//hkc-ap-10
void BPlusHistoryfk1Tree::APWorker(){
  sleep(20);
  while(true){
...
BPlusHistoryfk1Tree::BPlusHistoryfk1Tree(const char *path, const char *log_path, const char *chpt_path, const int bg_id_, const bool enable_backthd, const char *shadow_plog_path)
    : fd_(open(path, O_CREAT|O_DIRECT|O_RDWR)),fd_shadow_plog_(open(shadow_plog_path, O_CREAT|O_DIRECT|O_RDWR)),log_fd_(open(log_path, O_CREAT|O_APPEND|O_RDWR)),chpt_fd_(open(chpt_path, O_CREAT|O_APPEND|O_RDWR)), block_cache_(new BlockCache(enable_backthd)), internal_cache_(new InternalCache())
{
  ...
  if (fd_ == -1)
    //Exit("open db");
    printf("open historyfk1 db failed.\n");
  if (log_fd_ == -1)
    //Exit("open log");
    printf("open historyfk1 log failed.\n");
  if (chpt_fd_ == -1)
    //Exit("open chpt");
    printf("open historyfk1 chpt failed.\n");
  ...
  // meta_ = Map<Meta>(kMetaOffset_history_fk_1_);
  meta_ = new Meta();
  ...
  {
    // Initialize B+tree;
    constexpr off_t of_root = kMetaOffset_history_fk_1_ + kUnitNodeSize;
    meta_->block = of_root;
  ...
  //hkc-ap-14
  if(scan_ap){
    std::thread thd_ap = std::thread(&BPlusHistoryfk1Tree::APWorker,this);
    thd_ap.detach();
  }

}

BPlusHistoryfk1Tree::~BPlusHistoryfk1Tree()
{
  UnMap(meta_, 0, 0);
  delete block_cache_;
  close(fd_);
...
//hkc-ap-15
int BPlusHistoryfk1Tree::PrepareReadSetAP(off_t left, off_t right, void *scan_ptr){
...
void BPlusHistoryfk1Tree::SetPlog(){
  mtr_->plog_flag =1;
  mtr_->FLU_disabled =1;
  mtr_->warmup_flag =1;
  return;
...
void BPlusHistoryfk1Tree::GetStatic(int &hits_bp, int &hits_glog, int &hits_plog, int &disk_read, int &io_consume, int &error_cnt, int &dirtys, int &cleans){
...
off_t BPlusHistoryfk1Tree::GetLSN(){
  return mtr_->global_lsn;
}
void BPlusHistoryfk1Tree::IncreLSN(){
  mtr_->global_lsn++;
  return;
}

void BPlusHistoryfk1Tree::ResetStatic(){
    mtr_->hits_bp = 0; 
    mtr_->hits_glog = 0; 
    mtr_->hits_plog =0;
    mtr_->disk_read=0;
...
void BPlusHistoryfk1Tree::SetBaseline(){
  mtr_->FLU_disabled = 0;
  mtr_->plog_flag =-1;
  mtr_->warmup_flag =0;
}

void BPlusHistoryfk1Tree::SetIOStaic(){
    mtr_->plog_insert=0;
    mtr_->leaf_insert=0;
    mtr_->leaf_split=0;
...
off_t BPlusHistoryfk1Tree::GetChpt(){
  return mtr_->normal_chpt_now;
}
void BPlusHistoryfk1Tree::SetLoadFlag(int flag){
  mtr_->load_flag = flag;
}
void BPlusHistoryfk1Tree::DisableFLUOrNot(int flag){
  mtr_->FLU_disabled = flag;
}
void BPlusHistoryfk1Tree::LoadIOStatic(int &plog_insert,int &leaf_insert,int &leaf_split,int &phy_plog_write,int &phy_plog_read,int &phy_page_read,int &phy_page_write,int &logi_page_read,int &logi_page_write){
...
int BPlusHistoryfk1Tree::ApplyPendPages(std::map<off_t, std::pair<char*,char*>>* source, std::map<off_t, std::pair<char*,char*>>* target, std::map<off_t,std::pair<int,int>> *EmptyPages){
...
void BPlusHistoryfk1Tree::BackGround_GLog_Func()
{
  int total_KV=0;
  int final_KV=0;
  int total_Plog =0;
      ...
      if(now_lsn > cur_lsn){
        ...
        if (InsertKVIntoPlog(plog, key.data(), value1.data(),value2.data(),value3.data(),value4.data(),value5.data(), cur_lsn) <=
          ...
          */
          ...
          for (int i = 0; i < kOrder_history_fk_1_Plog; i++){
...
//hkc-modify-21
void BPlusHistoryfk1Tree::RowToColumn(char* initial_page, char* converted_page){
...
//hkc-modify-12
void BPlusHistoryfk1Tree::ColumnToRow(char* column_page, char* row_page){
  memset(row_page,0,kLeafPageSize);
  LeafNode* row_leaf = reinterpret_cast<LeafNode*>(row_page);
...
//hkc-gb-04
off_t BPlusHistoryfk1Tree::GetOnlyLeafOffset(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
  return of_leaf;
}
//hkc-gb-05
char* BPlusHistoryfk1Tree::GetLeafCopy(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
...
//hkc-gb-06
void BPlusHistoryfk1Tree::UpdateLeafCopy(const std::string &key, const std::string &value, off_t lsn, char* leaf_copy){
  ...
  std::string value4 = value.substr(52,16);
  std::string value5 = value.substr(68,16);
  if (InsertKVIntoLeafNode(leaf, key.data(),  value1.data(),value2.data(),value3.data(),
          value4.data(),value5.data(),lsn) <= GetMaxKeys(1)){
...
//hkc-gb-07
void BPlusHistoryfk1Tree::GetRowRecord(const std::string &key, std::string &value, char* leaf_copy) {
  LeafNode* leaf  = reinterpret_cast<LeafNode*>(&leaf_copy[0]);
  int index = GetIndexFromLeafNode(leaf, key.data());
  char* ptr = leaf->ValueALL(index);
  std::string tmp_value(ptr,ptr+kMaxValueSize_history_fk_1_col_all);
  value = tmp_value;
  free(ptr);
  return ;
...
//hkc-gb-08
void BPlusHistoryfk1Tree::TriggerFlush(const off_t* offsets, const int size, char* evict_buf){
  int batch_size = 200;
...
void BPlusHistoryfk1Tree::Put(const std::string &key, const std::string &value, off_t lsn)
{
  ...
  if (InsertKVIntoLeafNode(leaf_node, key.data(), value1.data(),value2.data(),value3.data(),value4.data(),value5.data(), lsn) <=
      GetMaxKeys(1))
  {
    // 2.If records of leaf node less than or equals kOrder_history_fk_1_ - 1 then finish.
  ...
  // until count <= kOrder_history_fk_1_ - 1.
  size_t count;
  do
  {
...
/*
bool BPlusHistoryfk1Tree::Get(const std::string &key, std::string &value) const
{
...
bool BPlusHistoryfk1Tree::Scan(const std::string &start, const std::string &end, std::string &value) const
{
  if(start>end){
    int x=1;
  }
...
//hkc-ap-19
bool BPlusHistoryfk1Tree::ScanAP(const std::string &start, const std::string &end, std::string &value) const
{
...
int BPlusHistoryfk1Tree::Check(off_t offset)
{
  return block_cache_->Check_Leaf(offset);
}

char *BPlusHistoryfk1Tree::GetMemCopy(off_t offset)
{
  return block_cache_->GetMemCopy_Leaf(offset);
}

template <typename T>
T *BPlusHistoryfk1Tree::Map(off_t offset, int isleaf, int is_plog, char *&plog_copy) const
{
  if (isleaf == 1)
  {
...
template <typename T>
void BPlusHistoryfk1Tree::InsertDirty(off_t offset) const
{
...
template <typename T>
void BPlusHistoryfk1Tree::RelocateDirty(off_t offset, off_t ori_oldset_lsn) const
{
...
template <typename T>
void BPlusHistoryfk1Tree::DirectInsertDirty(off_t offset_leaf, off_t offset_split) const
{
...
template <typename T>
void BPlusHistoryfk1Tree::UnMap(T *map_obj, int isleaf, int is_plog) const
{
  if (isleaf == 1)
  {
...
size_t BPlusHistoryfk1Tree::GetMinKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return (kOrder_history_fk_1_Plog + 1) / 2 - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return (kOrder_history_fk_1_ + 1) / 2 - 1;
  }
}
// 1: leaf node ; 0: plog
size_t BPlusHistoryfk1Tree::GetMaxKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return kOrder_history_fk_1_Plog - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return kOrder_history_fk_1_ - 1;
  }
}
size_t BPlusHistoryfk1Tree::GetMinIndexKeys() { return (kOrder_history_fk_1_Index + 1) / 2 - 1; }
size_t BPlusHistoryfk1Tree::GetMaxIndexKeys() { return kOrder_history_fk_1_Index - 1; }

BPlusHistoryfk1Tree::IndexNode *BPlusHistoryfk1Tree::GetOrCreateParent(Node *node)
{
  if (node->parent == 0)
  {
    // Split root node.
...
template <typename T>
int BPlusHistoryfk1Tree::UpperBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
  ...
  {
    int mid = (l + r) >> 1;
    if (std::strncmp(arr[mid].key, key, kMaxKeySize_history_fk_1_) <= 0)
    {
      l = mid + 1;
    }
    else
...
template <typename T>
int BPlusHistoryfk1Tree::UpperBoundIndex(T arr[], int n, const char *key) const
{
...
template <typename T>
int BPlusHistoryfk1Tree::LowerBoundIndex(T arr[], int n, const char *key) const
{
  ////assert(n <= GetMaxIndexKeys());
  int l = 0, r = n - 1;
  while (l <= r)
...
// flag: 1: data page; 0: plog
template <typename T>
int BPlusHistoryfk1Tree::LowerBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
    //assert(n <= GetMaxKeys(0));
...
template <typename T>
T *BPlusHistoryfk1Tree::AllocLeaf()
...
template <typename T>
T *BPlusHistoryfk1Tree::AllocIndex()
{ // 1: leaf node, 0: internal node
...
template <typename T>
void BPlusHistoryfk1Tree::Dealloc(T *node)
{
  UnMap<T>(node);
}

void BPlusHistoryfk1Tree::WarmUpVirtualPageCache(int x) const
{
  return block_cache_->WarmUpVPCache(x);
}
off_t BPlusHistoryfk1Tree::GetLeafOffset(const char *key) const
{
...
inline size_t BPlusHistoryfk1Tree::InsertKeyIntoIndexNode(IndexNode *index_node,
                                                const char *key,
                                                Node *left_node,
...
//hkc-modify-13
size_t BPlusHistoryfk1Tree::InsertKVIntoLeafNode(LeafNode *leaf_node, const char *key,
                                       const char *value_1, const char *value_2, const char *value_3, const char *value_4, const char *value_5, off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(leaf_node->Key(index - 1), key, kMaxKeySize_history_fk_1_) == 0)
  {
...
//hkc-modify-14
size_t BPlusHistoryfk1Tree::InsertKVIntoPlog(Plog *plog, const char *key,
                                   const char* value1, const char* value2,const char* value3,const char* value4,const char* value5, off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(plog->Key(index - 1), key, kMaxKeySize_history_fk_1_) == 0)
  {
...
BPlusHistoryfk1Tree::LeafNode *BPlusHistoryfk1Tree::SplitLeafNode(LeafNode *leaf_node)
{
  //assert(leaf_node->count == kOrder_history_fk_1_);
  off_t origian_lsn = leaf_node->GetSmallestLSN();
  constexpr int mid = (kOrder_history_fk_1_ - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_history_fk_1_ - mid;
...
BPlusHistoryfk1Tree::IndexNode *BPlusHistoryfk1Tree::SplitIndexNode(IndexNode *index_node)
{
  //assert(index_node->count == kOrder_history_fk_1_Index);
  constexpr int mid = (kOrder_history_fk_1_Index - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_history_fk_1_Index - mid - 1;
  ...
  for (int i = mid + 1; i <= kOrder_history_fk_1_Index; ++i)
  {
...
inline int BPlusHistoryfk1Tree::GetIndexFromLeafNode(LeafNode *leaf_node,
                                           const char *key) const
{
  int index = LowerBound(leaf_node->records, leaf_node->count, key, 1);
  return index < static_cast<int>(leaf_node->count) &&
                 std::strncmp(leaf_node->Key(index), key, kMaxKeySize_history_fk_1_) == 0
             ? index
             : -1;
}

inline int BPlusHistoryfk1Tree::GetIndexFromPlog(Plog *plog,
                                       const char *key) const
{
  int index = LowerBound(plog->records, plog->count, key, 0);
  return index < static_cast<int>(plog->count) &&
                 std::strncmp(plog->Key(index), key, kMaxKeySize_history_fk_1_) == 0
             ? index
             : -1;
}

std::vector<std::pair<std::string, std::string>> BPlusHistoryfk1Tree::GetRange(
    const std::string &left_key, const std::string &right_key) const
{
  ...
  while (of_leaf != 0 && !finish)
  {
    ...
    {
      if (strncmp(right_leaf_node->Key(i), right_key.data(), kMaxKeySize_history_fk_1_) <=
          0)
      {
...
bool BPlusHistoryfk1Tree::Empty() const { return meta_->size == 0; }

size_t BPlusHistoryfk1Tree::Size() const { return meta_->size; }