...
// switch
const int async_mode_customer_ = 0; // 0 for sync IO; 1 for async IO
const int kAIOBatchSize_item = 20;
const int kOrder_customer_ = 240; //how many record a page can contain
const int kOrder_customer_Plog = kOrder_customer_ / 6;
// const int kOrder_customer_ = 50; //for key
const int kOrder_customer_Index = 1500; // for index

static_assert(kOrder_customer_ >= 3,
              "The order of B+Tree should be greater than or equal to 3.");


const int kMaxKeySize_customer_ = 48;
const int kMaxLSNSize_customer_ = 8;
// const int kMaxValueSize_customer_ = 256;
const int kMaxValueSize_customer_col1_ = 16;
const int kMaxValueSize_customer_col2_ = 2;
const int kMaxValueSize_customer_col3_ = 16;
const int kMaxValueSize_customer_col4_ = 20;
const int kMaxValueSize_customer_col5_ = 20; 
const int kMaxValueSize_customer_col6_ = 20; 
const int kMaxValueSize_customer_col7_ = 2;
const int kMaxValueSize_customer_col8_ = 9; 
const int kMaxValueSize_customer_col9_ = 16; 
const int kMaxValueSize_customer_col10_ = 20;
const int kMaxValueSize_customer_col11_ = 2; 
const int kMaxValueSize_customer_col12_ = 16; 
const int kMaxValueSize_customer_col13_ = 6;
const int kMaxValueSize_customer_col14_ = 14; 
const int kMaxValueSize_customer_col15_ = 14; 
const int kMaxValueSize_customer_col16_ = 16;
const int kMaxValueSize_customer_col17_ = 16; 
const int kMaxValueSize_customer_col18_ = 100; 

//hkc-plog-1
const int KPosBalance = 16+2+16+20+20+20+2+9+16+20+2+16+6;
const int KPosData    = 16+2+16+20+20+20+2+9+16+20+2+16+6+14+14+16+16;

const int kMaxValueSize_customer_col_all = 16+2+16+20+20+20+2+9+16+20+2+16+6+14+14+16+16+100;
const off_t kMetaOffset_customer_ = 0;

int kMaxPages_customer_item = 500;     // 100M
const int kMaxFreePages_customer_ = 600; 
// const int kMaxPages_customer_item = 80000; //1G

typedef char Key[kMaxKeySize_customer_];
typedef char Value1[kMaxValueSize_customer_col1_];
typedef char Value2[kMaxValueSize_customer_col2_];
typedef char Value3[kMaxValueSize_customer_col3_];
typedef char Value4[kMaxValueSize_customer_col4_];
typedef char Value5[kMaxValueSize_customer_col5_];
typedef char Value6[kMaxValueSize_customer_col6_];
typedef char Value7[kMaxValueSize_customer_col7_];
typedef char Value8[kMaxValueSize_customer_col8_];
typedef char Value9[kMaxValueSize_customer_col9_];
typedef char Value10[kMaxValueSize_customer_col10_];
typedef char Value11[kMaxValueSize_customer_col11_];
typedef char Value12[kMaxValueSize_customer_col12_];
typedef char Value13[kMaxValueSize_customer_col13_];
typedef char Value14[kMaxValueSize_customer_col14_];
typedef char Value15[kMaxValueSize_customer_col15_];
typedef char Value16[kMaxValueSize_customer_col16_];
typedef char Value17[kMaxValueSize_customer_col17_];
typedef char Value18[kMaxValueSize_customer_col18_];

typedef char Value_all[kMaxValueSize_customer_col_all]; //for log record
...
const int innerKeyoffset_customer_ = 0;
const int innercoloffset_customer_ = 0;

const int kMaxUniqueKInGLogSeg=3000;
  
  

struct BPlusCustomerTree::Monitor{
  Monitor(){
    bg_id =0;
    plog_insert = 0;
    leaf_insert = 0;
...
struct BPlusCustomerTree::Index
{
  ...
  void UpdateIndex(off_t of, off_t lsn, const char *k)
  {
    offset = of;
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_customer_);
  }
...
//hkc-modify-2
struct BPlusCustomerTree::Record
{
  Key key;
  Value1 value_1;
  Value2 value_2;
  ...
  void UpdateKV(
    const char *k, 
    const char *v1, const char *v2, const char *v3, const char *v4,const char *v5,const char *v6, 
    const char *v7, const char *v8, const char *v9, const char *v10,const char *v11,const char *v12, 
    const char *v13, const char *v14, const char *v15, const char *v16,const char *v17,const char *v18, 
    off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_customer_);
    strncpy(value_1, v1, kMaxValueSize_customer_col1_);
    strncpy(value_2, v2, kMaxValueSize_customer_col2_);
    strncpy(value_3, v3, kMaxValueSize_customer_col3_);
    strncpy(value_4, v4, kMaxValueSize_customer_col4_);
    strncpy(value_5, v5, kMaxValueSize_customer_col5_);
    strncpy(value_6, v6, kMaxValueSize_customer_col6_);
    strncpy(value_7, v7, kMaxValueSize_customer_col7_);
    strncpy(value_8, v8, kMaxValueSize_customer_col8_);
    strncpy(value_9, v9, kMaxValueSize_customer_col9_);
    strncpy(value_10, v10, kMaxValueSize_customer_col10_);
    strncpy(value_11, v11, kMaxValueSize_customer_col11_);
    strncpy(value_12, v12, kMaxValueSize_customer_col12_);
    strncpy(value_13, v13, kMaxValueSize_customer_col13_);
    strncpy(value_14, v14, kMaxValueSize_customer_col14_);
    strncpy(value_15, v15, kMaxValueSize_customer_col15_);
    strncpy(value_16, v16, kMaxValueSize_customer_col16_);
    strncpy(value_17, v17, kMaxValueSize_customer_col17_);
    strncpy(value_18, v18, kMaxValueSize_customer_col18_);
  }

  //hkc-plog-2
  void UpdateDeltaKV(const char *k,const char *v_balance, const char *v_data,off_t lsn)
  {
    cur_lsn = lsn;
    partial = true;
    strncpy(key, k, kMaxKeySize_customer_);
    strncpy(value_14, v_balance, kMaxValueSize_customer_col14_);
    strncpy(value_18, v_data, kMaxValueSize_customer_col18_);
  }
  ...
  //hkc-plog-3
  void UpdateValueDelta(const char *v_balance, const char *v_data,off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(value_14, v_balance, kMaxValueSize_customer_col14_);
    strncpy(value_18, v_data, kMaxValueSize_customer_col18_);
...
struct BPlusCustomerTree::PAXKeyRecord
{
  Key key;
  off_t cur_lsn;
...
struct BPlusCustomerTree::PAXColumn1Record
{
  Value1 value_1;
...
struct BPlusCustomerTree::PAXColumn2Record
{
  Value2 value_2;
...
struct BPlusCustomerTree::PAXColumn3Record
{
  Value3 value_3;
...
struct BPlusCustomerTree::PAXColumn4Record
{
  Value4 value_4;
...
//hkc-modify-8
struct BPlusCustomerTree::PAXColumn5Record
{
  Value5 value_5;
...
//hkc-modify-9
struct BPlusCustomerTree::PAXColumn6Record
{
  Value6 value_6;

  void UpdateKV( const char *v6, off_t lsn)
  {
    strncpy(value_6, v6, kMaxValueSize_customer_col6_);
  }

  void UpdateValue(const char *v6, off_t lsn)
  {
    strncpy(value_6, v6, kMaxValueSize_customer_col6_);
  }
};

struct BPlusCustomerTree::PAXColumn7Record
{
  Value7 value_7;

  void UpdateKV( const char *v7, off_t lsn)
  {
    strncpy(value_7, v7, kMaxValueSize_customer_col7_);
  }

  void UpdateValue(const char *v7, off_t lsn)
  {
    strncpy(value_7, v7, kMaxValueSize_customer_col7_);
  }
};

struct BPlusCustomerTree::PAXColumn8Record
{
  Value8 value_8;

  void UpdateKV( const char *v8, off_t lsn)
  {
    strncpy(value_8, v8, kMaxValueSize_customer_col8_);
  }

  void UpdateValue(const char *v8, off_t lsn)
  {
    strncpy(value_8, v8, kMaxValueSize_customer_col8_);
  }
};

struct BPlusCustomerTree::PAXColumn9Record
{
  Value9 value_9;

  void UpdateKV( const char *v9, off_t lsn)
  {
    strncpy(value_9, v9, kMaxValueSize_customer_col9_);
  }

  void UpdateValue(const char *v9, off_t lsn)
  {
    strncpy(value_9, v9, kMaxValueSize_customer_col9_);
  }
};
...
struct BPlusCustomerTree::PAXColumn10Record
{
  Value10 value_10;

  void UpdateKV(const char *k, const char *v10, off_t lsn)
  {
    strncpy(value_10, v10, kMaxValueSize_customer_col10_);
  }

  void UpdateValue(const char *v10)
  {
    strncpy(value_10, v10, kMaxValueSize_customer_col10_);
  }
};
...
struct BPlusCustomerTree::PAXColumn11Record
{
  Value11 value_11;

  void UpdateKV(const char *k, const char *v11, off_t lsn)
  {
    strncpy(value_11, v11, kMaxValueSize_customer_col11_);
  }

  void UpdateValue(const char *v11)
  {
    strncpy(value_11, v11, kMaxValueSize_customer_col11_);
  }
};
...
struct BPlusCustomerTree::PAXColumn12Record
{
  Value12 value_12;

  void UpdateKV(const char *k, const char *v12, off_t lsn)
  {
    strncpy(value_12, v12, kMaxValueSize_customer_col12_);
  }

  void UpdateValue(const char *v12)
  {
    strncpy(value_12, v12, kMaxValueSize_customer_col12_);
  }
};
...
struct BPlusCustomerTree::PAXColumn13Record
{
  Value13 value_13;

  void UpdateKV(const char *k, const char *v13, off_t lsn)
  {
    strncpy(value_13, v13, kMaxValueSize_customer_col13_);
  }

  void UpdateValue(const char *v13)
  {
    strncpy(value_13, v13, kMaxValueSize_customer_col13_);
  }
};
...
struct BPlusCustomerTree::PAXColumn14Record
{
  Value14 value_14;

  void UpdateKV(const char *k, const char *v14, off_t lsn)
  {
    strncpy(value_14, v14, kMaxValueSize_customer_col14_);
  }

  void UpdateValue(const char *v14)
  {
    strncpy(value_14, v14, kMaxValueSize_customer_col14_);
  }
};
...
struct BPlusCustomerTree::PAXColumn15Record
{
  Value15 value_15;

  void UpdateKV(const char *k, const char *v15, off_t lsn)
  {
    strncpy(value_15, v15, kMaxValueSize_customer_col15_);
  }

  void UpdateValue(const char *v15)
  {
    strncpy(value_15, v15, kMaxValueSize_customer_col15_);
  }
};
...
struct BPlusCustomerTree::PAXColumn16Record
{
  Value16 value_16;

  void UpdateKV(const char *k, const char *v16, off_t lsn)
  {
    strncpy(value_16, v16, kMaxValueSize_customer_col16_);
  }

  void UpdateValue(const char *v16)
  {
    strncpy(value_16, v16, kMaxValueSize_customer_col16_);
  }
};
...
struct BPlusCustomerTree::PAXColumn17Record
{
  Value17 value_17;

  void UpdateKV(const char *k, const char *v17, off_t lsn)
  {
    strncpy(value_17, v17, kMaxValueSize_customer_col17_);
  }

  void UpdateValue(const char *v17)
  {
    strncpy(value_17, v17, kMaxValueSize_customer_col17_);
  }
};
...
struct BPlusCustomerTree::PAXColumn18Record
{
  Value18 value_18;

  void UpdateKV(const char *k, const char *v18, off_t lsn)
  {
    strncpy(value_18, v18, kMaxValueSize_customer_col18_);
  }

  void UpdateValue(const char *v18)
  {
    strncpy(value_18, v18, kMaxValueSize_customer_col18_);
  }
};
...
struct BPlusCustomerTree::Per_page_Log
{
  Key key;
  Value_all value_all;
  off_t cur_lsn;
  ...
  void UpdateKV(const char *k, const char *v, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_customer_);
    strncpy(value_all, v, kMaxValueSize_customer_col_all);
  }
  void UpdateKey(const char *k, off_t lsn)
  {
    cur_lsn = lsn;
...
struct BPlusCustomerTree::LogRecord
{
  LogRecord() : page_offset(0), log_lsn(0), log_offset(0), next_log_offset(0), page_index(0)
  {
    memset(key, 0, kMaxKeySize_customer_);
    memset(value, 0, kMaxValueSize_customer_col_all);
  }
  ...
  char key[kMaxKeySize_customer_];
  char value[kMaxValueSize_customer_col_all];

  void UpdateKV(const char *k, const char *v)
  {
    strncpy(key, k, kMaxKeySize_customer_);
    strncpy(value, v, kMaxValueSize_customer_col_all);
  }
  void UpdateKey(const char *k) { strncpy(key, k, kMaxKeySize_customer_); }
  void UpdateValue(const char *v) { strncpy(value, v, kMaxValueSize_customer_col_all); }
};

struct BPlusCustomerTree::Checkpoint
{
...
struct BPlusCustomerTree::IndexNode : BPlusCustomerTree::Node
{
  IndexNode() = default;
  ~IndexNode() = default;
  ...
  const char *Key(int index) const
  {
    //assert(count > 0);
    //assert(index >= 0);
    //assert(index <= kOrder_customer_Index);
  ...
  Index indexes[kOrder_customer_Index + 1];
};

//hkc-modify-9
struct BPlusCustomerTree::LeafNode : BPlusCustomerTree::Node
{
  LeafNode() = default;
  ~LeafNode() = default;
  ...
  {
    //assert(count > 0);
    char* all = (char*)malloc(sizeof(char)*kMaxValueSize_customer_col_all);
    int off =0;
    memcpy(&all[off],records[index].value_1,kMaxValueSize_customer_col1_);
    off+=kMaxValueSize_customer_col1_;
    memcpy(&all[off],records[index].value_2,kMaxValueSize_customer_col2_);
    off+=kMaxValueSize_customer_col2_;
    memcpy(&all[off],records[index].value_3,kMaxValueSize_customer_col3_);
    off+=kMaxValueSize_customer_col3_;
    memcpy(&all[off],records[index].value_4,kMaxValueSize_customer_col4_);
    off+=kMaxValueSize_customer_col4_;
    memcpy(&all[off],records[index].value_5,kMaxValueSize_customer_col5_);
    off+=kMaxValueSize_customer_col5_;
    memcpy(&all[off],records[index].value_6,kMaxValueSize_customer_col6_);
    off+=kMaxValueSize_customer_col6_;
    memcpy(&all[off],records[index].value_7,kMaxValueSize_customer_col7_);
    off+=kMaxValueSize_customer_col7_;
    memcpy(&all[off],records[index].value_8,kMaxValueSize_customer_col8_);
    off+=kMaxValueSize_customer_col8_;
    memcpy(&all[off],records[index].value_9,kMaxValueSize_customer_col9_);
    off+=kMaxValueSize_customer_col9_;
    memcpy(&all[off],records[index].value_10,kMaxValueSize_customer_col10_);
    off+=kMaxValueSize_customer_col10_;
    memcpy(&all[off],records[index].value_11,kMaxValueSize_customer_col11_);
    off+=kMaxValueSize_customer_col11_;
    memcpy(&all[off],records[index].value_12,kMaxValueSize_customer_col12_);
    off+=kMaxValueSize_customer_col12_;
    memcpy(&all[off],records[index].value_13,kMaxValueSize_customer_col13_);
    off+=kMaxValueSize_customer_col13_;
    memcpy(&all[off],records[index].value_14,kMaxValueSize_customer_col14_);
    off+=kMaxValueSize_customer_col14_;
    memcpy(&all[off],records[index].value_15,kMaxValueSize_customer_col15_);
    off+=kMaxValueSize_customer_col15_;
    memcpy(&all[off],records[index].value_16,kMaxValueSize_customer_col16_);
    off+=kMaxValueSize_customer_col16_;
    memcpy(&all[off],records[index].value_17,kMaxValueSize_customer_col17_);
    off+=kMaxValueSize_customer_col17_;
    memcpy(&all[off],records[index].value_18,kMaxValueSize_customer_col18_);
    off+=kMaxValueSize_customer_col18_;
  ...
  void UpdateValue(int index, const char *v1, const char *v2, const char *v3, const char *v4,const char *v5,const char *v6, 
    const char *v7, const char *v8, const char *v9, const char *v10,const char *v11,const char *v12, 
    const char *v13, const char *v14, const char *v15, const char *v16,const char *v17,const char *v18,off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateValue(v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,v11,v12,v13,v14,v15,v16,v17,v18, lsn);
  }
  //hkc-plog-4
  void UpdateValueDelta(int index, const char *v_balance, const char *v_data,off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateValueDelta(v_balance,v_data, lsn);
  }
  ...
  void UpdateKV(int index, const char *k, const char *v1, const char *v2, const char *v3, const char *v4,const char *v5,const char *v6, 
    const char *v7, const char *v8, const char *v9, const char *v10,const char *v11,const char *v12, 
    const char *v13, const char *v14, const char *v15, const char *v16,const char *v17,const char *v18,off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateKV(k, v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,v11,v12,v13,v14,v15,v16,v17,v18,lsn);
  }
  //hkc-plog-5
  void UpdateDeltaKV(int index, const char *k, const char *v_balance, const char *v_data,off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateDeltaKV(k, v_balance,v_data,lsn);
  }

  void AppendKV(int index, const char *k, const char *v1, const char *v2, const char *v3, const char *v4,const char *v5,const char *v6, 
    const char *v7, const char *v8, const char *v9, const char *v10,const char *v11,const char *v12, 
    const char *v13, const char *v14, const char *v15, const char *v16,const char *v17,const char *v18, off_t lsn)
  {
    //assert(index >= 0);
  ...
  void InsertKVAtIndex(int index, const char *k, const char *v1, const char *v2, const char *v3, const char *v4,const char *v5,const char *v6, 
    const char *v7, const char *v8, const char *v9, const char *v10,const char *v11,const char *v12, 
    const char *v13, const char *v14, const char *v15, const char *v16,const char *v17,const char *v18,off_t lsn)
  {
    //assert(index >= 0);
    ...
    UpdateKV(index, k, v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,v11,v12,v13,v14,v15,v16,v17,v18, lsn);
  }

  //hkc-plog-6
  void InsertDeltaKVAtIndex(int index, const char *k, const char *v_balance, const char *v_data,off_t lsn)
  {
    //assert(index >= 0);
    //assert(index < kOrder_customer_);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateDeltaKV(index, k, v_balance,v_data,lsn);
  }
  ...
  off_t offset_v17;
  off_t offset_v18;
  BPlusCustomerTree::Record records[kOrder_customer_];
  // BPlusCustomerTree::Record Per_page_logs[kOrder_customer_/2];
...
//hkc-modify-10
struct BPlusCustomerTree::Plog : BPlusCustomerTree::Node
{
  Plog() = default;
  ~Plog() = default;
  ...
  /*
  char *Value_ALL(int index) const
  {
    //assert(count > 0);
    char* all = (char*)malloc(sizeof(char)*kMaxValueSize_customer_col_all);
    int off =0;
  ...
  */
  ...
  //hkc-plog-7-1
  void UpdateKVDelta(int index, const char *k, const char *v_balance, const char *v_data,off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateDeltaKV(k, v_balance, v_data, lsn);
  }
  ...
  void InsertDeltaKVAtIndex(int index, const char *k, const char *v_balance, const char *v_data,off_t lsn)
  {
    ...
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateKVDelta(index, k,v_balance,v_data,lsn);
  }
  ...
  // BPlusCustomerTree::Record records[kOrder_customer_/2];
  BPlusCustomerTree::Record records[kOrder_customer_Plog];
};

class BPlusCustomerTree::BlockCache
{
  struct Node;
...
public:
  BlockCache(bool enable_backthd)
  {
    ...
    // pre_allocate
    // 2. init free list
    char *init_ptr = (char *)malloc(kMaxFreePages_customer_ * kLeafPageSize); // i.e., for each page
    memset(init_ptr, 0, kMaxFreePages_customer_ * kLeafPageSize);

    char *flag_addr = init_ptr;
    for (int i = 0; i < kMaxFreePages_customer_; i++)
    {
...
void RowToColumn_Cache(char* initial_page, char* converted_page){
  ...
  row_leaf->offset_lsn = (4+12)*1024;
  row_leaf->offset_v1  = (4+12+4)*1024;
  row_leaf->offset_v2  = (4+12+4+4)*1024;
  row_leaf->offset_v3  = (4+12+4+4+4)*1024;
  row_leaf->offset_v4  = (4+12+4+4+4+4)*1024;
  row_leaf->offset_v5  = (4+12+4+4+4+4+8)*1024;
  row_leaf->offset_v6  = (4+12+4+4+4+4+8+8)*1024;
  row_leaf->offset_v7  = (4+12+4+4+4+4+8+8+8)*1024;
  row_leaf->offset_v8  = (4+12+4+4+4+4+8+8+8+4)*1024;
  row_leaf->offset_v9  = (4+12+4+4+4+4+8+8+8+4+4)*1024;
  row_leaf->offset_v10 = (4+12+4+4+4+4+8+8+8+4+4+4)*1024;
  row_leaf->offset_v11 = (4+12+4+4+4+4+8+8+8+4+4+4+8)*1024;
  row_leaf->offset_v12 = (4+12+4+4+4+4+8+8+8+4+4+4+8+4)*1024;
  row_leaf->offset_v13 = (4+12+4+4+4+4+8+8+8+4+4+4+8+4+4)*1024;
  row_leaf->offset_v14 = (4+12+4+4+4+4+8+8+8+4+4+4+8+4+4+4)*1024;
  row_leaf->offset_v15 = (4+12+4+4+4+4+8+8+8+4+4+4+8+4+4+4+4)*1024;
  row_leaf->offset_v16 = (4+12+4+4+4+4+8+8+8+4+4+4+8+4+4+4+4+4)*1024;
  row_leaf->offset_v17 = (4+12+4+4+4+4+8+8+8+4+4+4+8+4+4+4+4+4+4)*1024;
  row_leaf->offset_v18 = (4+12+4+4+4+4+8+8+8+4+4+4+8+4+4+4+4+4+4+4)*1024;
  ...
  memcpy(&buf[fence],&(row_leaf->offset_v6), sizeof(row_leaf->offset_v6));
  fence+=sizeof(row_leaf->offset_v6);
  memcpy(&buf[fence],&(row_leaf->offset_v7), sizeof(row_leaf->offset_v7));
  fence+=sizeof(row_leaf->offset_v7);
  memcpy(&buf[fence],&(row_leaf->offset_v8), sizeof(row_leaf->offset_v8));
  fence+=sizeof(row_leaf->offset_v8);
  memcpy(&buf[fence],&(row_leaf->offset_v9), sizeof(row_leaf->offset_v9));
  fence+=sizeof(row_leaf->offset_v9);
  memcpy(&buf[fence],&(row_leaf->offset_v10), sizeof(row_leaf->offset_v10));
  fence+=sizeof(row_leaf->offset_v10);
  memcpy(&buf[fence],&(row_leaf->offset_v11), sizeof(row_leaf->offset_v11));
  fence+=sizeof(row_leaf->offset_v11);
  memcpy(&buf[fence],&(row_leaf->offset_v12), sizeof(row_leaf->offset_v12));
  fence+=sizeof(row_leaf->offset_v12);
  memcpy(&buf[fence],&(row_leaf->offset_v13), sizeof(row_leaf->offset_v13));
  fence+=sizeof(row_leaf->offset_v13);
  memcpy(&buf[fence],&(row_leaf->offset_v14), sizeof(row_leaf->offset_v14));
  fence+=sizeof(row_leaf->offset_v14);
  memcpy(&buf[fence],&(row_leaf->offset_v15), sizeof(row_leaf->offset_v15));
  fence+=sizeof(row_leaf->offset_v15);
  memcpy(&buf[fence],&(row_leaf->offset_v16), sizeof(row_leaf->offset_v16));
  fence+=sizeof(row_leaf->offset_v16);
  memcpy(&buf[fence],&(row_leaf->offset_v17), sizeof(row_leaf->offset_v17));
  fence+=sizeof(row_leaf->offset_v17);
  memcpy(&buf[fence],&(row_leaf->offset_v18), sizeof(row_leaf->offset_v18));
  fence+=sizeof(row_leaf->offset_v18);
  ...
  //convert row as column and fit in sub pages
    ...
    /*
      const int kMaxKeySize_customer_ = 16; 16K
      const int kMaxValueSize_customer_col1_ = 16; 16K
      const int kMaxValueSize_customer_col2_ = 24; 24K
      const int kMaxValueSize_customer_col3_ = 8; 8K
      const int kMaxValueSize_customer_col4_ = 50; 50K
    */
  ...
  off_t v6_of = row_leaf->offset_v6;
  off_t v7_of = row_leaf->offset_v7;
  off_t v8_of = row_leaf->offset_v8;
  off_t v9_of = row_leaf->offset_v9;
  off_t v10_of = row_leaf->offset_v10;
  off_t v11_of = row_leaf->offset_v11;
  off_t v12_of = row_leaf->offset_v12;
  off_t v13_of = row_leaf->offset_v13;
  off_t v14_of = row_leaf->offset_v14;
  off_t v15_of = row_leaf->offset_v15;
  off_t v16_of = row_leaf->offset_v16;
  off_t v17_of = row_leaf->offset_v17;
  off_t v18_of = row_leaf->offset_v18;



  for(int i=0; i<row_leaf->count;i++){
    memcpy(&buf[k_of],&(row_leaf->records[i].key),kMaxKeySize_customer_);
    k_of+=kMaxKeySize_customer_;
    memcpy(&buf[lsn_of],&(row_leaf->records[i].cur_lsn),sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&buf[v1_of],&(row_leaf->records[i].value_1),kMaxValueSize_customer_col1_);
    v1_of+=kMaxValueSize_customer_col1_;
    memcpy(&buf[v2_of],&(row_leaf->records[i].value_2),kMaxValueSize_customer_col2_);
    v2_of+=kMaxValueSize_customer_col2_;
    memcpy(&buf[v3_of],&(row_leaf->records[i].value_3),kMaxValueSize_customer_col3_);
    v3_of+=kMaxValueSize_customer_col3_;
    memcpy(&buf[v4_of],&(row_leaf->records[i].value_4),kMaxValueSize_customer_col4_);
    v4_of+=kMaxValueSize_customer_col4_;
    memcpy(&buf[v5_of],&(row_leaf->records[i].value_5),kMaxValueSize_customer_col5_);
    v5_of+=kMaxValueSize_customer_col5_;
    memcpy(&buf[v6_of],&(row_leaf->records[i].value_6),kMaxValueSize_customer_col6_);
    v6_of+=kMaxValueSize_customer_col6_;
    memcpy(&buf[v7_of],&(row_leaf->records[i].value_7),kMaxValueSize_customer_col7_);
    v7_of+=kMaxValueSize_customer_col7_;
    memcpy(&buf[v8_of],&(row_leaf->records[i].value_8),kMaxValueSize_customer_col8_);
    v8_of+=kMaxValueSize_customer_col8_;
    memcpy(&buf[v9_of],&(row_leaf->records[i].value_9),kMaxValueSize_customer_col9_);
    v9_of+=kMaxValueSize_customer_col9_;
    memcpy(&buf[v10_of],&(row_leaf->records[i].value_10),kMaxValueSize_customer_col10_);
    v10_of+=kMaxValueSize_customer_col10_;
    memcpy(&buf[v11_of],&(row_leaf->records[i].value_11),kMaxValueSize_customer_col11_);
    v11_of+=kMaxValueSize_customer_col11_;
    memcpy(&buf[v12_of],&(row_leaf->records[i].value_12),kMaxValueSize_customer_col12_);
    v12_of+=kMaxValueSize_customer_col12_;
    memcpy(&buf[v13_of],&(row_leaf->records[i].value_13),kMaxValueSize_customer_col13_);
    v13_of+=kMaxValueSize_customer_col3_;
    memcpy(&buf[v14_of],&(row_leaf->records[i].value_14),kMaxValueSize_customer_col14_);
    v14_of+=kMaxValueSize_customer_col14_;
    memcpy(&buf[v15_of],&(row_leaf->records[i].value_15),kMaxValueSize_customer_col15_);
    v15_of+=kMaxValueSize_customer_col15_;
    memcpy(&buf[v16_of],&(row_leaf->records[i].value_16),kMaxValueSize_customer_col16_);
    v16_of+=kMaxValueSize_customer_col16_;
    memcpy(&buf[v17_of],&(row_leaf->records[i].value_17),kMaxValueSize_customer_col17_);
    v17_of+=kMaxValueSize_customer_col17_;
    memcpy(&buf[v18_of],&(row_leaf->records[i].value_18),kMaxValueSize_customer_col18_);
    v18_of+=kMaxValueSize_customer_col18_;
  }
...
void ColumnToRow_Cache(char* column_page, char* row_page){
  ...
  fence+=sizeof(row_leaf->offset_v5);
  memcpy((char*)&row_leaf->offset_v6,&buf[fence],sizeof(row_leaf->offset_v6));
  fence+=sizeof(row_leaf->offset_v6);

  memcpy((char*)&row_leaf->offset_v7,&buf[fence],sizeof(row_leaf->offset_v7));
  fence+=sizeof(row_leaf->offset_v7);
  memcpy((char*)&row_leaf->offset_v8,&buf[fence],sizeof(row_leaf->offset_v8));
  fence+=sizeof(row_leaf->offset_v8);
  memcpy((char*)&row_leaf->offset_v9,&buf[fence],sizeof(row_leaf->offset_v9));
  fence+=sizeof(row_leaf->offset_v9);
  memcpy((char*)&row_leaf->offset_v10,&buf[fence],sizeof(row_leaf->offset_v10));
  fence+=sizeof(row_leaf->offset_v10);
  memcpy((char*)&row_leaf->offset_v11,&buf[fence],sizeof(row_leaf->offset_v11));
  fence+=sizeof(row_leaf->offset_v11);
  memcpy((char*)&row_leaf->offset_v12,&buf[fence],sizeof(row_leaf->offset_v12));
  fence+=sizeof(row_leaf->offset_v12);
  memcpy((char*)&row_leaf->offset_v13,&buf[fence],sizeof(row_leaf->offset_v13));
  fence+=sizeof(row_leaf->offset_v13);
  memcpy((char*)&row_leaf->offset_v14,&buf[fence],sizeof(row_leaf->offset_v14));
  fence+=sizeof(row_leaf->offset_v14);
  memcpy((char*)&row_leaf->offset_v15,&buf[fence],sizeof(row_leaf->offset_v15));
  fence+=sizeof(row_leaf->offset_v15);
  memcpy((char*)&row_leaf->offset_v16,&buf[fence],sizeof(row_leaf->offset_v16));
  fence+=sizeof(row_leaf->offset_v16);
  memcpy((char*)&row_leaf->offset_v17,&buf[fence],sizeof(row_leaf->offset_v17));
  fence+=sizeof(row_leaf->offset_v17);
  memcpy((char*)&row_leaf->offset_v18,&buf[fence],sizeof(row_leaf->offset_v18));
  fence+=sizeof(row_leaf->offset_v18);
  ...
  off_t v18_of = row_leaf->offset_v18;
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&(row_leaf->records[i].key),&buf[k_of],kMaxKeySize_customer_);
    k_of+=kMaxKeySize_customer_;
    memcpy(&(row_leaf->records[i].cur_lsn),&buf[lsn_of],sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&(row_leaf->records[i].value_1),&buf[v1_of],kMaxValueSize_customer_col1_);
    v1_of+=kMaxValueSize_customer_col1_;
    memcpy(&(row_leaf->records[i].value_2),&buf[v2_of],kMaxValueSize_customer_col2_);
    v2_of+=kMaxValueSize_customer_col2_;
    memcpy(&(row_leaf->records[i].value_3),&buf[v3_of],kMaxValueSize_customer_col3_);
    v3_of+=kMaxValueSize_customer_col3_;
    memcpy(&(row_leaf->records[i].value_4),&buf[v4_of],kMaxValueSize_customer_col4_);
    v4_of+=kMaxValueSize_customer_col4_;
    memcpy(&(row_leaf->records[i].value_5),&buf[v5_of],kMaxValueSize_customer_col5_);
    v5_of+=kMaxValueSize_customer_col5_;
    memcpy(&(row_leaf->records[i].value_6),&buf[v6_of],kMaxValueSize_customer_col6_);
    v6_of+=kMaxValueSize_customer_col6_;
    memcpy(&(row_leaf->records[i].value_7),&buf[v7_of],kMaxValueSize_customer_col7_);
    v7_of+=kMaxValueSize_customer_col7_;
    memcpy(&(row_leaf->records[i].value_8),&buf[v8_of],kMaxValueSize_customer_col8_);
    v8_of+=kMaxValueSize_customer_col8_;
    memcpy(&(row_leaf->records[i].value_9),&buf[v9_of],kMaxValueSize_customer_col9_);
    v9_of+=kMaxValueSize_customer_col9_;
    memcpy(&(row_leaf->records[i].value_10),&buf[v10_of],kMaxValueSize_customer_col10_);
    v10_of+=kMaxValueSize_customer_col10_;

    memcpy(&(row_leaf->records[i].value_11),&buf[v11_of],kMaxValueSize_customer_col11_);
    v11_of+=kMaxValueSize_customer_col11_;
    memcpy(&(row_leaf->records[i].value_12),&buf[v12_of],kMaxValueSize_customer_col12_);
    v12_of+=kMaxValueSize_customer_col12_;
    memcpy(&(row_leaf->records[i].value_13),&buf[v13_of],kMaxValueSize_customer_col13_);
    v13_of+=kMaxValueSize_customer_col13_;
    memcpy(&(row_leaf->records[i].value_14),&buf[v14_of],kMaxValueSize_customer_col14_);
    v14_of+=kMaxValueSize_customer_col14_;
    memcpy(&(row_leaf->records[i].value_15),&buf[v15_of],kMaxValueSize_customer_col15_);
    v15_of+=kMaxValueSize_customer_col15_;
    memcpy(&(row_leaf->records[i].value_16),&buf[v16_of],kMaxValueSize_customer_col16_);
    v16_of+=kMaxValueSize_customer_col16_;
    memcpy(&(row_leaf->records[i].value_17),&buf[v17_of],kMaxValueSize_customer_col17_);
    v17_of+=kMaxValueSize_customer_col17_;
    memcpy(&(row_leaf->records[i].value_18),&buf[v18_of],kMaxValueSize_customer_col18_);
    v18_of+=kMaxValueSize_customer_col18_;
  ...
  void Put(T *block, int is_plog)
  {
    ...
    if (async_mode_customer_ == 0)
    {
      if (size_lru > kMaxPages_customer_item * 0.9)
      {
        KickLRUbyUring();
      }
      if (mtr_cache->plog_flag != 1 && mtr_cache->FLU_disabled == 0 && size_flu > kMaxPages_customer_item * 0.5)
      {
        KickLFUbyUring();
      }
    }
  ...
  void SetMtr(BPlusCustomerTree::Monitor *mtr_1){
    mtr_cache = mtr_1;
  }
...
private:
  ...
  void KickLRUbyUring()
  {
    ...
    for (i = 0; i < Queue_depth; i++)
    {
      ...
      ptr = ptr->prev_lru;
      DeletePersistedNode(tmp);
      if (mtr_cache->plog_flag!=1 && mtr_cache->InFLUOrNot.count(tmp->offset))
      {
        DeletePersistedFLUNode(tmp);
        mtr_cache->InFLUOrNot.erase(tmp->offset);
      }
      if(offset2node_leaf.find(tmp->offset)== offset2node_leaf.end()){
        //what happened?
        LeafNode *leaf = reinterpret_cast<LeafNode *>(tmp->block);
        int x=1;
      }else{
...
class BPlusCustomerTree::InternalCache
{
  struct Node;
...
public:
  ...
  void SetMtr(BPlusCustomerTree::Monitor *mtr_2){
    mtr_internal = mtr_2;
  }
...
//hkc-ap-10*
void BPlusCustomerTree::ScanAP_impl(off_t &left, off_t &right, std::string &value){
...
//hkc-ap-10
void BPlusCustomerTree::APWorker(){
  sleep(20);
  while(true){
...
BPlusCustomerTree::BPlusCustomerTree(const char *path, const char *log_path, const char *chpt_path, const int bg_id_, const bool enable_backthd, const char *shadow_plog_path)
...
//hkc-ap-12
  ...
  if (fd_ == -1)
    //int x=1;
    printf("open customer db failed.\n");
  if (log_fd_ == -1)
    printf("open customer log failed.\n");
  if (chpt_fd_ == -1)
    printf("open customer chpt failed.\n");
  ...
  // meta_ = Map<Meta>(kMetaOffset_customer_);
  meta_ = new Meta();
  ...
  {
    // Initialize B+tree;
    constexpr off_t of_root = kMetaOffset_customer_ + kUnitNodeSize;
    meta_->block = of_root;
  ...
  //hkc-ap-14
  if(scan_ap){
    std::thread thd_ap = std::thread(&BPlusCustomerTree::APWorker,this);
    thd_ap.detach();
  }
}

BPlusCustomerTree::~BPlusCustomerTree()
{
  UnMap(meta_, 0, 0);
  delete block_cache_;
  close(fd_);
...
//hkc-ap-15
int BPlusCustomerTree::PrepareReadSetAP(off_t left, off_t right, void *scan_ptr){
...
void BPlusCustomerTree::SetPlog(){
  mtr_->plog_flag =1;
  mtr_->FLU_disabled =1;
  mtr_->warmup_flag =1;
  return;
...
void BPlusCustomerTree::GetStatic(int &hits_bp, int &hits_glog, int &hits_plog, int &disk_read, int &io_consume, int &error_cnt, int &dirtys, int &cleans){
...
off_t BPlusCustomerTree::GetLSN(){
  return mtr_->global_lsn;
}
void BPlusCustomerTree::IncreLSN(){
  mtr_->global_lsn++;
  return;
}

void BPlusCustomerTree::ResetStatic(){
    mtr_->hits_bp = 0; 
    mtr_->hits_glog = 0; 
    mtr_->hits_plog =0;
    mtr_->disk_read=0;
...
void BPlusCustomerTree::SetBaseline(){
  mtr_->FLU_disabled = 0;
  mtr_->plog_flag =-1;
  mtr_->warmup_flag =0;
}

void BPlusCustomerTree::SetIOStaic(){
    mtr_->plog_insert=0;
    mtr_->leaf_insert=0;
    mtr_->leaf_split=0;
...
off_t BPlusCustomerTree::GetChpt(){
  return mtr_->normal_chpt_now;
}
void BPlusCustomerTree::SetLoadFlag(int flag){
  mtr_->load_flag = flag;
}
void BPlusCustomerTree::DisableFLUOrNot(int flag){
  mtr_->FLU_disabled = flag;
}
void BPlusCustomerTree::LoadIOStatic(int &plog_insert,int &leaf_insert,int &leaf_split,int &phy_plog_write,int &phy_plog_read,int &phy_page_read,int &phy_page_write,int &logi_page_read,int &logi_page_write){
...
int BPlusCustomerTree::ApplyPendPages(std::map<off_t, std::pair<char*,char*>>* source, std::map<off_t, std::pair<char*,char*>>* target, std::map<off_t,std::pair<int,int>> *EmptyPages){
...
void BPlusCustomerTree::BackGround_GLog_Func()
{
  int total_KV=0;
  int final_KV=0;
  int total_Plog =0;
      ...
      if(now_lsn > cur_lsn){
        ...
        bool overflow =0;
          ...
          if(value[0]=='U'){
            ...
            std::string value_balance = value.substr(2,14);
            std::string value_data = value.substr(16,100);
            if(InsertDeltaIntoPlog(plog,key.data(),value_balance.data(),value_data.data(),cur_lsn)>GetMaxKeys(0)){
              overflow = true;
              mtr_->plog_insert += 1;
            }else{
              mtr_->plog_insert += 1;
            ...
            std::string value2 = value.substr(16,2);
            std::string value3 = value.substr(18,16);
            std::string value4 = value.substr(34,20);
            std::string value5 = value.substr(54,20);
            std::string value6 = value.substr(74,20);
            std::string value7 = value.substr(94,2);
            std::string value8 = value.substr(96,9);
            std::string value9 = value.substr(105,16);
            std::string value10 = value.substr(121,20);
            std::string value11 = value.substr(141,2);
            std::string value12 = value.substr(143,16);
            std::string value13 = value.substr(159,6);
            std::string value14 = value.substr(165,14);
            std::string value15 = value.substr(179,14);
            std::string value16 = value.substr(193,16);
            std::string value17 = value.substr(209,16);
            std::string value18 = value.substr(225,100);
              
            if (InsertKVIntoPlog(plog, key.data(), 
                  value1.data(),value2.data(),value3.data(),value4.data(),value5.data(),value6.data(),value7.data(),value8.data(),value9.data(),
                  value10.data(),value11.data(),value12.data(),value13.data(),value14.data(),value15.data(),value16.data(),value17.data(),value18.data(), 
                  cur_lsn) > GetMaxKeys(0)){
                mtr_->plog_insert += 1;
                overflow == true;
            }else{
                mtr_->plog_insert += 1;
          ...
          if(overflow == true){
            ...
            for (int i = 0; i < kOrder_customer_Plog; i++){
              if(plog->records[i].partial == true){
                //only update columns
                if(InsertDeltaKVIntoLeafNode( leaf_node,
                                          plog->records[i].key, 
                                          plog->records[i].value_14, 
                                          plog->records[i].value_18,
                ...
                if(InsertKVIntoLeafNode( leaf_node,
                                          ...
                                          plog->records[i].value_6,
                                          plog->records[i].value_7, 
                                          plog->records[i].value_8,
                                          plog->records[i].value_9,
                                          plog->records[i].value_10,
                                          plog->records[i].value_11, 
                                          plog->records[i].value_12, 
                                          plog->records[i].value_13, 
                                          plog->records[i].value_14,
                                          plog->records[i].value_15,
                                          plog->records[i].value_16,
                                          plog->records[i].value_17, 
                                          plog->records[i].value_18, 
...
//hkc-modify-21
void BPlusCustomerTree::RowToColumn(char* initial_page, char* converted_page){
...
//hkc-modify-22
void BPlusCustomerTree::ColumnToRow(char* column_page, char* row_page){
  memset(row_page,0,kLeafPageSize);
  LeafNode* row_leaf = reinterpret_cast<LeafNode*>(row_page);
  ...
  //memcpy(&buf[fence],&(row_leaf->plog_offset), sizeof(row_leaf->plog_offset));
  //row_leaf->plog_offset = reinterpret_cast<off_t>(&buf[fence]);
  memcpy((char*)&row_leaf->sha_plog_offset,&buf[fence],sizeof(row_leaf->sha_plog_offset));//hkc-ap-18
  fence+=sizeof(row_leaf->sha_plog_offset);
...
//hkc-gb-04
off_t BPlusCustomerTree::GetOnlyLeafOffset(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
  return of_leaf;
}
//hkc-gb-05
char* BPlusCustomerTree::GetLeafCopy(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
...
//hkc-gb-06
void BPlusCustomerTree::UpdateLeafCopy(const std::string &key, const std::string &value, off_t lsn, char* leaf_copy){
  ...
  std::string value17 = value.substr(209,16);
  std::string value18 = value.substr(225,100);
  if (InsertKVIntoLeafNode(leaf, key.data(),  value1.data(),value2.data(),value3.data(),value4.data(),value5.data(),value6.data(),
                          value7.data(),value8.data(),value9.data(),value10.data(),value11.data(),value12.data(),value13.data(),
                          value14.data(),value15.data(),value16.data(),value17.data(),value18.data(),  lsn) <= GetMaxKeys(1)){
...
//hkc-gb-07
void BPlusCustomerTree::GetRowRecord(const std::string &key, std::string &value, char* leaf_copy) {
  LeafNode* leaf  = reinterpret_cast<LeafNode*>(&leaf_copy[0]);
  int index = GetIndexFromLeafNode(leaf, key.data());
  char* ptr = leaf->ValueALL(index);
  std::string tmp_value(ptr,ptr+kMaxValueSize_customer_col_all);
  value = tmp_value;
  free(ptr);
  return ;
...
//hkc-gb-08
int into_time_cus=0;
void BPlusCustomerTree::TriggerFlush(const off_t* offsets, const int size, char* evict_buf){
  int batch_size = 200;
  into_time_cus++;
  struct io_uring_sqe *sqe_flush;
  struct io_uring_cqe *cqe_flush;
  struct iovec iov_flush[batch_size];
  ...
  for(int i=0;i<size;i++){
    ...
    iov_flush[i].iov_base = tmp_cache[i];
    iov_flush[i].iov_len = kLeafPageSize;
    io_uring_prep_writev(sqe_flush, mtr_->global_fd, &iov_flush[i], 1, offsets[i]);
  }
...
void BPlusCustomerTree::Put(const std::string &key, const std::string &value, off_t lsn)
{
  ...
  if(mtr_->plog_flag == 1){ 
    ...
    if(index_seg->count(key)==0){
      index_seg->emplace(key,lsn);
    }else{
      (*index_seg)[key]=lsn;
    }
    ...
    //  (*index_seg)[key]=lsn; //index_seg->emplace(key,lsn);
    //}else{
    //  (*index_seg)[key]=lsn;
    //}
  ...
  //hkc-modify-16
          ...
          std::string value17 = value.substr(209,16);
          std::string value18 = value.substr(225,100);
  if (InsertKVIntoLeafNode(leaf_node, key.data(),  value1.data(),value2.data(),value3.data(),value4.data(),value5.data(),value6.data(),value7.data(),value8.data(),value9.data(),
                value10.data(),value11.data(),value12.data(),value13.data(),value14.data(),value15.data(),value16.data(),value17.data(),value18.data(),  lsn) <=
      GetMaxKeys(1))
  {
    // 2.If records of leaf node less than or equals kOrder_customer_ - 1 then finish.
    // UnMap<LeafNode>(leaf_node);
    if (mtr_->FLU_disabled == 0 && mtr_->InFLUOrNot.count(leaf_node->offset) ==0)
    {
  ...
  // until count <= kOrder_customer_ - 1.
  size_t count;
  do
  {
...
bool BPlusCustomerTree::Get(const std::string &key, std::string &value) const
{
...
bool BPlusCustomerTree::Scan(const std::string &start, const std::string &end, std::string &value) const
{
  if(start>end){
    int x=1;
  }
...
//hkc-ap-19
bool BPlusCustomerTree::ScanAP(const std::string &start, const std::string &end, std::string &value) const
{
...
int BPlusCustomerTree::Check(off_t offset)
{
  return block_cache_->Check_Leaf(offset);
}

char *BPlusCustomerTree::GetMemCopy(off_t offset)
{
  return block_cache_->GetMemCopy_Leaf(offset);
}

template <typename T>
T *BPlusCustomerTree::Map(off_t offset, int isleaf, int is_plog, char *&plog_copy) const
{
  if (isleaf == 1)
  {
...
template <typename T>
void BPlusCustomerTree::InsertDirty(off_t offset) const
{
...
template <typename T>
void BPlusCustomerTree::RelocateDirty(off_t offset, off_t ori_oldset_lsn) const
{
...
template <typename T>
void BPlusCustomerTree::DirectInsertDirty(off_t offset_leaf, off_t offset_split) const
{
...
template <typename T>
void BPlusCustomerTree::UnMap(T *map_obj, int isleaf, int is_plog) const
{
  if (isleaf == 1)
  {
...
size_t BPlusCustomerTree::GetMinKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return (kOrder_customer_Plog + 1) / 2 - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return (kOrder_customer_ + 1) / 2 - 1;
  }
}
// 1: leaf node ; 0: plog
size_t BPlusCustomerTree::GetMaxKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return kOrder_customer_Plog - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return kOrder_customer_ - 1;
  }
}
size_t BPlusCustomerTree::GetMinIndexKeys() { return (kOrder_customer_Index + 1) / 2 - 1; }
size_t BPlusCustomerTree::GetMaxIndexKeys() { return kOrder_customer_Index - 1; }

BPlusCustomerTree::IndexNode *BPlusCustomerTree::GetOrCreateParent(Node *node)
{
  if (node->parent == 0)
  {
    // Split root node.
...
template <typename T>
int BPlusCustomerTree::UpperBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
  ...
  {
    int mid = (l + r) >> 1;
    if (std::strncmp(arr[mid].key, key, kMaxKeySize_customer_) <= 0)
    {
      l = mid + 1;
    }
    else
...
template <typename T>
int BPlusCustomerTree::UpperBoundIndex(T arr[], int n, const char *key) const
{
...
template <typename T>
int BPlusCustomerTree::LowerBoundIndex(T arr[], int n, const char *key) const
{
  ////assert(n <= GetMaxIndexKeys());
  int l = 0, r = n - 1;
  while (l <= r)
...
// flag: 1: data page; 0: plog
template <typename T>
int BPlusCustomerTree::LowerBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
    //assert(n <= GetMaxKeys(0));
...
template <typename T>
T *BPlusCustomerTree::AllocIndex()
{ // 1: leaf node, 0: internal node
...
template <typename T>
void BPlusCustomerTree::Dealloc(T *node)
{
  UnMap<T>(node);
}

void BPlusCustomerTree::WarmUpVirtualPageCache(int x) const
{
  return block_cache_->WarmUpVPCache(x);
}
off_t BPlusCustomerTree::GetLeafOffset(const char *key) const
{
...
inline size_t BPlusCustomerTree::InsertKeyIntoIndexNode(IndexNode *index_node,
                                                const char *key,
                                                Node *left_node,
...
//hkc-modify-13
size_t BPlusCustomerTree::InsertKVIntoLeafNode(LeafNode *leaf_node, const char *key,
                              ...
                              const char* value13, const char* value14,const char* value15,const char* value16,const char* value17,const char* value18, off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(leaf_node->Key(index - 1), key, kMaxKeySize_customer_) == 0)
  {
    leaf_node->UpdateValue(index - 1, value1,value2,value3,value4,value5, value6,value7,value8,value9,value10,value11, value12,value13,value14,value15, value16,value17,value18, cur_lsn);
    return leaf_node->count;
  }
  leaf_node->InsertKVAtIndex(index, key,  value1,value2,value3,value4,value5, value6,value7,value8,value9,value10,value11, value12,value13,value14,value15, value16,value17,value18, cur_lsn);
  ++meta_->size;
  return leaf_node->count;
}
//hkc-plog-9
size_t BPlusCustomerTree::InsertDeltaKVIntoLeafNode(LeafNode *leaf_node, const char *key,const char* value_balance, const char* value_data,off_t cur_lsn)
{
  //assert(leaf_node->count <= GetMaxKeys(1));
  ...
  if (index > 0 &&
      std::strncmp(leaf_node->Key(index - 1), key, kMaxKeySize_customer_) == 0)
  {
    leaf_node->UpdateValueDelta(index - 1, value_balance,value_data,cur_lsn);
    return leaf_node->count;
  }
  leaf_node->InsertDeltaKVAtIndex(index, key,  value_balance,value_data,cur_lsn);
  ++meta_->size;
  return leaf_node->count;
}

//hkc-modify-14
size_t BPlusCustomerTree::InsertKVIntoPlog(Plog *plog, const char *key,
                                   const char* value1, const char* value2,const char* value3,const char* value4,const char* value5,const char* value6,
                              const char* value7, const char* value8,const char* value9,const char* value10,const char* value11,const char* value12,
                              const char* value13, const char* value14,const char* value15,const char* value16,const char* value17,const char* value18, off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(plog->Key(index - 1), key, kMaxKeySize_customer_) == 0)
  {
    plog->UpdateValue(index - 1, value1,value2,value3,value4,value5, value6,value7,value8,value9,value10,value11, value12,value13,value14,value15, value16,value17,value18, cur_lsn);
    return plog->count;
  }
  plog->InsertKVAtIndex(index, key, value1,value2,value3,value4,value5, value6,value7,value8,value9,value10,value11, value12,value13,value14,value15, value16,value17,value18,cur_lsn);
  ++meta_->size;
  return plog->count;
}
//hkc-plog-10
//InsertDeltaIntoPlog(Plog* plog, const char* key,const char* value_balance, const char* value_data,off_t cur_lsn)
size_t BPlusCustomerTree::InsertDeltaIntoPlog(Plog *plog, const char *key, const char* value_balance, const char* value_data,off_t cur_lsn)
{
  //assert(plog->count <= GetMaxKeys(0));
  ...
  if (index > 0 &&
      std::strncmp(plog->Key(index - 1), key, kMaxKeySize_customer_) == 0)
  {
    plog->UpdateValueDelta(index - 1, value_balance,value_data, cur_lsn);
    return plog->count;
  }
  //KV NOT EXIST IN THE PLOG, SO WE PRETEND TO HAVE ALL KVS
  plog->InsertDeltaKVAtIndex(index, key, value_balance,value_data,cur_lsn);
  ++meta_->size;
  return plog->count;
}

BPlusCustomerTree::LeafNode *BPlusCustomerTree::SplitLeafNode(LeafNode *leaf_node)
{
  //assert(leaf_node->count == kOrder_customer_);
  off_t origian_lsn = leaf_node->GetSmallestLSN();
  constexpr int mid = (kOrder_customer_ - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_customer_ - mid;
...
BPlusCustomerTree::IndexNode *BPlusCustomerTree::SplitIndexNode(IndexNode *index_node)
{
  //assert(index_node->count == kOrder_customer_Index);
  constexpr int mid = (kOrder_customer_Index - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_customer_Index - mid - 1;
  ...
  for (int i = mid + 1; i <= kOrder_customer_Index; ++i)
  {
...
inline int BPlusCustomerTree::GetIndexFromLeafNode(LeafNode *leaf_node,
                                           const char *key) const
{
  int index = LowerBound(leaf_node->records, leaf_node->count, key, 1);
  return index < static_cast<int>(leaf_node->count) &&
                 std::strncmp(leaf_node->Key(index), key, kMaxKeySize_customer_) == 0
             ? index
             : -1;
}

inline int BPlusCustomerTree::GetIndexFromPlog(Plog *plog,
                                       const char *key) const
{
  int index = LowerBound(plog->records, plog->count, key, 0);
  return index < static_cast<int>(plog->count) &&
                 std::strncmp(plog->Key(index), key, kMaxKeySize_customer_) == 0
             ? index
             : -1;
}
/*
std::vector<std::pair<std::string, std::string>> BPlusCustomerTree::GetRange(
    const std::string &left_key, const std::string &right_key) const
{
  ...
  while (of_leaf != 0 && !finish)
  {
    ...
    {
      if (strncmp(right_leaf_node->Key(i), right_key.data(), kMaxKeySize_customer_) <=
          0)
      {
...
*/
bool BPlusCustomerTree::Empty() const { return meta_->size == 0; }

size_t BPlusCustomerTree::Size() const { return meta_->size; }