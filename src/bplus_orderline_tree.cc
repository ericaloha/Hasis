...
// switch
const int async_mode_orderline_ = 0; // 0 for sync IO; 1 for async IO
const int kAIOBatchSize_item = 20;
const int kOrder_orderline_ = 600; //how many record a page can contain
const int kOrder_orderline_Plog = kOrder_orderline_ / 10;
// const int kOrder_orderline_ = 50; //for key
const int kOrder_orderline_Index = 1000; // for index

static_assert(kOrder_orderline_ >= 3,
              "The order of B+Tree should be greater than or equal to 3.");


const int kMaxKeySize_orderline_ = 64;
const int kMaxLSNSize_orderline_ = 8;
// const int kMaxValueSize_orderline_ = 256;
const int kMaxValueSize_orderline_col1_ = 16;
const int kMaxValueSize_orderline_col2_ = 16;
const int kMaxValueSize_orderline_col3_ = 20;
const int kMaxValueSize_orderline_col4_ = 16;
const int kMaxValueSize_orderline_col5_ = 8; 
const int kMaxValueSize_orderline_col6_ = 24; 
const int kMaxValueSize_orderline_col_all = 16+16+20+16+8+24;
const off_t kMetaOffset_orderline_ = 0;

//hkc-plog-1
const int KPosBalance = 16+16;

int kMaxPages_orderline_item = 500;     // 100M
const int kMaxFreePages_orderline_ = 600; 
// const int kMaxPages_orderline_item = 80000; //1G

typedef char Key[kMaxKeySize_orderline_];
typedef char Value1[kMaxValueSize_orderline_col1_];
typedef char Value2[kMaxValueSize_orderline_col2_];
typedef char Value3[kMaxValueSize_orderline_col3_];
typedef char Value4[kMaxValueSize_orderline_col4_];
typedef char Value5[kMaxValueSize_orderline_col5_];
typedef char Value6[kMaxValueSize_orderline_col6_];
typedef char Value_all[kMaxValueSize_orderline_col_all]; //for log record
...
const int innerKeyoffset_orderline_ = 0;
const int innercoloffset_orderline_ = 0;

const int kMaxUniqueKInGLogSeg=3000;
  


struct BPlusOrderlineTree::Monitor{
  Monitor(){
    bg_id =0;
    plog_insert = 0;
    leaf_insert = 0;
...
struct BPlusOrderlineTree::Meta
{
...
struct BPlusOrderlineTree::Index
{
  ...
  void UpdateIndex(off_t of, off_t lsn, const char *k)
  {
    offset = of;
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_orderline_);
  }
...
//hkc-modify-2
struct BPlusOrderlineTree::Record
{
  Key key;
  Value1 value_1;
  Value2 value_2;
  ...
  void UpdateKV(const char *k, const char *v1, const char *v2, const char *v3, const char *v4,const char *v5,const char *v6, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_orderline_);
    strncpy(value_1, v1, kMaxValueSize_orderline_col1_);
    strncpy(value_2, v2, kMaxValueSize_orderline_col2_);
    strncpy(value_3, v3, kMaxValueSize_orderline_col3_);
    strncpy(value_4, v4, kMaxValueSize_orderline_col4_);
    strncpy(value_5, v5, kMaxValueSize_orderline_col5_);
    strncpy(value_6, v6, kMaxValueSize_orderline_col6_);
  }

  //hkc-plog-2
  void UpdateDeltaKV(const char *k,const char *v_ts, off_t lsn)
  {
    cur_lsn = lsn;
    partial = true;
    strncpy(key, k, kMaxKeySize_orderline_);
    strncpy(value_3, v_ts, kMaxValueSize_orderline_col3_);
  }
  ...
  //hkc-plog-3
  void UpdateValueDelta(const char *v_ts, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(value_3, v_ts, kMaxValueSize_orderline_col3_);
  }
...
struct BPlusOrderlineTree::PAXKeyRecord
{
  Key key;
  off_t cur_lsn;
...
struct BPlusOrderlineTree::PAXColumn1Record
{
  Value1 value_1;
...
struct BPlusOrderlineTree::PAXColumn2Record
{
  Value2 value_2;
...
struct BPlusOrderlineTree::PAXColumn3Record
{
  Value3 value_3;
...
struct BPlusOrderlineTree::PAXColumn4Record
{
  Value4 value_4;
...
//hkc-modify-8
struct BPlusOrderlineTree::PAXColumn5Record
{
  Value5 value_5;
...
//hkc-modify-9
struct BPlusOrderlineTree::PAXColumn6Record
{
  Value6 value_6;
...
struct BPlusOrderlineTree::Per_page_Log
{
  Key key;
  Value_all value_all;
  off_t cur_lsn;
  ...
  void UpdateKV(const char *k, const char *v, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_orderline_);
    strncpy(value_all, v, kMaxValueSize_orderline_col_all);
  }
  void UpdateKey(const char *k, off_t lsn)
  {
    cur_lsn = lsn;
...
struct BPlusOrderlineTree::LogRecord
{
  LogRecord() : page_offset(0), log_lsn(0), log_offset(0), next_log_offset(0), page_index(0)
  {
    memset(key, 0, kMaxKeySize_orderline_);
    memset(value, 0, kMaxValueSize_orderline_col_all);
  }
  ...
  char key[kMaxKeySize_orderline_];
  char value[kMaxValueSize_orderline_col_all];

  void UpdateKV(const char *k, const char *v)
  {
    strncpy(key, k, kMaxKeySize_orderline_);
    strncpy(value, v, kMaxValueSize_orderline_col_all);
  }
  void UpdateKey(const char *k) { strncpy(key, k, kMaxKeySize_orderline_); }
  void UpdateValue(const char *v) { strncpy(value, v, kMaxValueSize_orderline_col_all); }
};

struct BPlusOrderlineTree::Checkpoint
{
...
struct BPlusOrderlineTree::Node
{
  //hkc-ap-04
...
struct BPlusOrderlineTree::IndexNode : BPlusOrderlineTree::Node
{
  IndexNode() = default;
  ~IndexNode() = default;
  ...
  const char *Key(int index) const
  {
    //assert(count > 0);
    //assert(index >= 0);
    //assert(index <= kOrder_orderline_Index);
  ...
  Index indexes[kOrder_orderline_Index + 1];
};

//hkc-modify-9
struct BPlusOrderlineTree::LeafNode : BPlusOrderlineTree::Node
{
  LeafNode() = default;
  ~LeafNode() = default;
  ...
  {
    //assert(count > 0);
    char* all = (char*)malloc(sizeof(char)*kMaxValueSize_orderline_col_all);
    int off =0;
    memcpy(&all[off],records[index].value_1,kMaxValueSize_orderline_col1_);
    off+=kMaxValueSize_orderline_col1_;
    memcpy(&all[off],records[index].value_2,kMaxValueSize_orderline_col2_);
    off+=kMaxValueSize_orderline_col2_;
    memcpy(&all[off],records[index].value_3,kMaxValueSize_orderline_col3_);
    off+=kMaxValueSize_orderline_col3_;
    memcpy(&all[off],records[index].value_4,kMaxValueSize_orderline_col4_);
    off+=kMaxValueSize_orderline_col4_;
    memcpy(&all[off],records[index].value_5,kMaxValueSize_orderline_col5_);
    off+=kMaxValueSize_orderline_col5_;
    memcpy(&all[off],records[index].value_6,kMaxValueSize_orderline_col6_);
    off+=kMaxValueSize_orderline_col6_;
    return all;
  }
  ...
  //hkc-plog-4
  void UpdateValueDelta(int index, const char *v_ts, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateValueDelta(v_ts, lsn);
  }
  ...
  //hkc-plog-5
  void UpdateDeltaKV(int index, const char *k, const char *v_ts, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateDeltaKV(k, v_ts, lsn);
  }
  ...
  //hkc-plog-6
  void InsertDeltaKVAtIndex(int index, const char *k, const char *v_ts, off_t lsn)
  {
    //assert(index >= 0);
    //assert(index < kOrder_customer_);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateDeltaKV(index, k, v_ts, lsn);
  }
  ...
  {
    //assert(index >= 0);
    //assert(index < kOrder_orderline_);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
  ...
  off_t offset_v5;
  off_t offset_v6;
  BPlusOrderlineTree::Record records[kOrder_orderline_];
  // BPlusOrderlineTree::Record Per_page_logs[kOrder_orderline_/2];
...
//hkc-modify-10
struct BPlusOrderlineTree::Plog : BPlusOrderlineTree::Node
{
  Plog() = default;
  ~Plog() = default;
  ...
  //hkc-plog-7-1
  void UpdateKVDelta(int index, const char *k, const char *v_ts, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateDeltaKV(k, v_ts, lsn);
  }
  ...
  {
    //assert(index >= 0);
    //assert(index < kOrder_orderline_Plog);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateKV(index, k,v1,v2,v3,v4, v5, v6,lsn);
  }
  ...
  void InsertDeltaKVAtIndex(int index, const char *k, const char *v_ts, off_t lsn)
  {
    ...
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateKVDelta(index, k,v_ts,lsn);
  }
  ...
  // BPlusOrderlineTree::Record records[kOrder_orderline_/2];
  BPlusOrderlineTree::Record records[kOrder_orderline_Plog];
};

class BPlusOrderlineTree::BlockCache
{
  struct Node;
...
public:
  BlockCache(bool enable_backthd)
  {
    ...
    // pre_allocate
    // 2. init free list
    char *init_ptr = (char *)malloc(kMaxFreePages_orderline_ * kLeafPageSize); // i.e., for each page
    memset(init_ptr, 0, kMaxFreePages_orderline_ * kLeafPageSize);

    char *flag_addr = init_ptr;
    for (int i = 0; i < kMaxFreePages_orderline_; i++)
    {
...
void RowToColumn_Cache(char* initial_page, char* converted_page){
  ...
  row_leaf->offset_lsn =(4+40)*1024;
  row_leaf->offset_v1 = (4+40+8)*1024;
  row_leaf->offset_v2 = (4+40+8+12)*1024;
  row_leaf->offset_v3 = (4+40+8+12+12)*1024;
  row_leaf->offset_v4 = (4+40+8+12+12+16)*1024;
  row_leaf->offset_v5 = (4+40+8+12+12+16+12)*1024;
  row_leaf->offset_v6 = (4+40+8+12+12+16+12+8)*1024;
  ...
  //convert row as column and fit in sub pages
    ...
    /*
      const int kMaxKeySize_orderline_ = 16; 16K
      const int kMaxValueSize_orderline_col1_ = 16; 16K
      const int kMaxValueSize_orderline_col2_ = 24; 24K
      const int kMaxValueSize_orderline_col3_ = 8; 8K
      const int kMaxValueSize_orderline_col4_ = 50; 50K
    */
  ...
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&buf[k_of],&(row_leaf->records[i].key),kMaxKeySize_orderline_);
    k_of+=kMaxKeySize_orderline_;
    memcpy(&buf[lsn_of],&(row_leaf->records[i].cur_lsn),sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&buf[v1_of],&(row_leaf->records[i].value_1),kMaxValueSize_orderline_col1_);
    v1_of+=kMaxValueSize_orderline_col1_;
    memcpy(&buf[v2_of],&(row_leaf->records[i].value_2),kMaxValueSize_orderline_col2_);
    v2_of+=kMaxValueSize_orderline_col2_;
    memcpy(&buf[v3_of],&(row_leaf->records[i].value_3),kMaxValueSize_orderline_col3_);
    v3_of+=kMaxValueSize_orderline_col3_;
    memcpy(&buf[v4_of],&(row_leaf->records[i].value_4),kMaxValueSize_orderline_col4_);
    v4_of+=kMaxValueSize_orderline_col4_;
    memcpy(&buf[v5_of],&(row_leaf->records[i].value_5),kMaxValueSize_orderline_col5_);
    v5_of+=kMaxValueSize_orderline_col5_;
    memcpy(&buf[v6_of],&(row_leaf->records[i].value_6),kMaxValueSize_orderline_col6_);
    v6_of+=kMaxValueSize_orderline_col6_;
  }
...
void ColumnToRow_Cache(char* column_page, char* row_page){
  ...
  off_t v6_of = row_leaf->offset_v6;
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&(row_leaf->records[i].key),&buf[k_of],kMaxKeySize_orderline_);
    k_of+=kMaxKeySize_orderline_;
    memcpy(&(row_leaf->records[i].cur_lsn),&buf[lsn_of],sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&(row_leaf->records[i].value_1),&buf[v1_of],kMaxValueSize_orderline_col1_);
    v1_of+=kMaxValueSize_orderline_col1_;
    memcpy(&(row_leaf->records[i].value_2),&buf[v2_of],kMaxValueSize_orderline_col2_);
    v2_of+=kMaxValueSize_orderline_col2_;
    memcpy(&(row_leaf->records[i].value_3),&buf[v3_of],kMaxValueSize_orderline_col3_);
    v3_of+=kMaxValueSize_orderline_col3_;
    memcpy(&(row_leaf->records[i].value_4),&buf[v4_of],kMaxValueSize_orderline_col4_);
    v4_of+=kMaxValueSize_orderline_col4_;
    memcpy(&(row_leaf->records[i].value_5),&buf[v5_of],kMaxValueSize_orderline_col5_);
    v5_of+=kMaxValueSize_orderline_col5_;
    memcpy(&(row_leaf->records[i].value_6),&buf[v6_of],kMaxValueSize_orderline_col6_);
    v6_of+=kMaxValueSize_orderline_col6_;
  }
  ...
  void Put(T *block, int is_plog)
  {
    ...
    if (async_mode_orderline_ == 0)
    {
      if (size_lru > kMaxPages_orderline_item * 0.9)
      {
        KickLRUbyUring();
      }
      if (mtr_cache->plog_flag != 1 && mtr_cache->FLU_disabled == 0 && size_flu > kMaxPages_orderline_item * 0.5)
      {
        KickLFUbyUring();
      }
    }
  ...
  void SetMtr(BPlusOrderlineTree::Monitor *mtr_1){
    mtr_cache = mtr_1;
  }
...
class BPlusOrderlineTree::InternalCache
{
  struct Node;
...
public:
  ...
  void SetMtr(BPlusOrderlineTree::Monitor *mtr_2){
    mtr_internal = mtr_2;
  }
...
//hkc-ap-10*
void BPlusOrderlineTree::ScanAP_impl(off_t &left, off_t &right, std::string &value){
...
//hkc-ap-10
void BPlusOrderlineTree::APWorker(){
  sleep(20);
  int ap_cnt=0;
  while(true){
...
BPlusOrderlineTree::BPlusOrderlineTree(const char *path, const char *log_path, const char *chpt_path, const int bg_id_, const bool enable_backthd, const char *shadow_plog_path)
    : fd_(open(path, O_CREAT|O_DIRECT|O_RDWR)),fd_shadow_plog_(open(shadow_plog_path, O_CREAT|O_DIRECT|O_RDWR)),log_fd_(open(log_path, O_CREAT|O_APPEND|O_RDWR)),chpt_fd_(open(chpt_path, O_CREAT|O_APPEND|O_RDWR)), block_cache_(new BlockCache(enable_backthd)), internal_cache_(new InternalCache())
{
  ...
  if (fd_ == -1)
    //Exit("open db");
    printf("open orderline db failed.\n");
  if (log_fd_ == -1)
    //Exit("open log");
    printf("open orderline log failed.\n");
  if (chpt_fd_ == -1)
    //Exit("open chpt");
    printf("open orderline chpt failed.\n");
  ...
  // meta_ = Map<Meta>(kMetaOffset_orderline_);
  meta_ = new Meta();
  ...
  {
    // Initialize B+tree;
    constexpr off_t of_root = kMetaOffset_orderline_ + kUnitNodeSize;
    meta_->block = of_root;
  ...
  //hkc-ap-14
  if(scan_ap){
    std::thread thd_ap = std::thread(&BPlusOrderlineTree::APWorker,this);
    thd_ap.detach();
  }
}

BPlusOrderlineTree::~BPlusOrderlineTree()
{
  UnMap(meta_, 0, 0);
  delete block_cache_;
  close(fd_);
...
//hkc-ap-15
int BPlusOrderlineTree::PrepareReadSetAP(off_t left, off_t right, void *scan_ptr){
...
void BPlusOrderlineTree::SetPlog(){
  mtr_->plog_flag =1;
  mtr_->FLU_disabled =1;
  mtr_->warmup_flag =1;
  return;
}
void BPlusOrderlineTree::Getfd(int &fd){
  fd = mtr_->global_fd;
  return;
}
void BPlusOrderlineTree::GetStatic(int &hits_bp, int &hits_glog, int &hits_plog, int &disk_read, int &io_consume, int &error_cnt, int &dirtys, int &cleans){
...
off_t BPlusOrderlineTree::GetLSN(){
  return mtr_->global_lsn;
}
void BPlusOrderlineTree::IncreLSN(){
  mtr_->global_lsn++;
  return;
}

void BPlusOrderlineTree::ResetStatic(){
    mtr_->hits_bp = 0; 
    mtr_->hits_glog = 0; 
    mtr_->hits_plog =0;
    mtr_->disk_read=0;
...
void BPlusOrderlineTree::SetBaseline(){
  mtr_->FLU_disabled = 0;
  mtr_->plog_flag =-1;
  mtr_->warmup_flag =0;
}

void BPlusOrderlineTree::SetIOStaic(){
    mtr_->plog_insert=0;
    mtr_->leaf_insert=0;
    mtr_->leaf_split=0;
...
off_t BPlusOrderlineTree::GetChpt(){
  return mtr_->normal_chpt_now;
}

off_t BPlusOrderlineTree::GetAPTime(){
  return mtr_->exe_time;
}
off_t BPlusOrderlineTree::GetAPOff(){
  return mtr_->global_shadow_plog_off;
}
void BPlusOrderlineTree::SetLoadFlag(int flag){
  mtr_->load_flag = flag;
}
void BPlusOrderlineTree::DisableFLUOrNot(int flag){
  mtr_->FLU_disabled = flag;
}
void BPlusOrderlineTree::LoadIOStatic(int &plog_insert,int &leaf_insert,int &leaf_split,int &phy_plog_write,int &phy_plog_read,int &phy_page_read,int &phy_page_write,int &logi_page_read,int &logi_page_write){
...
int BPlusOrderlineTree::ApplyPendPages(std::map<off_t, std::pair<char*,char*>>* source, std::map<off_t, std::pair<char*,char*>>* target, std::map<off_t,std::pair<int,int>> *EmptyPages){
...
/*
void BPlusOrderlineTree::BackGround_GLog_Func()
{
  int total_KV=0;
  int final_KV=0;
  int total_Plog =0;
  ...
  for(std::map<off_t, std::pair<std::string,std::string>>::iterator it = pending_Seg->begin(); it!=pending_Seg->end();it++){    
      ...
      if(now_lsn > cur_lsn){
        ...
        std::string value = it->second.second;
          ...
          std::string value2 = value.substr(16,16);
          std::string value3 = value.substr(32,20);
          std::string value4 = value.substr(52,16);
          std::string value5 = value.substr(68,8);
          std::string value6 = value.substr(76,24);
        if (InsertKVIntoPlog(plog, key.data(), value1.data(),value2.data(),value3.data(),value4.data(),value5.data(),value6.data(), cur_lsn) <=
          ...
          for (int i = 0; i < kOrder_orderline_Plog; i++){
      ...
      if(now_lsn > cur_lsn){
        ...
        bool overflow =0;
          ...
          if(value[0]=='U'){
            ...
            std::string value_ts = value.substr(2,20);
            if(InsertDeltaIntoPlog(plog,key.data(),value_ts.data(),cur_lsn)>GetMaxKeys(0)){
              overflow = true;
              mtr_->plog_insert += 1;
            }else{
              mtr_->plog_insert += 1;
            ...
            if (InsertKVIntoPlog(plog, key.data(), value1.data(),value2.data(),value3.data(),value4.data(),value5.data(),value6.data(),cur_lsn) > GetMaxKeys(0)){
                mtr_->plog_insert += 1;
                overflow == true;
            }else{
                mtr_->plog_insert += 1;
...
//hkc-modify-21
void BPlusOrderlineTree::RowToColumn(char* initial_page, char* converted_page){
  ...
  memcpy(&buf[fence],&(row_leaf->offset_v6), sizeof(row_leaf->offset_v6));
  fence+=sizeof(row_leaf->offset_v6);
  memcpy(&buf[fence],&(row_leaf->sha_plog_offset), sizeof(row_leaf->sha_plog_offset)); //hkc-ap-17
  fence+=sizeof(row_leaf->sha_plog_offset);
...
//hkc-modify-22
void BPlusOrderlineTree::ColumnToRow(char* column_page, char* row_page){
  memset(row_page,0,kLeafPageSize);
  LeafNode* row_leaf = reinterpret_cast<LeafNode*>(row_page);
...
//hkc-gb-04
off_t BPlusOrderlineTree::GetOnlyLeafOffset(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
  return of_leaf;
}
//hkc-gb-05
char* BPlusOrderlineTree::GetLeafCopy(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
...
//hkc-gb-06
void BPlusOrderlineTree::UpdateLeafCopy(const std::string &key, const std::string &value, off_t lsn, char* leaf_copy){
...
//hkc-gb-07
void BPlusOrderlineTree::GetRowRecord(const std::string &key, std::string &value, char* leaf_copy) {
  LeafNode* leaf  = reinterpret_cast<LeafNode*>(&leaf_copy[0]);
  int index = GetIndexFromLeafNode(leaf, key.data());
  char* ptr = leaf->ValueALL(index);
  std::string tmp_value(ptr,ptr+kMaxValueSize_orderline_col_all);
  value = tmp_value;
  free(ptr);
  return ;
...
//hkc-gb-08
void BPlusOrderlineTree::TriggerFlush(const off_t* offsets, const int size, char* evict_buf){
  int batch_size = 200;
...
void BPlusOrderlineTree::Put(const std::string &key, const std::string &value, off_t lsn)
{
  ...
  if (InsertKVIntoLeafNode(leaf_node, key.data(), value1.data(),value2.data(),value3.data(),value4.data(),value5.data(),value6.data(), lsn) <=
      GetMaxKeys(1))
  {
    // 2.If records of leaf node less than or equals kOrder_orderline_ - 1 then finish.
  ...
  // until count <= kOrder_orderline_ - 1.
  size_t count;
  do
  {
...
bool BPlusOrderlineTree::Scan(const std::string &start, const std::string &end, std::string &value) const
{
  if(start>end){
    int x=1;
  }
...
//hkc-ap-19
bool BPlusOrderlineTree::ScanAP(const std::string &start, const std::string &end, std::string &value) const
{
...
/*
bool BPlusOrderlineTree::Get(const std::string &key, std::string &value) const
{
...
int BPlusOrderlineTree::Check(off_t offset)
{
  return block_cache_->Check_Leaf(offset);
}

char *BPlusOrderlineTree::GetMemCopy(off_t offset)
{
  return block_cache_->GetMemCopy_Leaf(offset);
}

template <typename T>
T *BPlusOrderlineTree::Map(off_t offset, int isleaf, int is_plog, char *&plog_copy) const
{
  if (isleaf == 1)
  {
...
template <typename T>
void BPlusOrderlineTree::InsertDirty(off_t offset) const
{
...
template <typename T>
void BPlusOrderlineTree::RelocateDirty(off_t offset, off_t ori_oldset_lsn) const
{
...
template <typename T>
void BPlusOrderlineTree::DirectInsertDirty(off_t offset_leaf, off_t offset_split) const
{
...
template <typename T>
void BPlusOrderlineTree::UnMap(T *map_obj, int isleaf, int is_plog) const
{
  if (isleaf == 1)
  {
...
size_t BPlusOrderlineTree::GetMinKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return (kOrder_orderline_Plog + 1) / 2 - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return (kOrder_orderline_ + 1) / 2 - 1;
  }
}
// 1: leaf node ; 0: plog
size_t BPlusOrderlineTree::GetMaxKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return kOrder_orderline_Plog - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return kOrder_orderline_ - 1;
  }
}
size_t BPlusOrderlineTree::GetMinIndexKeys() { return (kOrder_orderline_Index + 1) / 2 - 1; }
size_t BPlusOrderlineTree::GetMaxIndexKeys() { return kOrder_orderline_Index - 1; }

BPlusOrderlineTree::IndexNode *BPlusOrderlineTree::GetOrCreateParent(Node *node)
{
  if (node->parent == 0)
  {
    // Split root node.
...
template <typename T>
int BPlusOrderlineTree::UpperBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
  ...
  {
    int mid = (l + r) >> 1;
    if (std::strncmp(arr[mid].key, key, kMaxKeySize_orderline_) <= 0)
    {
      l = mid + 1;
    }
    else
...
template <typename T>
int BPlusOrderlineTree::UpperBoundIndex(T arr[], int n, const char *key) const
{
...
template <typename T>
int BPlusOrderlineTree::LowerBoundIndex(T arr[], int n, const char *key) const
{
  ////assert(n <= GetMaxIndexKeys());
  int l = 0, r = n - 1;
  while (l <= r)
...
// flag: 1: data page; 0: plog
template <typename T>
int BPlusOrderlineTree::LowerBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
    //assert(n <= GetMaxKeys(0));
...
template <typename T>
T *BPlusOrderlineTree::AllocLeaf()
...
template <typename T>
T *BPlusOrderlineTree::AllocIndex()
{ // 1: leaf node, 0: internal node
...
template <typename T>
void BPlusOrderlineTree::Dealloc(T *node)
{
  UnMap<T>(node);
}

void BPlusOrderlineTree::WarmUpVirtualPageCache(int x) const
{
  return block_cache_->WarmUpVPCache(x);
}
off_t BPlusOrderlineTree::GetLeafOffset(const char *key) const
{
...
inline size_t BPlusOrderlineTree::InsertKeyIntoIndexNode(IndexNode *index_node,
                                                const char *key,
                                                Node *left_node,
...
//hkc-modify-13
size_t BPlusOrderlineTree::InsertKVIntoLeafNode(LeafNode *leaf_node, const char *key,
                                       const char *value_1, const char *value_2, const char *value_3, const char *value_4, const char *value_5,const char *value_6, off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(leaf_node->Key(index - 1), key, kMaxKeySize_orderline_) == 0)
  {
...
//hkc-plog-9
size_t BPlusOrderlineTree::InsertDeltaKVIntoLeafNode(LeafNode *leaf_node, const char *key,const char* value_ts, off_t cur_lsn)
{
  //assert(leaf_node->count <= GetMaxKeys(1));
  ...
  if (index > 0 &&
      std::strncmp(leaf_node->Key(index - 1), key, kMaxKeySize_orderline_) == 0)
  {
    leaf_node->UpdateValueDelta(index - 1, value_ts,cur_lsn);
    return leaf_node->count;
  }
  leaf_node->InsertDeltaKVAtIndex(index, key,  value_ts,cur_lsn);
  ++meta_->size;
  return leaf_node->count;
}

//hkc-modify-14
size_t BPlusOrderlineTree::InsertKVIntoPlog(Plog *plog, const char *key,
                                   const char* value1, const char* value2,const char* value3,const char* value4,const char* value5,const char* value6, off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(plog->Key(index - 1), key, kMaxKeySize_orderline_) == 0)
  {
...
size_t BPlusOrderlineTree::InsertDeltaIntoPlog(Plog *plog, const char *key, const char* value_ts, off_t cur_lsn)
{
  //assert(plog->count <= GetMaxKeys(0));
  ...
  if (index > 0 &&
      std::strncmp(plog->Key(index - 1), key, kMaxKeySize_orderline_) == 0)
  {
    plog->UpdateValueDelta(index - 1, value_ts, cur_lsn);
    return plog->count;
  }
  //KV NOT EXIST IN THE PLOG, SO WE PRETEND TO HAVE ALL KVS
  plog->InsertDeltaKVAtIndex(index, key, value_ts, cur_lsn);
  ++meta_->size;
  return plog->count;
}

BPlusOrderlineTree::LeafNode *BPlusOrderlineTree::SplitLeafNode(LeafNode *leaf_node)
{
  //assert(leaf_node->count == kOrder_orderline_);
  off_t origian_lsn = leaf_node->GetSmallestLSN();
  constexpr int mid = (kOrder_orderline_ - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_orderline_ - mid;
...
BPlusOrderlineTree::IndexNode *BPlusOrderlineTree::SplitIndexNode(IndexNode *index_node)
{
  //assert(index_node->count == kOrder_orderline_Index);
  constexpr int mid = (kOrder_orderline_Index - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_orderline_Index - mid - 1;
  ...
  for (int i = mid + 1; i <= kOrder_orderline_Index; ++i)
  {
...
inline int BPlusOrderlineTree::GetIndexFromLeafNode(LeafNode *leaf_node,
                                           const char *key) const
{
  int index = LowerBound(leaf_node->records, leaf_node->count, key, 1);
  return index < static_cast<int>(leaf_node->count) &&
                 std::strncmp(leaf_node->Key(index), key, kMaxKeySize_orderline_) == 0
             ? index
             : -1;
}

inline int BPlusOrderlineTree::GetIndexFromPlog(Plog *plog,
                                       const char *key) const
{
  int index = LowerBound(plog->records, plog->count, key, 0);
  return index < static_cast<int>(plog->count) &&
                 std::strncmp(plog->Key(index), key, kMaxKeySize_orderline_) == 0
             ? index
             : -1;
}

std::vector<std::pair<std::string, std::string>> BPlusOrderlineTree::GetRange(
    const std::string &left_key, const std::string &right_key) const
{
  ...
  while (of_leaf != 0 && !finish)
  {
    ...
    {
      if (strncmp(right_leaf_node->Key(i), right_key.data(), kMaxKeySize_orderline_) <=
          0)
      {
...
bool BPlusOrderlineTree::Empty() const { return meta_->size == 0; }

size_t BPlusOrderlineTree::Size() const { return meta_->size; }