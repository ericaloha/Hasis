...
// Schema  
  ...
  //i_id int not null,  16
  //i_im_id int, 16
  //i_name varchar(24), 24
  //i_price decimal(5,2), 8
  //i_data varchar(50), 50
...
// switch
const int async_mode_item_ = 0; // 0 for sync IO; 1 for async IO
const int kAIOBatchSize_item = 20;
const int kOrder_item_ = 1000; //how many record a page can contain
const int kOrder_item_Plog = kOrder_item_ / 10;
// const int kOrder_item_ = 50; //for key
const int kOrder_item_Index = 6 * 500; // for index

static_assert(kOrder_item_ >= 3,
              "The order of B+Tree should be greater than or equal to 3.");
const int kMaxKeySize_item_ = 16;
const int kMaxLSNSize_item_ = 8;
// const int kMaxValueSize_item_ = 256;
const int kMaxValueSize_item_col1_ = 16;
const int kMaxValueSize_item_col2_ = 24;
const int kMaxValueSize_item_col3_ = 8;
const int kMaxValueSize_item_col4_ = 50;
const int kMaxValueSize_item_col_all = 16+24+8+50;
const off_t kMetaOffset_item_ = 0;

int kMaxPages_item_item = 1000;     // 100M
const int kMaxFreePages_item_ = 1100; 
// const int kMaxPages_item_item = 80000; //1G

typedef char Key[kMaxKeySize_item_];
typedef char Value1[kMaxValueSize_item_col1_];
typedef char Value2[kMaxValueSize_item_col2_];
typedef char Value3[kMaxValueSize_item_col3_];
typedef char Value4[kMaxValueSize_item_col4_];
typedef char Value_all[kMaxValueSize_item_col1_+kMaxValueSize_item_col2_+kMaxValueSize_item_col3_+kMaxValueSize_item_col4_]; //for log record
...
const int innerKeyoffset_item_ = 0;
const int innercoloffset_item_ = 0;

const int kMaxUniqueKInGLogSeg=3000;
  


struct BPlusItemTree::Monitor{
  Monitor(){
    bg_id =0;
    plog_insert = 0;
    leaf_insert = 0;
...
struct BPlusItemTree::Index
{
  ...
  void UpdateIndex(off_t of, off_t lsn, const char *k)
  {
    offset = of;
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_item_);
  }
...
struct BPlusItemTree::Record
{
  ...
  void UpdateKV(const char *k, const char *v1, const char *v2, const char *v3, const char *v4, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_item_);
    strncpy(value_1, v1, kMaxValueSize_item_col1_);
    strncpy(value_2, v2, kMaxValueSize_item_col2_);
    strncpy(value_3, v3, kMaxValueSize_item_col3_);
    strncpy(value_4, v4, kMaxValueSize_item_col4_);
  }
  void UpdateKey(const char *k, off_t lsn)
  {
    cur_lsn = lsn;
  ...
  void UpdateValue(const char *v1, const char *v2, const char *v3, const char *v4, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(value_1, v1, kMaxValueSize_item_col1_);
    strncpy(value_2, v2, kMaxValueSize_item_col2_);
...
struct BPlusItemTree::PAXKeyRecord
{
  Key key;
  off_t cur_lsn;
...
struct BPlusItemTree::PAXColumn1Record
{
  Value1 value_1;
...
struct BPlusItemTree::PAXColumn2Record
{
  Value2 value_2;
...
struct BPlusItemTree::PAXColumn3Record
{
  Value3 value_3;
...
struct BPlusItemTree::PAXColumn4Record
{
  Value4 value_4;
...
struct BPlusItemTree::Per_page_Log
{
  Key key;
  Value_all value_all;
  off_t cur_lsn;
  ...
  void UpdateKV(const char *k, const char *v, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_item_);
    strncpy(value_all, v, kMaxValueSize_item_col_all);
  }
  void UpdateKey(const char *k, off_t lsn)
  {
    cur_lsn = lsn;
...
struct BPlusItemTree::LogRecord
{
  LogRecord() : page_offset(0), log_lsn(0), log_offset(0), next_log_offset(0), page_index(0)
  {
    memset(key, 0, kMaxKeySize_item_);
    memset(value, 0, kMaxValueSize_item_col_all);
  }
  ...
  char key[kMaxKeySize_item_];
  char value[kMaxValueSize_item_col_all];

  void UpdateKV(const char *k, const char *v)
  {
    strncpy(key, k, kMaxKeySize_item_);
    strncpy(value, v, kMaxValueSize_item_col_all);
  }
  void UpdateKey(const char *k) { strncpy(key, k, kMaxKeySize_item_); }
  void UpdateValue(const char *v) { strncpy(value, v, kMaxValueSize_item_col_all); }
};

struct BPlusItemTree::Checkpoint
{
...
struct BPlusItemTree::IndexNode : BPlusItemTree::Node
{
  IndexNode() = default;
  ~IndexNode() = default;
  ...
  const char *Key(int index) const
  {
    //assert(count > 0);
    //assert(index >= 0);
    //assert(index <= kOrder_item_Index);
  ...
  Index indexes[kOrder_item_Index + 1];
};

struct BPlusItemTree::LeafNode : BPlusItemTree::Node
{
  LeafNode() = default;
  ~LeafNode() = default;
  ...
  {
    //assert(count > 0);
    char* all = (char*)malloc(sizeof(char)*kMaxValueSize_item_col_all);
    int off =0;
    memcpy(&all[off],records[index].value_1,kMaxValueSize_item_col1_);
    off+=kMaxValueSize_item_col1_;
    memcpy(&all[off],records[index].value_2,kMaxValueSize_item_col2_);
    off+=kMaxValueSize_item_col2_;
    memcpy(&all[off],records[index].value_3,kMaxValueSize_item_col3_);
    off+=kMaxValueSize_item_col3_;
    memcpy(&all[off],records[index].value_4,kMaxValueSize_item_col4_);
    return all;
  }
  ...
  void UpdateValue(int index, const char *v1, const char *v2, const char *v3, const char *v4, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateValue(v1, v2, v3,v4, lsn);
  }
  ...
  void UpdateKV(int index, const char *k, const char *v1,const char *v2,const char *v3,const char *v4, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateKV(k, v1,v2,v3,v4, lsn);
  }

  void AppendKV(int index, const char *k, const char *v1,const char *v2,const char *v3,const char *v4, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateKV(k, v1,v2,v3,v4, lsn);
  }

  void InsertKVAtIndex(int index, const char *k, const char *v1,const char *v2,const char *v3,const char *v4, off_t lsn)
  {
    //assert(index >= 0);
    //assert(index < kOrder_item_);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateKV(index, k, v1,v2,v3,v4, lsn);
  }
  ...
  off_t offset_v3;
  off_t offset_v4;
  BPlusItemTree::Record records[kOrder_item_];
  // BPlusItemTree::Record Per_page_logs[kOrder_item_/2];
  // off_t cur_pplog_index;
};


struct BPlusItemTree::PAXLeafNode : BPlusItemTree::Node
{
  PAXLeafNode() = default;
  ~PAXLeafNode() = default;
  ...
  off_t offset_v3;
  off_t offset_v4;
  BPlusItemTree::PAXKeyRecord key_records[kOrder_item_];
  BPlusItemTree::PAXColumn1Record v1_records[kOrder_item_];
  BPlusItemTree::PAXColumn2Record v2_records[kOrder_item_];
  BPlusItemTree::PAXColumn3Record v3_records[kOrder_item_];
  BPlusItemTree::PAXColumn4Record v4_records[kOrder_item_];
  // BPlusItemTree::Record Per_page_logs[kOrder_item_/2];
  // off_t cur_pplog_index;
};
...
struct BPlusItemTree::Plog : BPlusItemTree::Node
{
  Plog() = default;
  ~Plog() = default;
  ...
  // BPlusItemTree::Record records[kOrder_item_/2];
  BPlusItemTree::Record records[kOrder_item_Plog];
};

class BPlusItemTree::BlockCache
{
  struct Node;
...
public:
  BlockCache(bool enable_backthd)
  {
    ...
    // pre_allocate
    // 2. init free list
    char *init_ptr = (char *)malloc(kMaxFreePages_item_ * kLeafPageSize); // i.e., for each page
    memset(init_ptr, 0, kMaxFreePages_item_ * kLeafPageSize);

    char *flag_addr = init_ptr;
    for (int i = 0; i < kMaxFreePages_item_; i++)
    {
...
void RowToColumn_Cache(char* initial_page, char* converted_page){
  ...
  */
  ...
  row_leaf->offset_lsn =(4+16)*1024;
  row_leaf->offset_v1 = (4+16+8)*1024;
  row_leaf->offset_v2 = (4+16+8+16)*1024;
  row_leaf->offset_v3 = (4+16+8+16+24)*1024;
  row_leaf->offset_v4 = (4+16+8+16+24+8)*1024;
  ...
  //convert row as column and fit in sub pages
    ...
    /*
      const int kMaxKeySize_item_ = 16; 16K
      const int kMaxValueSize_item_col1_ = 16; 16K
      const int kMaxValueSize_item_col2_ = 24; 24K
      const int kMaxValueSize_item_col3_ = 8; 8K
      const int kMaxValueSize_item_col4_ = 50; 50K
    */
  ...
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&buf[k_of],&(row_leaf->records[i].key),kMaxKeySize_item_);
    k_of+=kMaxKeySize_item_;
    memcpy(&buf[lsn_of],&(row_leaf->records[i].cur_lsn),sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&buf[v1_of],&(row_leaf->records[i].value_1),kMaxValueSize_item_col1_);
    v1_of+=kMaxValueSize_item_col1_;
    memcpy(&buf[v2_of],&(row_leaf->records[i].value_2),kMaxValueSize_item_col2_);
    v2_of+=kMaxValueSize_item_col2_;
    memcpy(&buf[v3_of],&(row_leaf->records[i].value_3),kMaxValueSize_item_col3_);
    v3_of+=kMaxValueSize_item_col3_;
    memcpy(&buf[v4_of],&(row_leaf->records[i].value_4),kMaxValueSize_item_col4_);
    v4_of+=kMaxValueSize_item_col4_;
  }
...
void ColumnToRow_Cache(char* column_page, char* row_page){
  ...
  */
  ...
  off_t v4_of = row_leaf->offset_v4;
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&(row_leaf->records[i].key),&buf[k_of],kMaxKeySize_item_);
    k_of+=kMaxKeySize_item_;
    memcpy(&(row_leaf->records[i].cur_lsn),&buf[lsn_of],sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&(row_leaf->records[i].value_1),&buf[v1_of],kMaxValueSize_item_col1_);
    v1_of+=kMaxValueSize_item_col1_;
    memcpy(&(row_leaf->records[i].value_2),&buf[v2_of],kMaxValueSize_item_col2_);
    v2_of+=kMaxValueSize_item_col2_;
    memcpy(&(row_leaf->records[i].value_3),&buf[v3_of],kMaxValueSize_item_col3_);
    v3_of+=kMaxValueSize_item_col3_;
    memcpy(&(row_leaf->records[i].value_4),&buf[v4_of],kMaxValueSize_item_col4_);
    v4_of+=kMaxValueSize_item_col4_;
  }
  ...
  void Put(T *block, int is_plog)
  {
    ...
    if (async_mode_item_ == 0)
    {
      if (size_lru > kMaxPages_item_item * 0.9)
      {
        KickLRUbyUring();
      }
      if (mtr_cache->plog_flag != 1 && mtr_cache->FLU_disabled == 0 && size_flu > kMaxPages_item_item * 0.5)
      {
        KickLFUbyUring();
      }
    }
  ...
  void SetMtr(BPlusItemTree::Monitor *mtr_1){
    mtr_cache = mtr_1;
  }
...
class BPlusItemTree::InternalCache
{
  struct Node;
...
public:
  ...
  void SetMtr(BPlusItemTree::Monitor *mtr_2){
    mtr_internal = mtr_2;
  }
...
//hkc-ap-10*
void BPlusItemTree::ScanAP_impl(off_t &left, off_t &right, std::string &value){
...
BPlusItemTree::BPlusItemTree(const char *path, const char *log_path, const char *chpt_path, const int bg_id_, const bool enable_backthd, const char *shadow_plog_path)
    : fd_(open(path, O_CREAT|O_DIRECT|O_RDWR)),fd_shadow_plog_(open(shadow_plog_path, O_CREAT|O_DIRECT|O_RDWR)),log_fd_(open(log_path, O_CREAT|O_APPEND|O_RDWR)),chpt_fd_(open(chpt_path, O_CREAT|O_APPEND|O_RDWR)), block_cache_(new BlockCache(enable_backthd)), internal_cache_(new InternalCache())
{
  ...
  if (fd_ == -1)
    //Exit("open db");
    printf("open item db failed.\n");
  if (log_fd_ == -1)
    //Exit("open log");
    printf("open item log failed.\n");
  if (chpt_fd_ == -1)
    //Exit("open chpt");
    printf("open item chpt failed.\n");
  ...
  // meta_ = Map<Meta>(kMetaOffset_item_);
  meta_ = new Meta();
  ...
  {
    // Initialize B+tree;
    constexpr off_t of_root = kMetaOffset_item_ + kUnitNodeSize;
    meta_->block = of_root;
    ...
    //hkc-ap-14
  if(scan_ap){
    std::thread thd_ap = std::thread(&BPlusItemTree::APWorker,this);
    thd_ap.detach();
  }
}

BPlusItemTree::~BPlusItemTree()
{
  UnMap(meta_, 0, 0);
  delete block_cache_;
  close(fd_);
...
//hkc-ap-15
int BPlusItemTree::PrepareReadSetAP(off_t left, off_t right, void *scan_ptr){
...
void BPlusItemTree::GetStatic(int &hits_bp, int &hits_glog, int &hits_plog, int &disk_read, int &io_consume, int &error_cnt, int &dirtys, int &cleans){
...
off_t BPlusItemTree::GetLSN(){
  return mtr_->global_lsn;
}
void BPlusItemTree::IncreLSN(){
  mtr_->global_lsn++;
  return;
}

void BPlusItemTree::ResetStatic(){
    mtr_->hits_bp = 0; 
    mtr_->hits_glog = 0; 
    mtr_->hits_plog =0;
    mtr_->disk_read=0;
...
void BPlusItemTree::SetBaseline(){
  mtr_->FLU_disabled = 0;
  mtr_->plog_flag =-1;
  mtr_->warmup_flag =0;
}

void BPlusItemTree::SetIOStaic(){
    mtr_->plog_insert=0;
    mtr_->leaf_insert=0;
    mtr_->leaf_split=0;
...
off_t BPlusItemTree::GetChpt(){
  return mtr_->normal_chpt_now;
}
void BPlusItemTree::SetLoadFlag(int flag){
  mtr_->load_flag = flag;
}
void BPlusItemTree::DisableFLUOrNot(int flag){
  mtr_->FLU_disabled = flag;
}
void BPlusItemTree::LoadIOStatic(int &plog_insert,int &leaf_insert,int &leaf_split,int &phy_plog_write,int &phy_plog_read,int &phy_page_read,int &phy_page_write,int &logi_page_read,int &logi_page_write){
...
int BPlusItemTree::ApplyPendPages(std::map<off_t, std::pair<char*,char*>>* source, std::map<off_t, std::pair<char*,char*>>* target, std::map<off_t,std::pair<int,int>> *EmptyPages){
...
void BPlusItemTree::BackGround_GLog_Func()
{
  int total_KV=0;
  int final_KV=0;
  int total_Plog =0;
  ...
  for(std::map<off_t, std::pair<std::string,std::string>>::iterator it = pending_Seg->begin(); it!=pending_Seg->end();it++){    
      ...
      if(now_lsn > cur_lsn){
        ...
        std::string value = it->second.second;
          ...
          std::string value2 = value.substr(16,24);
          std::string value3 = value.substr(40,8);
          std::string value4 = value.substr(48,50);
        if (InsertKVIntoPlog(plog, key.data(), value1.data(),value2.data(),value3.data(),value4.data(), cur_lsn) <=
          GetMaxKeys(0)){
          mtr_->plog_insert += 1;
          /*
          if (Check(of_leaf) == 1){
          ...
          */
          ...
          for (int i = 0; i < kOrder_item_Plog; i++){
...
void BPlusItemTree::RowToColumn(char* initial_page, char* converted_page){
...
void BPlusItemTree::ColumnToRow(char* column_page, char* row_page){
...
//hkc-gb-04
off_t BPlusItemTree::GetOnlyLeafOffset(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
  return of_leaf;
}
//hkc-gb-05
char* BPlusItemTree::GetLeafCopy(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
...
//hkc-gb-06
void BPlusItemTree::UpdateLeafCopy(const std::string &key, const std::string &value, off_t lsn, char* leaf_copy){
  LeafNode* leaf = reinterpret_cast<LeafNode*>(&leaf_copy[0]);
  std::string value1 = value.substr(0,16);
  std::string value2 = value.substr(16,24);
  std::string value3 = value.substr(40,8);
  std::string value4 = value.substr(48,50);
  if (InsertKVIntoLeafNode(leaf, key.data(),  value1.data(),value2.data(),value3.data(),value4.data(), lsn) <= GetMaxKeys(1)){
...
//hkc-gb-07
void BPlusItemTree::GetRowRecord(const std::string &key, std::string &value, char* leaf_copy) {
  LeafNode* leaf  = reinterpret_cast<LeafNode*>(&leaf_copy[0]);
  int index = GetIndexFromLeafNode(leaf, key.data());
  char* ptr = leaf->ValueALL(index);
  std::string tmp_value(ptr,ptr+kMaxValueSize_item_col_all);
  value = tmp_value;
  free(ptr);
  return ;
...
//hkc-gb-08
void BPlusItemTree::TriggerFlush(const off_t* offsets, const int size, char* evict_buf){
  int batch_size = 200;
...
void BPlusItemTree::Put(const std::string &key, const std::string &value, off_t lsn)
{
  ...
  std::string value3 = value.substr(40,8);
  std::string value4 = value.substr(48,50);
  if (InsertKVIntoLeafNode(leaf_node, key.data(), value1.data(),value2.data(),value3.data(),value4.data(), lsn) <=
      GetMaxKeys(1))
  {
    // 2.If records of leaf node less than or equals kOrder_item_ - 1 then finish.
...
/*
bool BPlusItemTree::Get(const std::string &key, std::string &value) const
{
...
bool BPlusItemTree::Scan(const std::string &start, const std::string &end, std::string &value) const
{
  if(start>end){
    int x=1;
  }
...
//hkc-ap-19
bool BPlusItemTree::ScanAP(const std::string &start, const std::string &end, std::string &value) const
{
...
int BPlusItemTree::Check(off_t offset)
{
  return block_cache_->Check_Leaf(offset);
}

char *BPlusItemTree::GetMemCopy(off_t offset)
{
  return block_cache_->GetMemCopy_Leaf(offset);
}

template <typename T>
T *BPlusItemTree::Map(off_t offset, int isleaf, int is_plog, char *&plog_copy) const
{
  if (isleaf == 1)
  {
...
template <typename T>
void BPlusItemTree::InsertDirty(off_t offset) const
{
...
template <typename T>
void BPlusItemTree::RelocateDirty(off_t offset, off_t ori_oldset_lsn) const
{
...
template <typename T>
void BPlusItemTree::DirectInsertDirty(off_t offset_leaf, off_t offset_split) const
{
...
template <typename T>
void BPlusItemTree::UnMap(T *map_obj, int isleaf, int is_plog) const
{
  if (isleaf == 1)
  {
...
size_t BPlusItemTree::GetMinKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return (kOrder_item_Plog + 1) / 2 - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return (kOrder_item_ + 1) / 2 - 1;
  }
}
// 1: leaf node ; 0: plog
size_t BPlusItemTree::GetMaxKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return kOrder_item_Plog - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return kOrder_item_ - 1;
  }
}
size_t BPlusItemTree::GetMinIndexKeys() { return (kOrder_item_Index + 1) / 2 - 1; }
size_t BPlusItemTree::GetMaxIndexKeys() { return kOrder_item_Index - 1; }

BPlusItemTree::IndexNode *BPlusItemTree::GetOrCreateParent(Node *node)
{
  if (node->parent == 0)
  {
    // Split root node.
...
template <typename T>
int BPlusItemTree::UpperBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
  ...
  {
    int mid = (l + r) >> 1;
    if (std::strncmp(arr[mid].key, key, kMaxKeySize_item_) <= 0)
    {
      l = mid + 1;
    }
    else
...
template <typename T>
int BPlusItemTree::UpperBoundIndex(T arr[], int n, const char *key) const
{
...
template <typename T>
int BPlusItemTree::LowerBoundIndex(T arr[], int n, const char *key) const
{
  ////assert(n <= GetMaxIndexKeys());
  int l = 0, r = n - 1;
  while (l <= r)
...
// flag: 1: data page; 0: plog
template <typename T>
int BPlusItemTree::LowerBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
    //assert(n <= GetMaxKeys(0));
...
template <typename T>
void BPlusItemTree::Dealloc(T *node)
{
  UnMap<T>(node);
}

void BPlusItemTree::WarmUpVirtualPageCache(int x) const
{
  return block_cache_->WarmUpVPCache(x);
}
off_t BPlusItemTree::GetLeafOffset(const char *key) const
{
...
inline size_t BPlusItemTree::InsertKeyIntoIndexNode(IndexNode *index_node,
                                                const char *key,
                                                Node *left_node,
...
size_t BPlusItemTree::InsertKVIntoLeafNode(LeafNode *leaf_node, const char *key,
                                       const char *value_1, const char *value_2, const char *value_3, const char *value_4, off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(leaf_node->Key(index - 1), key, kMaxKeySize_item_) == 0)
  {
    leaf_node->UpdateValue(index - 1, value_1,value_2,value_3,value_4, cur_lsn);
    return leaf_node->count;
  }
  leaf_node->InsertKVAtIndex(index, key, value_1,value_2,value_3,value_4,cur_lsn);
  ++meta_->size;
  return leaf_node->count;
}

size_t BPlusItemTree::InsertKVIntoPlog(Plog *plog, const char *key,
                                   const char* value1, const char* value2,const char* value3,const char* value4, off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(plog->Key(index - 1), key, kMaxKeySize_item_) == 0)
  {
    plog->UpdateValue(index - 1, value1,value2,value3,value4, cur_lsn);
    return plog->count;
  }
  plog->InsertKVAtIndex(index, key, value1,value2,value3,value4, cur_lsn);
  ++meta_->size;
  return plog->count;
}

BPlusItemTree::LeafNode *BPlusItemTree::SplitLeafNode(LeafNode *leaf_node)
{
  //assert(leaf_node->count == kOrder_item_);
  off_t origian_lsn = leaf_node->GetSmallestLSN();
  constexpr int mid = (kOrder_item_ - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_item_ - mid;
...
BPlusItemTree::IndexNode *BPlusItemTree::SplitIndexNode(IndexNode *index_node)
{
  //assert(index_node->count == kOrder_item_Index);
  constexpr int mid = (kOrder_item_Index - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_item_Index - mid - 1;
  ...
  for (int i = mid + 1; i <= kOrder_item_Index; ++i)
  {
...
inline int BPlusItemTree::GetIndexFromLeafNode(LeafNode *leaf_node,
                                           const char *key) const
{
  int index = LowerBound(leaf_node->records, leaf_node->count, key, 1);
  return index < static_cast<int>(leaf_node->count) &&
                 std::strncmp(leaf_node->Key(index), key, kMaxKeySize_item_) == 0
             ? index
             : -1;
}

inline int BPlusItemTree::GetIndexFromPlog(Plog *plog,
                                       const char *key) const
{
  int index = LowerBound(plog->records, plog->count, key, 0);
  return index < static_cast<int>(plog->count) &&
                 std::strncmp(plog->Key(index), key, kMaxKeySize_item_) == 0
             ? index
             : -1;
}

std::vector<std::pair<std::string, std::string>> BPlusItemTree::GetRange(
    const std::string &left_key, const std::string &right_key) const
{
  ...
  while (of_leaf != 0 && !finish)
  {
    ...
    {
      if (strncmp(right_leaf_node->Key(i), right_key.data(), kMaxKeySize_item_) <=
          0)
      {
...
bool BPlusItemTree::Empty() const { return meta_->size == 0; }

size_t BPlusItemTree::Size() const { return meta_->size; }