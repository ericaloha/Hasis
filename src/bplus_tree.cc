...
struct BPlusTree::Meta
{
  ...
  //new statistics
  size_t key_size;  //key size in bytes, replace the above size
  size_t no_of_col_;  //for this b-tree, how many columns in a page
  std::vector<int> *value_size_; //the value size of each column
};
...
struct BPlusTree::LeafNode : BPlusTree::Node
{
  ...
  off_t offset_leaf;
  std::vector<off_t>* internal_offset;
...
BPlusTree::BPlusTree(const char *path, const char *log_path, const char *chpt_path, const int bg_id_, const bool enable_backthd, const int key_size, const int no_of_col, std::vector<int> *value_of_col_)
    : fd_(open(path, O_CREAT|O_DIRECT|O_RDWR)),log_fd_(open(log_path, O_CREAT|O_APPEND|O_RDWR)),chpt_fd_(open(chpt_path, O_CREAT|O_APPEND|O_RDWR)), block_cache_(new BlockCache(enable_backthd)), internal_cache_(new InternalCache())
{
  ...
  key_size_ = key_size;
  no_of_column_ = no_of_col;
  value_size_of_each_column_ = value_of_col_;
  mtr_ = new Monitor();
  mtr_->bg_fd=bg_id_;
  ...
  if (meta_->height == 0)
  {
    ...
    meta_->block = of_root;
    meta_->key_size = key_size;
    meta_->no_of_col_ = no_of_column_;
    meta_->value_size_ = value_of_col_;
