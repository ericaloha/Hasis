...
//  s_i_id int not null, key 16
//  s_w_id smallint not null, key 16
//                  lsn,    8

//  s_quantity smallint, v1 16

//  s_dist_01 char(24), agg_v2 (240)  
//  s_dist_02 char(24),
//  s_dist_03 char(24),
//  s_dist_04 char(24), 
//  s_dist_05 char(24), 
...
//  s_dist_09 char(24), 
//  s_dist_10 char(24), 
//  s_ytd decimal(8,0), v3 8
//  s_order_cnt smallint, v4 16
//  s_remote_cnt smallint, v5 16
//  s_data varchar(50), v6 50
//  PRIMARY KEY(s_w_id, s_i_id) ) Engine=InnoDB

//  INSERT INTO stock values(1,1,23,
//    'bm4cdxlSRMRh7dFbFfF8TbKk','tUiCgklSoL3l6VCQvuXW96lm',
//    'sGuCy83SoD6YzYMKZ8R5KryC','UJTD6p5KjHIsIigWoFTKZnth',
//    'w04uCRMUsaahfuPvAHDsvhg8','yiwrN8oNAgYZ32Sba9GUYBen',
//    'Uh2ApD6V7xtFjEQOkzPmP0MX','o3QQrluWn2Jy8p54gJlsj9dg',
//    'N4WiblUe9vTunzfqJew6h0k0','chtDMQZQuFzfjdoc5gQ2MGJy',
//     0,0,0,'NfB7eviJotfCS37h0kPeZ1hDRj'
//  )
...
// switch
const int async_mode_stock_ = 0; // 0 for sync IO; 1 for async IO
const int kAIOBatchSize_item = 20;
const int kOrder_stock_ = 220; //how many record a page can contain
const int kOrder_stock_Plog = kOrder_stock_ / 10;
// const int kOrder_stock_ = 50; //for key
const int kOrder_stock_Index = 3 * 200; // for index

static_assert(kOrder_stock_ >= 3,
              "The order of B+Tree should be greater than or equal to 3.");
const int kMaxKeySize_stock_ = 32;
const int kMaxLSNSize_stock_ = 8;
// const int kMaxValueSize_stock_ = 256;
const int kMaxValueSize_stock_col1_ = 16;
const int kMaxValueSize_stock_col2_ = 240;
const int kMaxValueSize_stock_col3_ = 8;
const int kMaxValueSize_stock_col4_ = 16;
const int kMaxValueSize_stock_col5_ = 16;
const int kMaxValueSize_stock_col6_ = 50;

const int kMaxValueSize_stock_col_all = 16+240+8+32+50;
const off_t kMetaOffset_stock_ = 0;

int kMaxPages_stock_stock = 500;     // 100M
const int kMaxFreePages_stock_ = 600; 
// const int kMaxPages_stock_stock = 80000; //1G

typedef char Key[kMaxKeySize_stock_];
typedef char Value1[kMaxValueSize_stock_col1_];
typedef char Value2[kMaxValueSize_stock_col2_];
typedef char Value3[kMaxValueSize_stock_col3_];
typedef char Value4[kMaxValueSize_stock_col4_];
typedef char Value5[kMaxValueSize_stock_col5_];
typedef char Value6[kMaxValueSize_stock_col6_];
typedef char Value_all[kMaxValueSize_stock_col_all]; //for log record

//hkc-plog-1
const int KPosQuality = 0;
...
const int innerKeyoffset_stock_ = 0;
const int innercoloffset_stock_ = 0;

const int kMaxUniqueKInGLogSeg=3000;
  


struct BPlusStockTree::Monitor{
  Monitor(){
    bg_id =0;
    plog_insert = 0;
    leaf_insert = 0;
...
struct BPlusStockTree::Index
{
  ...
  void UpdateIndex(off_t of, off_t lsn, const char *k)
  {
    offset = of;
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_stock_);
  }
...
struct BPlusStockTree::Record
{
  ...
  void UpdateKV(const char *k, const char *v1, const char *v2, const char *v3, const char *v4, const char *v5, const char *v6, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_stock_);
    strncpy(value_1, v1, kMaxValueSize_stock_col1_);
    strncpy(value_2, v2, kMaxValueSize_stock_col2_);
    strncpy(value_3, v3, kMaxValueSize_stock_col3_);
    strncpy(value_4, v4, kMaxValueSize_stock_col4_);
    strncpy(value_5, v5, kMaxValueSize_stock_col5_);
    strncpy(value_6, v6, kMaxValueSize_stock_col6_);
  }
  
  //hkc-plog-2
  void UpdateDeltaKV(const char *k,const char *v_quality,off_t lsn)
  {
    cur_lsn = lsn;
    partial = true;
    strncpy(key, k, kMaxKeySize_stock_);
    strncpy(value_1, v_quality, kMaxValueSize_stock_col1_);
  }
  ...
  void UpdateValue(const char *v1, const char *v2, const char *v3, const char *v4, const char *v5, const char *v6, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(value_1, v1, kMaxValueSize_stock_col1_);
    strncpy(value_2, v2, kMaxValueSize_stock_col2_);
  ...
  //hkc-plog-3
  void UpdateValueDelta(const char *v_quality, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(value_1, v_quality, kMaxValueSize_stock_col1_);
  }
...
struct BPlusStockTree::PAXKeyRecord
{
  Key key;
  off_t cur_lsn;
...
struct BPlusStockTree::PAXColumn1Record
{
  Value1 value_1;

  void UpdateKV(const char *v1)
  {
    strncpy(value_1, v1, kMaxValueSize_stock_col1_);
  }
...
struct BPlusStockTree::PAXColumn2Record
{
  Value2 value_2;

  void UpdateKV( const char *v2)
  {
    strncpy(value_2, v2, kMaxValueSize_stock_col2_);
  }
...
struct BPlusStockTree::PAXColumn3Record
{
  Value3 value_3;

  void UpdateKV(const char *v3)
  {
    strncpy(value_3, v3, kMaxValueSize_stock_col3_);
  }

  void UpdateValue(const char *v3)
  {
    strncpy(value_3, v3, kMaxValueSize_stock_col3_);
  }
};
...
struct BPlusStockTree::PAXColumn4Record
{
  Value4 value_4;

  void UpdateKV(const char *v4)
  {
    strncpy(value_4, v4, kMaxValueSize_stock_col4_);
  }

  void UpdateValue(const char *v4)
  {
    strncpy(value_4, v4, kMaxValueSize_stock_col4_);
  }
};

struct BPlusStockTree::PAXColumn5Record
{
  Value5 value_5;

  void UpdateKV(const char *v5)
  {
    strncpy(value_5, v5, kMaxValueSize_stock_col5_);
  }

  void UpdateValue(const char *v5)
  {
    strncpy(value_5, v5, kMaxValueSize_stock_col5_);
  }
};

struct BPlusStockTree::PAXColumn6Record
{
  Value6 value_6;

  void UpdateKV(const char *v6)
  {
    strncpy(value_6, v6, kMaxValueSize_stock_col6_);
  }

  void UpdateValue(const char *v6)
  {
    strncpy(value_6, v6, kMaxValueSize_stock_col6_);
  }
};
...
struct BPlusStockTree::Per_page_Log
{
  Key key;
  Value_all value_all;
  off_t cur_lsn;
  ...
  void UpdateKV(const char *k, const char *v, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_stock_);
    strncpy(value_all, v, kMaxValueSize_stock_col_all);
  }
  void UpdateKey(const char *k, off_t lsn)
  {
    cur_lsn = lsn;
...
struct BPlusStockTree::LogRecord
{
  LogRecord() : page_offset(0), log_lsn(0), log_offset(0), next_log_offset(0), page_index(0)
  {
    memset(key, 0, kMaxKeySize_stock_);
    memset(value, 0, kMaxValueSize_stock_col_all);
  }
  ...
  char key[kMaxKeySize_stock_];
  char value[kMaxValueSize_stock_col_all];

  void UpdateKV(const char *k, const char *v)
  {
    strncpy(key, k, kMaxKeySize_stock_);
    strncpy(value, v, kMaxValueSize_stock_col_all);
  }
  void UpdateKey(const char *k) { strncpy(key, k, kMaxKeySize_stock_); }
  void UpdateValue(const char *v) { strncpy(value, v, kMaxValueSize_stock_col_all); }
};

struct BPlusStockTree::Checkpoint
{
...
struct BPlusStockTree::IndexNode : BPlusStockTree::Node
{
  IndexNode() = default;
  ~IndexNode() = default;
  ...
  const char *Key(int index) const
  {
    //assert(count > 0);
    //assert(index >= 0);
    //assert(index <= kOrder_stock_Index);
  ...
  Index indexes[kOrder_stock_Index + 1];
};

struct BPlusStockTree::LeafNode : BPlusStockTree::Node
{
  LeafNode() = default;
  ~LeafNode() = default;
  ...
  {
    //assert(count > 0);
    char* all = (char*)malloc(sizeof(char)*kMaxValueSize_stock_col_all);
    int off =0;
    memcpy(&all[off],records[index].value_1,kMaxValueSize_stock_col1_);
    off+=kMaxValueSize_stock_col1_;
    memcpy(&all[off],records[index].value_2,kMaxValueSize_stock_col2_);
    off+=kMaxValueSize_stock_col2_;
    memcpy(&all[off],records[index].value_3,kMaxValueSize_stock_col3_);
    off+=kMaxValueSize_stock_col3_;
    memcpy(&all[off],records[index].value_4,kMaxValueSize_stock_col4_);
    off+=kMaxValueSize_stock_col4_;
    memcpy(&all[off],records[index].value_5,kMaxValueSize_stock_col5_);
    off+=kMaxValueSize_stock_col5_;
    memcpy(&all[off],records[index].value_6,kMaxValueSize_stock_col6_);
    return all;
  }
  ...
  void UpdateValue(int index, const char *v1, const char *v2, const char *v3, const char *v4,const char *v5,const char *v6, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateValue(v1, v2, v3,v4,v5,v6, lsn);
  }

  //hkc-plog-4
  void UpdateValueDelta(int index, const char *v_quality, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateValueDelta(v_quality, lsn);
  }
  ...
  void UpdateKV(int index, const char *k, const char *v1,const char *v2,const char *v3,const char *v4, const char *v5,const char *v6, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateKV(k, v1,v2,v3,v4,v5,v6, lsn);
  }

  //hkc-plog-5
  void UpdateDeltaKV(int index, const char *k, const char *v_quality, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateDeltaKV(k, v_quality, lsn);
  }

  void AppendKV(int index, const char *k, const char *v1,const char *v2,const char *v3,const char *v4, const char *v5,const char *v6, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateKV(k, v1,v2,v3,v4,v5,v6, lsn);
  }

  void InsertKVAtIndex(int index, const char *k, const char *v1,const char *v2,const char *v3,const char *v4, const char *v5,const char *v6, off_t lsn)
  {
    //assert(index >= 0);
    //assert(index < kOrder_stock_);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateKV(index, k, v1,v2,v3,v4,v5,v6,lsn);
  }

  //hkc-plog-6
  void InsertDeltaKVAtIndex(int index, const char *k, const char *v_quality, off_t lsn)
  {
    //assert(index >= 0);
    //assert(index < kOrder_customer_);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateDeltaKV(index, k, v_quality,lsn);
  }
  ...
  off_t offset_v5;
  off_t offset_v6;
  BPlusStockTree::Record records[kOrder_stock_];
  // BPlusStockTree::Record Per_page_logs[kOrder_stock_/2];
  // off_t cur_pplog_index;
};

/*
struct BPlusStockTree::PAXLeafNode : BPlusStockTree::Node
{
  PAXLeafNode() = default;
  ~PAXLeafNode() = default;
  ...
  {
    //assert(count > 0);
    return key_records[0].key;
  }

  const char *LastKey() const
  {
    //assert(count > 0);
    return key_records[count - 1].key;
  }
  ...
  const char *Key(int index) const
  {
    //assert(count > 0);
    //assert(index >= 0);
    return key_records[index].key;
  }

  const char *FirstValue_v1() const
  {
    //assert(count > 0);
    return v1_records[0].value_1;
  }
  const char *FirstValue_v2() const
  {
    //assert(count > 0);
    return v2_records[0].value_2;
  }
  const char *FirstValue_v3() const
  {
    //assert(count > 0);
    return v3_records[0].value_3;
  }

  const char *FirstValue_v4() const
  {
    //assert(count > 0);
    return v4_records[0].value_4;
  }
  const char *LastValue_v1() const
  {
    //assert(count > 0);
    return v1_records[count - 1].value_1;
  }
  const char *LastValue_v2() const
  {
    //assert(count > 0);
    return v2_records[count - 1].value_2;
  }

  const char *LastValue_v3() const
  {
    //assert(count > 0);
    return v3_records[count - 1].value_3;
  }

  const char *LastValue_v4() const
  {
    //assert(count > 0);
    return v4_records[count - 1].value_4;
  }
  const char *Value_v1(int index) const
  {
    //assert(count > 0);
    return v1_records[index].value_1;
  }
  const char *Value_v2(int index) const
  {
    //assert(count > 0);
    return v2_records[index].value_2;
  }
  const char *Value_v3(int index) const
  {
    //assert(count > 0);
    return v3_records[index].value_3;
  }
  const char *Value_v4(int index) const
  {
    //assert(count > 0);
    return v4_records[index].value_4;
  }
  ...
  off_t GetSmallestLSN()
  {
    ...
    for (int i = 0; i < count; i++)
    {
      if (key_records[i].cur_lsn < smallest || smallest == -1)
      {
        smallest = key_records[i].cur_lsn;
      }
    }
    return smallest;
  }
  ...
  off_t GetBiggestLSN()
  {
    ...
    for (int i = 0; i < count; i++)
    {
      if (key_records[i].cur_lsn > biggest || biggest == -1)
      {
        biggest = key_records[i].cur_lsn;
      }
    }
    return biggest;
  }

  void UpdateValue_v1(int index, const char *v1, const char *v2, const char *v3, const char *v4, off_t lsn)
  {
    //assert(index >= 0);
    v1_records[index].UpdateValue(v1);
  }
  ...
  void DeleteKVAtIndex(int index)
  {
    //assert(index >= 0);
    //assert(index < kOrder_stock_);
    std::memmove(&key_records[index], &key_records[index + 1],
                 sizeof(key_records[0]) * (--count - index));
    std::memmove(&v1_records[index], &v1_records[index + 1],
                 sizeof(v1_records[0]) * (--count - index));                 
    std::memmove(&v2_records[index], &v2_records[index + 1],
                 sizeof(v2_records[0]) * (--count - index));                 
    std::memmove(&v3_records[index], &v3_records[index + 1],
                 sizeof(v3_records[0]) * (--count - index));                 
    std::memmove(&v4_records[index], &v4_records[index + 1],
                 sizeof(v4_records[0]) * (--count - index));                 
  }
  ...
  off_t offset_v3;
  off_t offset_v4;
  BPlusStockTree::PAXKeyRecord key_records[kOrder_stock_];
  BPlusStockTree::PAXColumn1Record v1_records[kOrder_stock_];
  BPlusStockTree::PAXColumn2Record v2_records[kOrder_stock_];
  BPlusStockTree::PAXColumn3Record v3_records[kOrder_stock_];
  BPlusStockTree::PAXColumn4Record v4_records[kOrder_stock_];
  // BPlusStockTree::Record Per_page_logs[kOrder_stock_/2];
  // off_t cur_pplog_index;
};
*/
...
struct BPlusStockTree::Plog : BPlusStockTree::Node
{
  Plog() = default;
  ~Plog() = default;
  ...
  //hkc-plog-7-1
  void UpdateKVDelta(int index, const char *k, const char *v_quality, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateDeltaKV(k, v_quality, lsn);
  }
  ...
  {
    //assert(index >= 0);
    //assert(index < kOrder_stock_Plog);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateKV(index, k,v1,v2,v3,v4, v5,v6, lsn);
  }
  ...
  void InsertDeltaKVAtIndex(int index, const char *k, const char *v_quality,off_t lsn)
  {
    ...
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateKVDelta(index, k,v_quality,lsn);
  }
  ...
  // BPlusStockTree::Record records[kOrder_stock_/2];
  BPlusStockTree::Record records[kOrder_stock_Plog];
};

class BPlusStockTree::BlockCache
{
  struct Node;
...
public:
  BlockCache(bool enable_backthd)
  {
    ...
    // pre_allocate
    // 2. init free list
    char *init_ptr = (char *)malloc(kMaxFreePages_stock_ * kLeafPageSize); // i.e., for each page
    memset(init_ptr, 0, kMaxFreePages_stock_ * kLeafPageSize);

    char *flag_addr = init_ptr;
    for (int i = 0; i < kMaxFreePages_stock_; i++)
    {
...
void RowToColumn_Cache(char* initial_page, char* converted_page){
  ...
  */
  ...
  row_leaf->offset_v2 = (4+12+4+8)*1024;
  row_leaf->offset_v3 = (4+12+4+8+60)*1024;
  row_leaf->offset_v4 = (4+12+4+8+60+4)*1024;
  row_leaf->offset_v5 = (4+12+4+8+60+4+8)*1024;
  row_leaf->offset_v6 = (4+12+4+8+60+4+8+8)*1024;
  ...
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&buf[k_of],&(row_leaf->records[i].key),kMaxKeySize_stock_);
    k_of+=kMaxKeySize_stock_;
    memcpy(&buf[lsn_of],&(row_leaf->records[i].cur_lsn),sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&buf[v1_of],&(row_leaf->records[i].value_1),kMaxValueSize_stock_col1_);
    v1_of+=kMaxValueSize_stock_col1_;
    memcpy(&buf[v2_of],&(row_leaf->records[i].value_2),kMaxValueSize_stock_col2_);
    v2_of+=kMaxValueSize_stock_col2_;
    memcpy(&buf[v3_of],&(row_leaf->records[i].value_3),kMaxValueSize_stock_col3_);
    v3_of+=kMaxValueSize_stock_col3_;
    memcpy(&buf[v4_of],&(row_leaf->records[i].value_4),kMaxValueSize_stock_col4_);
    v4_of+=kMaxValueSize_stock_col4_;
    memcpy(&buf[v5_of],&(row_leaf->records[i].value_5),kMaxValueSize_stock_col5_);
    v5_of+=kMaxValueSize_stock_col5_;
    memcpy(&buf[v6_of],&(row_leaf->records[i].value_6),kMaxValueSize_stock_col6_);
    v6_of+=kMaxValueSize_stock_col6_;
  }
...
void ColumnToRow_Cache(char* column_page, char* row_page){
  ...
  */
  ...
  //convert row as column and fit in sub pages
    ...
    /*
      const int kMaxKeySize_stock_ = 16; 16K
      const int kMaxValueSize_stock_col1_ = 16; 16K
      const int kMaxValueSize_stock_col2_ = 24; 24K
      const int kMaxValueSize_stock_col3_ = 8; 8K
      const int kMaxValueSize_stock_col4_ = 50; 50K
    */
  ...
  off_t v6_of = row_leaf->offset_v6;
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&(row_leaf->records[i].key),&buf[k_of],kMaxKeySize_stock_);
    k_of+=kMaxKeySize_stock_;
    memcpy(&(row_leaf->records[i].cur_lsn),&buf[lsn_of],sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&(row_leaf->records[i].value_1),&buf[v1_of],kMaxValueSize_stock_col1_);
    v1_of+=kMaxValueSize_stock_col1_;
    memcpy(&(row_leaf->records[i].value_2),&buf[v2_of],kMaxValueSize_stock_col2_);
    v2_of+=kMaxValueSize_stock_col2_;
    memcpy(&(row_leaf->records[i].value_3),&buf[v3_of],kMaxValueSize_stock_col3_);
    v3_of+=kMaxValueSize_stock_col3_;
    memcpy(&(row_leaf->records[i].value_4),&buf[v4_of],kMaxValueSize_stock_col4_);
    v4_of+=kMaxValueSize_stock_col4_;
    memcpy(&(row_leaf->records[i].value_5),&buf[v5_of],kMaxValueSize_stock_col5_);
    v5_of+=kMaxValueSize_stock_col5_;
    memcpy(&(row_leaf->records[i].value_6),&buf[v6_of],kMaxValueSize_stock_col6_);
    v6_of+=kMaxValueSize_stock_col6_;
  }
  ...
  void InPlaceInsertToFLU(off_t offset_leaf, off_t offset_split)
  {
    ...
    if (mtr_cache->InFLUOrNot.count(offset_leaf) == 0)
    {
      //--+printf("error_1 ! FLU re- direct insert");
    }

    Node *node_leaf = nullptr;
    if (offset2node_leaf.count(offset_leaf) == 0)
    {
    ...
    if (offset2node_leaf.count(offset_split) == 0)
    {
  ...
  void ReinsertToFLU(off_t offset, off_t ori_oldset_lsn)
  {
    ...
    Node *node = nullptr;
    if (offset2node_leaf.count(offset) == 0)
    {
  ...
  void Put(T *block, int is_plog)
  {
    ...
    if (async_mode_stock_ == 0)
    {
      if (size_lru > kMaxPages_stock_stock * 0.9)
      {
        KickLRUbyUring();
      }
      if (mtr_cache->plog_flag != 1 && mtr_cache->FLU_disabled == 0 && size_flu > kMaxPages_stock_stock * 0.7)
      {
        KickLFUbyUring();
      }
    }
  ...
  void SetMtr(BPlusStockTree::Monitor *mtr_1){
    mtr_cache = mtr_1;
  }
  int Check_Leaf(off_t offset)
  {
    if (offset2node_leaf.count(offset))
    {
      return 1;
    }
    else
  ...
  T *Get(int fd, off_t offset, int is_plog, char *plog_copy)
  {
    ...
    if (is_plog == 1)
    {
      ...
      if (mtr_cache->Virtual_Page_In_Cache.count(offset))
      {
    ...
    for (i = 0; i < Queue_depth; i++)
    {
      ...
      if(debug_flag==1){
        ...
        if( (leaf->count!=row_leaf_test->count) || 
          ...
          int x=1; printf("Convert #1 error occur!!!\n");
        }
        for(int i =0; i< leaf->count;i++){
          if(
            ...
            int x=1; printf("Convert #2 error occur!!!\n");
          }
        }
        free(row);
      }
  ...
  void KickLRUbyUring()
  {
    ...
    if(mtr_cache->plog_flag == 1){
      ...
      for (int i = 0; i < 200; i++){
        ...
        ptr = ptr->prev_lru;
        if(mtr_cache->pendingWrites_In_Glog.count(tmp->offset)>0){
          continue;
        }
       ...
       // if(tmp->offset == 1694560256){
        //  int x=0;
       // }
        if (mtr_cache->Virtual_Page_In_Cache.count(tmp->offset)>0 )
        {
    ...
    for (i = 0; i < Queue_depth; i++)
    {
      ...
      ptr = ptr->prev_lru;
      DeletePersistedNode(tmp);
      if (mtr_cache->plog_flag!=1 && mtr_cache->InFLUOrNot.count(tmp->offset)>0 )
      {
...
class BPlusStockTree::InternalCache
{
  struct Node;
  ...
  void SetMtr( BPlusStockTree::Monitor *mtr_2){
    mtr_internal = mtr_2;
  }
  ...
  void Put(T *block)
  {
    ...
    if (offset2node_Index.count(block->offset) == 0)
    {
  ...
  {
    // only for pages that have already been written to data file
    if (offset2node_Index.count(offset) == 0)
    {
       // never happend
...
//hkc-ap-10*
void BPlusStockTree::ScanAP_impl(off_t &left, off_t &right, std::string &value){
...
//hkc-ap-11
//BPlusStockTree::BPlusStockTree(const char *path, const char *log_path, const char *chpt_path, const int bg_id_, const bool enable_backthd, const char *shadow_plog_path)
    ...
    : fd_(open(path, O_CREAT|O_DIRECT|O_RDWR)),fd_shadow_plog_(open(shadow_plog_path, O_CREAT|O_DIRECT|O_RDWR)),log_fd_(open(log_path, O_CREAT|O_APPEND|O_RDWR)),chpt_fd_(open(chpt_path, O_CREAT|O_APPEND|O_RDWR)), block_cache_(new BlockCache(enable_backthd)), internal_cache_(new InternalCache())
{
  ...
  if (fd_ == -1)
    //Exit("open db");
    printf("open stock db failed.\n");
  if (log_fd_ == -1)
    //Exit("open log");
    printf("open stock log failed.\n");
  if (chpt_fd_ == -1)
    //Exit("open chpt");
    printf("open stock chpt failed.\n");
  ...
  // meta_ = Map<Meta>(kMetaOffset_stock_);
  meta_ = new Meta();
  ...
  {
    // Initialize B+tree;
    constexpr off_t of_root = kMetaOffset_stock_ + kUnitNodeSize;
    meta_->block = of_root;
    ...
    //hkc-ap-14
  if(scan_ap){
    std::thread thd_ap = std::thread(&BPlusStockTree::APWorker,this);
    thd_ap.detach();
  }
}

 BPlusStockTree::~ BPlusStockTree()
{
  UnMap(meta_, 0, 0);
  delete block_cache_;
  close(fd_);
...
//hkc-ap-15
int BPlusStockTree::PrepareReadSetAP(off_t left, off_t right, void *scan_ptr){
...
void BPlusStockTree::GetStatic(int &hits_bp, int &hits_glog, int &hits_plog, int &disk_read, int &io_consume, int &error_cnt, int &dirtys, int &cleans){
...
off_t BPlusStockTree::GetLSN(){
  return mtr_->global_lsn;
}
void BPlusStockTree::IncreLSN(){
  mtr_->global_lsn++;
  return;
}

void BPlusStockTree::ResetStatic(){
    mtr_->hits_bp = 0; 
    mtr_->hits_glog = 0; 
    mtr_->hits_plog =0;
    mtr_->disk_read=0;
...
void BPlusStockTree::SetBaseline(){
  mtr_->FLU_disabled = 0;
  mtr_->plog_flag =-1;
  mtr_->warmup_flag =0;
}

void BPlusStockTree::SetIOStaic(){
    mtr_->plog_insert=0;
    mtr_->leaf_insert=0;
    mtr_->leaf_split=0;
...
off_t BPlusStockTree::GetChpt(){
  return mtr_->normal_chpt_now;
}
void BPlusStockTree::SetLoadFlag(int flag){
  mtr_->load_flag = flag;
}
void BPlusStockTree::DisableFLUOrNot(int flag){
  mtr_->FLU_disabled = flag;
}
void BPlusStockTree::LoadIOStatic(int &plog_insert,int &leaf_insert,int &leaf_split,int &phy_plog_write,int &phy_plog_read,int &phy_page_read,int &phy_page_write,int &logi_page_read,int &logi_page_write){
...
int BPlusStockTree::ApplyPendPages(std::map<off_t, std::pair<char*,char*>>* source, std::map<off_t, std::pair<char*,char*>>* target, std::map<off_t,std::pair<int,int>> *EmptyPages){
...
/*
void BPlusStockTree::BackGround_GLog_Func()
{
  int total_KV=0;
  int final_KV=0;
  int total_Plog =0;
  ...
  for(std::map<off_t, std::pair<std::string,std::string>>::iterator it = pending_Seg->begin(); it!=pending_Seg->end();it++){    
      ...
      if(now_lsn > cur_lsn){
        ...
        std::string value = it->second.second;
          ...
          std::string value2 = value.substr(16,240);
          std::string value3 = value.substr(256,8);
          std::string value4 = value.substr(264,16);
          std::string value5 = value.substr(280,16);
          std::string value6 = value.substr(296,50);

        if (InsertKVIntoPlog(plog, key.data(), value1.data(),value2.data(),value3.data(),value4.data(),value5.data(),value6.data(), cur_lsn) <=
          GetMaxKeys(0)){
          mtr_->plog_insert += 1;
          ...
          for (int i = 0; i < kOrder_stock_Plog; i++){
      ...
      if(now_lsn > cur_lsn){
        ...
        bool overflow =0;
          ...
          if(value[0]=='U'){
            ...
            std::string value_quality = value.substr(2,16);
            if(InsertDeltaIntoPlog(plog,key.data(),value_quality.data(),cur_lsn)>GetMaxKeys(0)){
              overflow = true;
              mtr_->plog_insert += 1;
            }else{
              mtr_->plog_insert += 1;
            ...
            if (InsertKVIntoPlog(plog, key.data(), 
                  value1.data(),value2.data(),value3.data(),value4.data(),value5.data(),value6.data(),cur_lsn) > GetMaxKeys(0)){
                mtr_->plog_insert += 1;
                overflow == true;
            }else{
                mtr_->plog_insert += 1;
...
void BPlusStockTree::RowToColumn(char* initial_page, char* converted_page){
...
void BPlusStockTree::ColumnToRow(char* column_page, char* row_page){
  memset(row_page,0,kLeafPageSize);
  LeafNode* row_leaf = reinterpret_cast<LeafNode*>(row_page);
  /*
  const int kMaxKeySize_stock_ = 32;
...
//hkc-gb-04
off_t BPlusStockTree::GetOnlyLeafOffset(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
  return of_leaf;
}
//hkc-gb-05
char* BPlusStockTree::GetLeafCopy(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
...
//hkc-gb-06
void BPlusStockTree::UpdateLeafCopy(const std::string &key, const std::string &value, off_t lsn, char* leaf_copy){
  ...
  std::string value5 = value.substr(280,16);
  std::string value6 = value.substr(296,50);
  if (InsertKVIntoLeafNode(leaf, key.data(),  value1.data(),value2.data(),value3.data(),value4.data(),value5.data(),value6.data(),  lsn) <= GetMaxKeys(1)){
...
//hkc-gb-07
void BPlusStockTree::GetRowRecord(const std::string &key, std::string &value, char* leaf_copy) {
  LeafNode* leaf  = reinterpret_cast<LeafNode*>(&leaf_copy[0]);
  int index = GetIndexFromLeafNode(leaf, key.data());
  char* ptr = leaf->ValueALL(index);
  std::string tmp_value(ptr,ptr+kMaxValueSize_stock_col_all);
  value = tmp_value;
  free(ptr);
  return ;
...
//hkc-gb-08
int into_time_stock=0;
void BPlusStockTree::TriggerFlush(const off_t* offsets, const int size, char* evict_buf){
  int batch_size = 200;
  into_time_stock++;
...
void BPlusStockTree::Put(const std::string &key, const std::string &value, off_t lsn)
{
    ...
    if (index != -1)
    {
      ...
      if (now_lsn >= lsn)
      {
        ...
        return;
          ...
          std::string value5 = value.substr(280,16);
          std::string value6 = value.substr(296,50);
  if (InsertKVIntoLeafNode(leaf_node, key.data(), value1.data(),value2.data(),value3.data(),value4.data(), value5.data(),value6.data(), lsn) <=
      GetMaxKeys(1))
  {
    // 2.If records of leaf node less than or equals kOrder_stock_ - 1 then finish.
...
/*
bool BPlusStockTree::Get(const std::string &key, std::string &value) const
{
 ...
 // if(of_leaf == 1694560256){
  ...
  else if(mtr_->Virtual_Page_In_Cache.count(of_leaf) >0){
    mtr_->hits_bp++;
...
bool BPlusStockTree::Scan(const std::string &start, const std::string &end, std::string &value) const
{
  if(start>end){
    int x=1;
  }
...
//hkc-ap-19
bool BPlusStockTree::ScanAP(const std::string &start, const std::string &end, std::string &value) const
{
...
int BPlusStockTree::Check(off_t offset)
{
  return block_cache_->Check_Leaf(offset);
}

char * BPlusStockTree::GetMemCopy(off_t offset)
{
  return block_cache_->GetMemCopy_Leaf(offset);
}

template <typename T>
T * BPlusStockTree::Map(off_t offset, int isleaf, int is_plog, char *&plog_copy) const
{
  if (isleaf == 1)
  {
...
template <typename T>
void BPlusStockTree::InsertDirty(off_t offset) const
{
...
template <typename T>
void BPlusStockTree::RelocateDirty(off_t offset, off_t ori_oldset_lsn) const
{
...
template <typename T>
void BPlusStockTree::DirectInsertDirty(off_t offset_leaf, off_t offset_split) const
{
...
template <typename T>
void BPlusStockTree::UnMap(T *map_obj, int isleaf, int is_plog) const
{
  if (isleaf == 1)
  {
...
size_t BPlusStockTree::GetMinKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return (kOrder_stock_Plog + 1) / 2 - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return (kOrder_stock_ + 1) / 2 - 1;
  }
}
// 1: leaf node ; 0: plog
size_t BPlusStockTree::GetMaxKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return kOrder_stock_Plog - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return kOrder_stock_ - 1;
  }
}
size_t BPlusStockTree::GetMinIndexKeys() { return (kOrder_stock_Index + 1) / 2 - 1; }
size_t BPlusStockTree::GetMaxIndexKeys() { return kOrder_stock_Index - 1; }

 BPlusStockTree::IndexNode * BPlusStockTree::GetOrCreateParent(Node *node)
{
  if (node->parent == 0)
  {
    // Split root node.
...
template <typename T>
int BPlusStockTree::UpperBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
  ...
  {
    int mid = (l + r) >> 1;
    if (std::strncmp(arr[mid].key, key, kMaxKeySize_stock_) <= 0)
    {
      l = mid + 1;
    }
    else
...
template <typename T>
int BPlusStockTree::UpperBoundIndex(T arr[], int n, const char *key) const
{
...
template <typename T>
int BPlusStockTree::LowerBoundIndex(T arr[], int n, const char *key) const
{
  ////assert(n <= GetMaxIndexKeys());
  int l = 0, r = n - 1;
  while (l <= r)
...
// flag: 1: data page; 0: plog
template <typename T>
int BPlusStockTree::LowerBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
    //assert(n <= GetMaxKeys(0));
...
template <typename T>
void BPlusStockTree::Dealloc(T *node)
{
  UnMap<T>(node);
}

void BPlusStockTree::WarmUpVirtualPageCache(int x) const
{
  return block_cache_->WarmUpVPCache(x);
}
off_t BPlusStockTree::GetLeafOffset(const char *key) const
{
...
inline size_t BPlusStockTree::InsertKeyIntoIndexNode(IndexNode *index_node,
                                                const char *key,
                                                Node *left_node,
...
//hkc-modify
size_t BPlusStockTree::InsertKVIntoLeafNode(LeafNode *leaf_node, const char *key,
                                       const char *value_1, const char *value_2, const char *value_3, const char *value_4, const char *value_5, const char *value_6, off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(leaf_node->Key(index - 1), key, kMaxKeySize_stock_) == 0)
  {
    leaf_node->UpdateValue(index - 1, value_1,value_2,value_3,value_4,value_5,value_6, cur_lsn);
    return leaf_node->count;
  }
  leaf_node->InsertKVAtIndex(index, key, value_1,value_2,value_3,value_4,value_5,value_6, cur_lsn);
  ++meta_->size;
  return leaf_node->count;
}

//hkc-plog-9
size_t BPlusStockTree::InsertDeltaKVIntoLeafNode(LeafNode *leaf_node, const char *key,const char* value_quality, off_t cur_lsn)
{
  //assert(leaf_node->count <= GetMaxKeys(1));
  ...
  if (index > 0 &&
      std::strncmp(leaf_node->Key(index - 1), key, kMaxKeySize_stock_) == 0)
  {
    leaf_node->UpdateValueDelta(index - 1, value_quality, cur_lsn);
    return leaf_node->count;
  }
  leaf_node->InsertDeltaKVAtIndex(index, key,  value_quality,cur_lsn);
  ++meta_->size;
  return leaf_node->count;
}
...
size_t BPlusStockTree::InsertKVIntoPlog(Plog *plog, const char *key,
                                   const char* value1, const char* value2,const char* value3,const char* value4, const char* value5,const char* value6, off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(plog->Key(index - 1), key, kMaxKeySize_stock_) == 0)
  {
    plog->UpdateValue(index - 1, value1,value2,value3,value4,value5,value6, cur_lsn);
    return plog->count;
  }
  plog->InsertKVAtIndex(index, key, value1,value2,value3,value4,value5,value6, cur_lsn);
  ++meta_->size;
  return plog->count;
}

//hkc-plog-10
size_t BPlusStockTree::InsertDeltaIntoPlog(Plog *plog, const char *key, const char* value_quality,off_t cur_lsn)
{
  //assert(plog->count <= GetMaxKeys(0));
  ...
  if (index > 0 &&
      std::strncmp(plog->Key(index - 1), key, kMaxKeySize_stock_) == 0)
  {
    plog->UpdateValueDelta(index - 1, value_quality, cur_lsn);
    return plog->count;
  }
  //KV NOT EXIST IN THE PLOG, SO WE PRETEND TO HAVE ALL KVS
  plog->InsertDeltaKVAtIndex(index, key, value_quality,cur_lsn);
  ++meta_->size;
  return plog->count;
}

 BPlusStockTree::LeafNode * BPlusStockTree::SplitLeafNode(LeafNode *leaf_node)
{
  //assert(leaf_node->count == kOrder_stock_);
  off_t origian_lsn = leaf_node->GetSmallestLSN();
  constexpr int mid = (kOrder_stock_ - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_stock_ - mid;
  ...
  if (mtr_->lazy_chpt_lsn_flag == 1)
  {
    ...
    if (mtr_->FLU_disabled == 0 && mtr_->InFLUOrNot.count(split_node->offset) == 0)
    {
 ...
 BPlusStockTree::IndexNode * BPlusStockTree::SplitIndexNode(IndexNode *index_node)
{
  //assert(index_node->count == kOrder_stock_Index);
  constexpr int mid = (kOrder_stock_Index - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_stock_Index - mid - 1;
  ...
  for (int i = mid + 1; i <= kOrder_stock_Index; ++i)
  {
    off_t of_child = index_node->indexes[i].offset;

    if (mtr_->IndexOrNot.count(of_child)>0)
    {
      char *p = nullptr;
...
inline int BPlusStockTree::GetIndexFromLeafNode(LeafNode *leaf_node,
                                           const char *key) const
{
  int index = LowerBound(leaf_node->records, leaf_node->count, key, 1);
  return index < static_cast<int>(leaf_node->count) &&
                 std::strncmp(leaf_node->Key(index), key, kMaxKeySize_stock_) == 0
             ? index
             : -1;
}

inline int BPlusStockTree::GetIndexFromPlog(Plog *plog,
                                       const char *key) const
{
  int index = LowerBound(plog->records, plog->count, key, 0);
  return index < static_cast<int>(plog->count) &&
                 std::strncmp(plog->Key(index), key, kMaxKeySize_stock_) == 0
             ? index
             : -1;
}

std::vector<std::pair<std::string, std::string>> BPlusStockTree::GetRange(
    const std::string &left_key, const std::string &right_key) const
{
  ...
  while (of_leaf != 0 && !finish)
  {
    ...
    {
      if (strncmp(right_leaf_node->Key(i), right_key.data(), kMaxKeySize_stock_) <=
          0)
      {
...
bool BPlusStockTree::Empty() const { return meta_->size == 0; }

size_t BPlusStockTree::Size() const { return meta_->size; }