...
// switch
const int async_mode_neworders_ = 0; // 0 for sync IO; 1 for async IO
const int kAIOBatchSize_item = 20;
const int kOrder_neworders_ = 1500; //how many record a page can contain
const int kOrder_neworders_Plog = kOrder_neworders_ / 10;
// const int kOrder_neworders_ = 50; //for key
const int kOrder_neworders_Index = 3 * 500; // for index

static_assert(kOrder_neworders_ >= 3,
              "The order of B+Tree should be greater than or equal to 3.");
const int kMaxKeySize_neworders_ = 48;
const int kMaxLSNSize_neworders_ = 8;
// const int kMaxValueSize_neworders_ = 256;
const int kMaxValueSize_neworders_col1_ = 4;
//const int kMaxValueSize_neworders_col2_ = 24;
//const int kMaxValueSize_neworders_col3_ = 8;
//const int kMaxValueSize_neworders_col4_ = 50;
const int kMaxValueSize_neworders_col_all = 4;
const off_t kMetaOffset_neworders_ = 0;

int kMaxPages_neworders_item = 500;     // 100M
const int kMaxFreePages_neworders_ = 600; 
// const int kMaxPages_neworders_item = 80000; //1G

typedef char Key[kMaxKeySize_neworders_];
typedef char Value1[kMaxValueSize_neworders_col1_];
//typedef char Value2[kMaxValueSize_neworders_col2_];
//typedef char Value3[kMaxValueSize_neworders_col3_];
//typedef char Value4[kMaxValueSize_neworders_col4_];
typedef char Value_all[kMaxValueSize_neworders_col_all]; //for log record
...
const int innerKeyoffset_neworders_ = 0;
const int innercoloffset_neworders_ = 0;

const int kMaxUniqueKInGLogSeg=3000;
  


struct BPlusNewOrdersTree::Monitor{
  Monitor(){
    bg_id =0;
    plog_insert = 0;
    leaf_insert = 0;
...
struct BPlusNewOrdersTree::Meta
{
...
struct BPlusNewOrdersTree::Index
{
  ...
  void UpdateIndex(off_t of, off_t lsn, const char *k)
  {
    offset = of;
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_neworders_);
  }
...
struct BPlusNewOrdersTree::Record
{
  Key key;
  Value1 value_1;
  off_t cur_lsn;
  ...
  void UpdateKV(const char *k, const char *v1, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_neworders_);
    strncpy(value_1, v1, kMaxValueSize_neworders_col1_);
  }
  void UpdateKey(const char *k, off_t lsn)
  {
    cur_lsn = lsn;
  ...
  void UpdateValue(const char *v1, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(value_1, v1, kMaxValueSize_neworders_col1_);
  }
...
struct BPlusNewOrdersTree::PAXKeyRecord
{
  Key key;
  off_t cur_lsn;
...
struct BPlusNewOrdersTree::PAXColumn1Record
{
  Value1 value_1;
...
struct BPlusNewOrdersTree::Per_page_Log
{
  Key key;
  Value_all value_all;
  off_t cur_lsn;
  ...
  void UpdateKV(const char *k, const char *v, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_neworders_);
    strncpy(value_all, v, kMaxValueSize_neworders_col_all);
  }
  void UpdateKey(const char *k, off_t lsn)
  {
    cur_lsn = lsn;
...
struct BPlusNewOrdersTree::LogRecord
{
  LogRecord() : page_offset(0), log_lsn(0), log_offset(0), next_log_offset(0), page_index(0)
  {
    memset(key, 0, kMaxKeySize_neworders_);
    memset(value, 0, kMaxValueSize_neworders_col_all);
  }
  ...
  char key[kMaxKeySize_neworders_];
  char value[kMaxValueSize_neworders_col_all];

  void UpdateKV(const char *k, const char *v)
  {
    strncpy(key, k, kMaxKeySize_neworders_);
    strncpy(value, v, kMaxValueSize_neworders_col_all);
  }
  void UpdateKey(const char *k) { strncpy(key, k, kMaxKeySize_neworders_); }
  void UpdateValue(const char *v) { strncpy(value, v, kMaxValueSize_neworders_col_all); }
};

struct BPlusNewOrdersTree::Checkpoint
{
...
struct BPlusNewOrdersTree::Node
{
...
struct BPlusNewOrdersTree::IndexNode : BPlusNewOrdersTree::Node
{
  IndexNode() = default;
  ~IndexNode() = default;
  ...
  const char *Key(int index) const
  {
    //assert(count > 0);
    //assert(index >= 0);
    //assert(index <= kOrder_neworders_Index);
  ...
  Index indexes[kOrder_neworders_Index + 1];
};

struct BPlusNewOrdersTree::LeafNode : BPlusNewOrdersTree::Node
{
  LeafNode() = default;
  ~LeafNode() = default;
  ...
  {
    //assert(count > 0);
    char* all = (char*)malloc(sizeof(char)*kMaxValueSize_neworders_col_all);
    int off =0;
    memcpy(&all[off],records[index].value_1,kMaxValueSize_neworders_col1_);
    off+=kMaxValueSize_neworders_col1_;
    return all;
  }
  ...
  void UpdateValue(int index, const char *v1,  off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateValue(v1, lsn);
  }
  ...
  void UpdateKV(int index, const char *k, const char *v1, off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateKV(k, v1, lsn);
  }

  void AppendKV(int index, const char *k, const char *v1,off_t lsn)
  {
    //assert(index >= 0);
    records[index].UpdateKV(k, v1,lsn);
  }

  void InsertKVAtIndex(int index, const char *k, const char *v1, off_t lsn)
  {
    //assert(index >= 0);
    //assert(index < kOrder_neworders_);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateKV(index, k, v1, lsn);
  }
  ...
  off_t offset_lsn;
  off_t offset_v1;
  BPlusNewOrdersTree::Record records[kOrder_neworders_];
  // BPlusNewOrdersTree::Record Per_page_logs[kOrder_neworders_/2];
  // off_t cur_pplog_index;
};



struct BPlusNewOrdersTree::Plog : BPlusNewOrdersTree::Node
{
  Plog() = default;
  ~Plog() = default;
  ...
  {
    //assert(index >= 0);
    //assert(index < kOrder_neworders_Plog);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateKV(index, k,v1,  lsn);
  }
  ...
  // BPlusNewOrdersTree::Record records[kOrder_neworders_/2];
  BPlusNewOrdersTree::Record records[kOrder_neworders_Plog];
};

class BPlusNewOrdersTree::BlockCache
{
  struct Node;
...
public:
  BlockCache(bool enable_backthd)
  {
    ...
    // pre_allocate
    // 2. init free list
    char *init_ptr = (char *)malloc(kMaxFreePages_neworders_ * kLeafPageSize); // i.e., for each page
    memset(init_ptr, 0, kMaxFreePages_neworders_ * kLeafPageSize);

    char *flag_addr = init_ptr;
    for (int i = 0; i < kMaxFreePages_neworders_; i++)
    {
...
void RowToColumn_Cache(char* initial_page, char* converted_page){
  ...
  /*
  const int kMaxKeySize_neworders_ = 16;
  const int kMaxLSNSize_neworders_ = 8;
  const int kMaxValueSize_neworders_col1_ = 16;
  const int kMaxValueSize_neworders_col2_ = 24;
  const int kMaxValueSize_neworders_col3_ = 8;
  const int kMaxValueSize_neworders_col4_ = 50;
  */
  ...
  //convert row as column and fit in sub pages
    ...
    /*
      const int kMaxKeySize_neworders_ = 16; 16K
      const int kMaxValueSize_neworders_col1_ = 16; 16K
      const int kMaxValueSize_neworders_col2_ = 24; 24K
      const int kMaxValueSize_neworders_col3_ = 8; 8K
      const int kMaxValueSize_neworders_col4_ = 50; 50K
    */
  ...
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&buf[k_of],&(row_leaf->records[i].key),kMaxKeySize_neworders_);
    k_of+=kMaxKeySize_neworders_;
    memcpy(&buf[lsn_of],&(row_leaf->records[i].cur_lsn),sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&buf[v1_of],&(row_leaf->records[i].value_1),kMaxValueSize_neworders_col1_);
    v1_of+=kMaxValueSize_neworders_col1_;
  }
...
void ColumnToRow_Cache(char* column_page, char* row_page){
  ...
  */
  ...
  off_t v1_of = row_leaf->offset_v1;
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&(row_leaf->records[i].key),&buf[k_of],kMaxKeySize_neworders_);
    k_of+=kMaxKeySize_neworders_;
    memcpy(&(row_leaf->records[i].cur_lsn),&buf[lsn_of],sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&(row_leaf->records[i].value_1),&buf[v1_of],kMaxValueSize_neworders_col1_);
    v1_of+=kMaxValueSize_neworders_col1_;
  }
  ...
  void Put(T *block, int is_plog)
  {
    ...
    if (async_mode_neworders_ == 0)
    {
      if (size_lru > kMaxPages_neworders_item * 0.9)
      {
        KickLRUbyUring();
      }
      if (mtr_cache->plog_flag != 1 && mtr_cache->FLU_disabled == 0 && size_flu > kMaxPages_neworders_item * 0.5)
      {
        KickLFUbyUring();
      }
    }
  ...
  void SetMtr(BPlusNewOrdersTree::Monitor *mtr_1){
    mtr_cache = mtr_1;
  }
...
class BPlusNewOrdersTree::InternalCache
{
  struct Node;
...
public:
  ...
  void SetMtr(BPlusNewOrdersTree::Monitor *mtr_2){
    mtr_internal = mtr_2;
  }
...
//hkc-ap-10*
void BPlusNewOrdersTree::ScanAP_impl(off_t &left, off_t &right, std::string &value){
...
//hkc-ap-10
void BPlusNewOrdersTree::APWorker(){
  sleep(20);
  while(true){
...
BPlusNewOrdersTree::BPlusNewOrdersTree(const char *path, const char *log_path, const char *chpt_path, const int bg_id_, const bool enable_backthd, const char *shadow_plog_path)
    : fd_(open(path, O_CREAT|O_DIRECT|O_RDWR)),fd_shadow_plog_(open(shadow_plog_path, O_CREAT|O_DIRECT|O_RDWR)),log_fd_(open(log_path, O_CREAT|O_APPEND|O_RDWR)),chpt_fd_(open(chpt_path, O_CREAT|O_APPEND|O_RDWR)), block_cache_(new BlockCache(enable_backthd)), internal_cache_(new InternalCache())
{
  ...
  if (fd_ == -1)
    //Exit("open db");
    printf("open neworders db failed.\n");
  if (log_fd_ == -1)
    //Exit("open log");
    printf("open neworders log failed.\n");
  if (chpt_fd_ == -1)
    //Exit("open chpt");
    printf("open neworders chpt failed.\n");
  ...
  // meta_ = Map<Meta>(kMetaOffset_neworders_);
  meta_ = new Meta();
  ...
  {
    // Initialize B+tree;
    constexpr off_t of_root = kMetaOffset_neworders_ + kUnitNodeSize;
    meta_->block = of_root;
    ...
    //hkc-ap-14
  if(scan_ap){
    std::thread thd_ap = std::thread(&BPlusNewOrdersTree::APWorker,this);
    thd_ap.detach();
  }
}

BPlusNewOrdersTree::~BPlusNewOrdersTree()
{
  UnMap(meta_, 0, 0);
  delete block_cache_;
  close(fd_);
...
//hkc-ap-15
int BPlusNewOrdersTree::PrepareReadSetAP(off_t left, off_t right, void *scan_ptr){
...
void BPlusNewOrdersTree::SetPlog(){
  mtr_->plog_flag =1;
  mtr_->FLU_disabled =1;
  mtr_->warmup_flag =1;
  return;
...
void BPlusNewOrdersTree::GetStatic(int &hits_bp, int &hits_glog, int &hits_plog, int &disk_read, int &io_consume, int &error_cnt, int &dirtys, int &cleans){
...
off_t BPlusNewOrdersTree::GetLSN(){
  return mtr_->global_lsn;
}
void BPlusNewOrdersTree::IncreLSN(){
  mtr_->global_lsn++;
  return;
}

void BPlusNewOrdersTree::ResetStatic(){
    mtr_->hits_bp = 0; 
    mtr_->hits_glog = 0; 
    mtr_->hits_plog =0;
    mtr_->disk_read=0;
...
void BPlusNewOrdersTree::SetBaseline(){
  mtr_->FLU_disabled = 0;
  mtr_->plog_flag =-1;
  mtr_->warmup_flag =0;
}

void BPlusNewOrdersTree::SetIOStaic(){
    mtr_->plog_insert=0;
    mtr_->leaf_insert=0;
    mtr_->leaf_split=0;
...
off_t BPlusNewOrdersTree::GetChpt(){
  return mtr_->normal_chpt_now;
}
void BPlusNewOrdersTree::SetLoadFlag(int flag){
  mtr_->load_flag = flag;
}
void BPlusNewOrdersTree::DisableFLUOrNot(int flag){
  mtr_->FLU_disabled = flag;
}
void BPlusNewOrdersTree::LoadIOStatic(int &plog_insert,int &leaf_insert,int &leaf_split,int &phy_plog_write,int &phy_plog_read,int &phy_page_read,int &phy_page_write,int &logi_page_read,int &logi_page_write){
...
int BPlusNewOrdersTree::ApplyPendPages(std::map<off_t, std::pair<char*,char*>>* source, std::map<off_t, std::pair<char*,char*>>* target, std::map<off_t,std::pair<int,int>> *EmptyPages){
...
void BPlusNewOrdersTree::BackGround_GLog_Func()
{
  int total_KV=0;
  int final_KV=0;
  int total_Plog =0;
  ...
  for(std::map<off_t, std::pair<std::string,std::string>>::iterator it = pending_Seg->begin(); it!=pending_Seg->end();it++){    
      ...
      if(now_lsn > cur_lsn){
        ...
        std::string value = it->second.second;
          ...
          std::string value1 = value.substr(0,4);
        if (InsertKVIntoPlog(plog, key.data(), value1.data(),cur_lsn) <=
          GetMaxKeys(0)){
          mtr_->plog_insert += 1;
          /*
          if (Check(of_leaf) == 1){
          ...
          */
          ...
          for (int i = 0; i < kOrder_neworders_Plog; i++){
...
void BPlusNewOrdersTree::RowToColumn(char* initial_page, char* converted_page){
...
void BPlusNewOrdersTree::ColumnToRow(char* column_page, char* row_page){
...
//hkc-gb-04
off_t BPlusNewOrdersTree::GetOnlyLeafOffset(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
  return of_leaf;
}
//hkc-gb-05
char* BPlusNewOrdersTree::GetLeafCopy(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
...
//hkc-gb-06
void BPlusNewOrdersTree::UpdateLeafCopy(const std::string &key, const std::string &value, off_t lsn, char* leaf_copy){
  LeafNode* leaf = reinterpret_cast<LeafNode*>(&leaf_copy[0]);
  std::string value1 = value.substr(0,4);
  if (InsertKVIntoLeafNode(leaf, key.data(),  value1.data(), lsn) <= GetMaxKeys(1)){
...
//hkc-gb-07
void BPlusNewOrdersTree::GetRowRecord(const std::string &key, std::string &value, char* leaf_copy) {
  LeafNode* leaf  = reinterpret_cast<LeafNode*>(&leaf_copy[0]);
  int index = GetIndexFromLeafNode(leaf, key.data());
  char* ptr = leaf->ValueALL(index);
  std::string tmp_value(ptr,ptr+kMaxValueSize_neworders_col_all);
  value = tmp_value;
  free(ptr);
  return ;
...
//hkc-gb-08
void BPlusNewOrdersTree::TriggerFlush(const off_t* offsets, const int size, char* evict_buf){
  int batch_size = 200;
...
void BPlusNewOrdersTree::Put(const std::string &key, const std::string &value, off_t lsn)
{
  ...
  std::string value1 = value.substr(0,4);
  if (InsertKVIntoLeafNode(leaf_node, key.data(), value1.data(), lsn) <=
      GetMaxKeys(1))
  {
    // 2.If records of leaf node less than or equals kOrder_neworders_ - 1 then finish.
  ...
  // until count <= kOrder_neworders_ - 1.
  size_t count;
  do
  {
...
/*
bool BPlusNewOrdersTree::Get(const std::string &key, std::string &value) const
{
...
int BPlusNewOrdersTree::Check(off_t offset)
{
...
bool BPlusNewOrdersTree::Scan(const std::string &start, const std::string &end, std::string &value) const
{
  if(start>end){
    int x=1;
  }
...
//hkc-ap-19
bool BPlusNewOrdersTree::ScanAP(const std::string &start, const std::string &end, std::string &value) const
{
...
char *BPlusNewOrdersTree::GetMemCopy(off_t offset)
{
  return block_cache_->GetMemCopy_Leaf(offset);
}

template <typename T>
T *BPlusNewOrdersTree::Map(off_t offset, int isleaf, int is_plog, char *&plog_copy) const
{
  if (isleaf == 1)
  {
...
template <typename T>
void BPlusNewOrdersTree::InsertDirty(off_t offset) const
{
...
template <typename T>
void BPlusNewOrdersTree::RelocateDirty(off_t offset, off_t ori_oldset_lsn) const
{
...
template <typename T>
void BPlusNewOrdersTree::DirectInsertDirty(off_t offset_leaf, off_t offset_split) const
{
...
template <typename T>
void BPlusNewOrdersTree::UnMap(T *map_obj, int isleaf, int is_plog) const
{
  if (isleaf == 1)
  {
...
size_t BPlusNewOrdersTree::GetMinKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return (kOrder_neworders_Plog + 1) / 2 - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return (kOrder_neworders_ + 1) / 2 - 1;
  }
}
// 1: leaf node ; 0: plog
size_t BPlusNewOrdersTree::GetMaxKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return kOrder_neworders_Plog - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return kOrder_neworders_ - 1;
  }
}
size_t BPlusNewOrdersTree::GetMinIndexKeys() { return (kOrder_neworders_Index + 1) / 2 - 1; }
size_t BPlusNewOrdersTree::GetMaxIndexKeys() { return kOrder_neworders_Index - 1; }

BPlusNewOrdersTree::IndexNode *BPlusNewOrdersTree::GetOrCreateParent(Node *node)
{
  if (node->parent == 0)
  {
    // Split root node.
...
template <typename T>
int BPlusNewOrdersTree::UpperBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
  ...
  {
    int mid = (l + r) >> 1;
    if (std::strncmp(arr[mid].key, key, kMaxKeySize_neworders_) <= 0)
    {
      l = mid + 1;
    }
    else
...
template <typename T>
int BPlusNewOrdersTree::UpperBoundIndex(T arr[], int n, const char *key) const
{
...
template <typename T>
int BPlusNewOrdersTree::LowerBoundIndex(T arr[], int n, const char *key) const
{
  ////assert(n <= GetMaxIndexKeys());
  int l = 0, r = n - 1;
  while (l <= r)
...
// flag: 1: data page; 0: plog
template <typename T>
int BPlusNewOrdersTree::LowerBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
    //assert(n <= GetMaxKeys(0));
...
template <typename T>
T *BPlusNewOrdersTree::AllocLeaf()
...
template <typename T>
T *BPlusNewOrdersTree::AllocIndex()
{ // 1: leaf node, 0: internal node
...
template <typename T>
void BPlusNewOrdersTree::Dealloc(T *node)
{
  UnMap<T>(node);
}

void BPlusNewOrdersTree::WarmUpVirtualPageCache(int x) const
{
  return block_cache_->WarmUpVPCache(x);
}
off_t BPlusNewOrdersTree::GetLeafOffset(const char *key) const
{
...
inline size_t BPlusNewOrdersTree::InsertKeyIntoIndexNode(IndexNode *index_node,
                                                const char *key,
                                                Node *left_node,
...
size_t BPlusNewOrdersTree::InsertKVIntoLeafNode(LeafNode *leaf_node, const char *key,
                                       const char *value_1, off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(leaf_node->Key(index - 1), key, kMaxKeySize_neworders_) == 0)
  {
    leaf_node->UpdateValue(index - 1, value_1, cur_lsn);
    return leaf_node->count;
  }
  leaf_node->InsertKVAtIndex(index, key, value_1,cur_lsn);
  ++meta_->size;
  return leaf_node->count;
}

size_t BPlusNewOrdersTree::InsertKVIntoPlog(Plog *plog, const char *key,
                                   const char* value1, off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(plog->Key(index - 1), key, kMaxKeySize_neworders_) == 0)
  {
    plog->UpdateValue(index - 1, value1, cur_lsn);
    return plog->count;
  }
  plog->InsertKVAtIndex(index, key, value1, cur_lsn);
  ++meta_->size;
  return plog->count;
}

BPlusNewOrdersTree::LeafNode *BPlusNewOrdersTree::SplitLeafNode(LeafNode *leaf_node)
{
  //assert(leaf_node->count == kOrder_neworders_);
  off_t origian_lsn = leaf_node->GetSmallestLSN();
  constexpr int mid = (kOrder_neworders_ - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_neworders_ - mid;
...
BPlusNewOrdersTree::IndexNode *BPlusNewOrdersTree::SplitIndexNode(IndexNode *index_node)
{
  //assert(index_node->count == kOrder_neworders_Index);
  constexpr int mid = (kOrder_neworders_Index - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_neworders_Index - mid - 1;
  ...
  for (int i = mid + 1; i <= kOrder_neworders_Index; ++i)
  {
...
inline int BPlusNewOrdersTree::GetIndexFromLeafNode(LeafNode *leaf_node,
                                           const char *key) const
{
  int index = LowerBound(leaf_node->records, leaf_node->count, key, 1);
  return index < static_cast<int>(leaf_node->count) &&
                 std::strncmp(leaf_node->Key(index), key, kMaxKeySize_neworders_) == 0
             ? index
             : -1;
}

inline int BPlusNewOrdersTree::GetIndexFromPlog(Plog *plog,
                                       const char *key) const
{
  int index = LowerBound(plog->records, plog->count, key, 0);
  return index < static_cast<int>(plog->count) &&
                 std::strncmp(plog->Key(index), key, kMaxKeySize_neworders_) == 0
             ? index
             : -1;
}

std::vector<std::pair<std::string, std::string>> BPlusNewOrdersTree::GetRange(
    const std::string &left_key, const std::string &right_key) const
{
  ...
  while (of_leaf != 0 && !finish)
  {
    ...
    {
      if (strncmp(right_leaf_node->Key(i), right_key.data(), kMaxKeySize_neworders_) <=
          0)
      {
...
bool BPlusNewOrdersTree::Empty() const { return meta_->size == 0; }

size_t BPlusNewOrdersTree::Size() const { return meta_->size; }