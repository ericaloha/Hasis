...
// switch
const int async_mode_district_ = 0; // 0 for sync IO; 1 for async IO
const int kAIOBatchSize_item = 20;
const int kOrder_district_ = 240; //how many record a page can contain
const int kOrder_district_Plog = kOrder_district_ / 10;
// const int kOrder_district_ = 50; //for key
const int kOrder_district_Index = 1500; // for index

static_assert(kOrder_district_ >= 3,
              "The order of B+Tree should be greater than or equal to 3.");


const int kMaxKeySize_district_ = 32;
const int kMaxLSNSize_district_ = 8;
// const int kMaxValueSize_district_ = 256;
const int kMaxValueSize_district_col1_ = 10;
const int kMaxValueSize_district_col2_ = 20;
const int kMaxValueSize_district_col3_ = 20;
const int kMaxValueSize_district_col4_ = 20;
const int kMaxValueSize_district_col5_ = 2; 
const int kMaxValueSize_district_col6_ = 9; 
const int kMaxValueSize_district_col7_ = 6;
const int kMaxValueSize_district_col8_ = 14; 
const int kMaxValueSize_district_col9_ = 16; 
const int kMaxValueSize_district_col_all = 10+20+20+20+2+9+6+14+16;
const off_t kMetaOffset_district_ = 0;

int kMaxPages_district_item = 1000;     // 100M
const int kMaxFreePages_district_ = 1100; 
// const int kMaxPages_district_item = 80000; //1G

typedef char Key[kMaxKeySize_district_];
typedef char Value1[kMaxValueSize_district_col1_];
typedef char Value2[kMaxValueSize_district_col2_];
typedef char Value3[kMaxValueSize_district_col3_];
typedef char Value4[kMaxValueSize_district_col4_];
typedef char Value5[kMaxValueSize_district_col5_];
typedef char Value6[kMaxValueSize_district_col6_];
typedef char Value7[kMaxValueSize_district_col7_];
typedef char Value8[kMaxValueSize_district_col8_];
typedef char Value9[kMaxValueSize_district_col9_];

typedef char Value_all[kMaxValueSize_district_col_all]; //for log record
...
const int innerKeyoffset_district_ = 0;
const int innercoloffset_district_ = 0;

const int kMaxUniqueKInGLogSeg=3000;
  


struct BPlusDistrictTree::Monitor{
  Monitor(){
    bg_id =0;
    plog_insert = 0;
    leaf_insert = 0;
...
struct BPlusDistrictTree::Index
{
  ...
  void UpdateIndex(off_t of, off_t lsn, const char *k)
  {
    offset = of;
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_district_);
  }
...
//hkc-modify-2
struct BPlusDistrictTree::Record
{
  Key key;
  Value1 value_1;
  Value2 value_2;
  ...
  void UpdateKV(
    ...
    const char *v7, const char *v8, const char *v9, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_district_);
    strncpy(value_1, v1, kMaxValueSize_district_col1_);
    strncpy(value_2, v2, kMaxValueSize_district_col2_);
    strncpy(value_3, v3, kMaxValueSize_district_col3_);
    strncpy(value_4, v4, kMaxValueSize_district_col4_);
    strncpy(value_5, v5, kMaxValueSize_district_col5_);
    strncpy(value_6, v6, kMaxValueSize_district_col6_);
    strncpy(value_7, v7, kMaxValueSize_district_col7_);
    strncpy(value_8, v8, kMaxValueSize_district_col8_);
    strncpy(value_9, v9, kMaxValueSize_district_col9_);
  }
  void UpdateKey(const char *k, off_t lsn)
  {
    cur_lsn = lsn;
...
struct BPlusDistrictTree::PAXKeyRecord
{
  Key key;
  off_t cur_lsn;
...
struct BPlusDistrictTree::PAXColumn1Record
{
  Value1 value_1;
...
struct BPlusDistrictTree::PAXColumn2Record
{
  Value2 value_2;
...
struct BPlusDistrictTree::PAXColumn3Record
{
  Value3 value_3;
...
struct BPlusDistrictTree::PAXColumn4Record
{
  Value4 value_4;
...
//hkc-modify-8
struct BPlusDistrictTree::PAXColumn5Record
{
  Value5 value_5;
...
//hkc-modify-9
struct BPlusDistrictTree::PAXColumn6Record
{
  Value6 value_6;
...
struct BPlusDistrictTree::PAXColumn7Record
{
  Value7 value_7;
...
struct BPlusDistrictTree::PAXColumn8Record
{
  Value8 value_8;
...
struct BPlusDistrictTree::PAXColumn9Record
{
  Value9 value_9;
...
struct BPlusDistrictTree::Per_page_Log
{
  Key key;
  Value_all value_all;
  off_t cur_lsn;
  ...
  void UpdateKV(const char *k, const char *v, off_t lsn)
  {
    cur_lsn = lsn;
    strncpy(key, k, kMaxKeySize_district_);
    strncpy(value_all, v, kMaxValueSize_district_col_all);
  }
  void UpdateKey(const char *k, off_t lsn)
  {
    cur_lsn = lsn;
...
struct BPlusDistrictTree::LogRecord
{
  LogRecord() : page_offset(0), log_lsn(0), log_offset(0), next_log_offset(0), page_index(0)
  {
    memset(key, 0, kMaxKeySize_district_);
    memset(value, 0, kMaxValueSize_district_col_all);
  }
  ...
  char key[kMaxKeySize_district_];
  char value[kMaxValueSize_district_col_all];

  void UpdateKV(const char *k, const char *v)
  {
    strncpy(key, k, kMaxKeySize_district_);
    strncpy(value, v, kMaxValueSize_district_col_all);
  }
  void UpdateKey(const char *k) { strncpy(key, k, kMaxKeySize_district_); }
  void UpdateValue(const char *v) { strncpy(value, v, kMaxValueSize_district_col_all); }
};

struct BPlusDistrictTree::Checkpoint
{
...
struct BPlusDistrictTree::IndexNode : BPlusDistrictTree::Node
{
  IndexNode() = default;
  ~IndexNode() = default;
  ...
  const char *Key(int index) const
  {
    //assert(count > 0);
    //assert(index >= 0);
    //assert(index <= kOrder_district_Index);
  ...
  Index indexes[kOrder_district_Index + 1];
};

//hkc-modify-9
struct BPlusDistrictTree::LeafNode : BPlusDistrictTree::Node
{
  LeafNode() = default;
  ~LeafNode() = default;
  ...
  {
    //assert(count > 0);
    char* all = (char*)malloc(sizeof(char)*kMaxValueSize_district_col_all);
    int off =0;
    memcpy(&all[off],records[index].value_1,kMaxValueSize_district_col1_);
    off+=kMaxValueSize_district_col1_;
    memcpy(&all[off],records[index].value_2,kMaxValueSize_district_col2_);
    off+=kMaxValueSize_district_col2_;
    memcpy(&all[off],records[index].value_3,kMaxValueSize_district_col3_);
    off+=kMaxValueSize_district_col3_;
    memcpy(&all[off],records[index].value_4,kMaxValueSize_district_col4_);
    off+=kMaxValueSize_district_col4_;
    memcpy(&all[off],records[index].value_5,kMaxValueSize_district_col5_);
    off+=kMaxValueSize_district_col5_;
    memcpy(&all[off],records[index].value_6,kMaxValueSize_district_col6_);
    off+=kMaxValueSize_district_col6_;
    memcpy(&all[off],records[index].value_7,kMaxValueSize_district_col7_);
    off+=kMaxValueSize_district_col7_;
    memcpy(&all[off],records[index].value_8,kMaxValueSize_district_col8_);
    off+=kMaxValueSize_district_col8_;
    memcpy(&all[off],records[index].value_9,kMaxValueSize_district_col9_);
    return all;
  }
  ...
  {
    //assert(index >= 0);
    records[index].UpdateValue(v1,v2,v3,v4,v5,v6,v7,v8,v9, lsn);
  }
  ...
  {
    //assert(index >= 0);
    records[index].UpdateKV(k, v1,v2,v3,v4,v5,v6,v7,v8,v9,lsn);
  }
  ...
  {
    //assert(index >= 0);
    //assert(index < kOrder_district_);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateKV(index, k, v1,v2,v3,v4,v5,v6,v7,v8,v9, lsn);
  }
  ...
  off_t offset_v8;
  off_t offset_v9;
  BPlusDistrictTree::Record records[kOrder_district_];
  // BPlusDistrictTree::Record Per_page_logs[kOrder_district_/2];
...
//hkc-modify-10
struct BPlusDistrictTree::Plog : BPlusDistrictTree::Node
{
  Plog() = default;
  ~Plog() = default;
  ...
  {
    //assert(index >= 0);
    //assert(index < kOrder_district_Plog);
    std::memmove(&records[index + 1], &records[index],
                 sizeof(records[0]) * (count++ - index));
    UpdateKV(index, k,v1,v2,v3,v4,v5,v6,v7,v8,v9,lsn);
  }
  ...
  // BPlusDistrictTree::Record records[kOrder_district_/2];
  BPlusDistrictTree::Record records[kOrder_district_Plog];
};

class BPlusDistrictTree::BlockCache
{
  struct Node;
...
public:
  BlockCache(bool enable_backthd)
  {
    ...
    // pre_allocate
    // 2. init free list
    char *init_ptr = (char *)malloc(kMaxFreePages_district_ * kLeafPageSize); // i.e., for each page
    memset(init_ptr, 0, kMaxFreePages_district_ * kLeafPageSize);

    char *flag_addr = init_ptr;
    for (int i = 0; i < kMaxFreePages_district_; i++)
    {
...
void RowToColumn_Cache(char* initial_page, char* converted_page){
  ...
  if(row_leaf->count == 0 || row_leaf->records[0].cur_lsn == 0){
    int x=1; ////printf("empty row page!!");
    memcpy(converted_page,initial_page,kLeafPageSize);
    return;
  }
  row_leaf->offset_k =    4*1024;
  row_leaf->offset_lsn = (4+20)*1024;
  row_leaf->offset_v1  = (4+20+8)*1024;
  row_leaf->offset_v2  = (4+20+8+8)*1024;
  row_leaf->offset_v3  = (4+20+8+8+12)*1024;
  row_leaf->offset_v4  = (4+20+8+8+12+12)*1024;
  row_leaf->offset_v5  = (4+20+8+8+12+12+12)*1024;
  row_leaf->offset_v6  = (4+20+8+8+12+12+12+4)*1024;
  row_leaf->offset_v7  = (4+20+8+8+12+12+12+4+8)*1024;
  row_leaf->offset_v8  = (4+20+8+8+12+12+12+4+8+4)*1024;
  row_leaf->offset_v9  = (4+20+8+8+12+12+12+4+8+4+12)*1024;
  ...
  fence+=sizeof(row_leaf->plog_offset);
  ...
  if(fence > 4*1024){
    int x=1; ////printf("overflow!\n");
  } 
  //convert row as column and fit in sub pages

    /*
      const int kMaxKeySize_district_ = 16; 16K
      const int kMaxValueSize_district_col1_ = 16; 16K
      const int kMaxValueSize_district_col2_ = 24; 24K
      const int kMaxValueSize_district_col3_ = 8; 8K
      const int kMaxValueSize_district_col4_ = 50; 50K
    */
  ...
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&buf[k_of],&(row_leaf->records[i].key),kMaxKeySize_district_);
    k_of+=kMaxKeySize_district_;
    memcpy(&buf[lsn_of],&(row_leaf->records[i].cur_lsn),sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&buf[v1_of],&(row_leaf->records[i].value_1),kMaxValueSize_district_col1_);
    v1_of+=kMaxValueSize_district_col1_;
    memcpy(&buf[v2_of],&(row_leaf->records[i].value_2),kMaxValueSize_district_col2_);
    v2_of+=kMaxValueSize_district_col2_;
    memcpy(&buf[v3_of],&(row_leaf->records[i].value_3),kMaxValueSize_district_col3_);
    v3_of+=kMaxValueSize_district_col3_;
    memcpy(&buf[v4_of],&(row_leaf->records[i].value_4),kMaxValueSize_district_col4_);
    v4_of+=kMaxValueSize_district_col4_;
    memcpy(&buf[v5_of],&(row_leaf->records[i].value_5),kMaxValueSize_district_col5_);
    v5_of+=kMaxValueSize_district_col5_;
    memcpy(&buf[v6_of],&(row_leaf->records[i].value_6),kMaxValueSize_district_col6_);
    v6_of+=kMaxValueSize_district_col6_;
    memcpy(&buf[v7_of],&(row_leaf->records[i].value_7),kMaxValueSize_district_col7_);
    v7_of+=kMaxValueSize_district_col7_;
    memcpy(&buf[v8_of],&(row_leaf->records[i].value_8),kMaxValueSize_district_col8_);
    v8_of+=kMaxValueSize_district_col8_;
    memcpy(&buf[v9_of],&(row_leaf->records[i].value_9),kMaxValueSize_district_col9_);
    v9_of+=kMaxValueSize_district_col9_;
...
void ColumnToRow_Cache(char* column_page, char* row_page){
 ...
 row_leaf->offset_k =    4*1024;
  ...
  memcpy((char*)&row_leaf->plog_offset,&buf[fence],sizeof(row_leaf->plog_offset));
  fence+=sizeof(row_leaf->plog_offset);
  ...
  for(int i=0; i<row_leaf->count;i++){
    memcpy(&(row_leaf->records[i].key),&buf[k_of],kMaxKeySize_district_);
    k_of+=kMaxKeySize_district_;
    memcpy(&(row_leaf->records[i].cur_lsn),&buf[lsn_of],sizeof(row_leaf->records[i].cur_lsn));
    lsn_of+=sizeof(row_leaf->records[i].cur_lsn);
    memcpy(&(row_leaf->records[i].value_1),&buf[v1_of],kMaxValueSize_district_col1_);
    v1_of+=kMaxValueSize_district_col1_;
    memcpy(&(row_leaf->records[i].value_2),&buf[v2_of],kMaxValueSize_district_col2_);
    v2_of+=kMaxValueSize_district_col2_;
    memcpy(&(row_leaf->records[i].value_3),&buf[v3_of],kMaxValueSize_district_col3_);
    v3_of+=kMaxValueSize_district_col3_;
    memcpy(&(row_leaf->records[i].value_4),&buf[v4_of],kMaxValueSize_district_col4_);
    v4_of+=kMaxValueSize_district_col4_;
    memcpy(&(row_leaf->records[i].value_5),&buf[v5_of],kMaxValueSize_district_col5_);
    v5_of+=kMaxValueSize_district_col5_;
    memcpy(&(row_leaf->records[i].value_6),&buf[v6_of],kMaxValueSize_district_col6_);
    v6_of+=kMaxValueSize_district_col6_;
    memcpy(&(row_leaf->records[i].value_7),&buf[v7_of],kMaxValueSize_district_col7_);
    v7_of+=kMaxValueSize_district_col7_;
    memcpy(&(row_leaf->records[i].value_8),&buf[v8_of],kMaxValueSize_district_col8_);
    v8_of+=kMaxValueSize_district_col8_;
    memcpy(&(row_leaf->records[i].value_9),&buf[v9_of],kMaxValueSize_district_col9_);
    v9_of+=kMaxValueSize_district_col9_;
  ...
  void Put(T *block, int is_plog)
  {
    ...
    if (async_mode_district_ == 0)
    {
      if (size_lru > kMaxPages_district_item * 0.9)
      {
        KickLRUbyUring();
      }
      if (mtr_cache->plog_flag != 1 && mtr_cache->FLU_disabled == 0 && size_flu > kMaxPages_district_item * 0.5)
      {
        KickLFUbyUring();
      }
    }
  ...
  void SetMtr(BPlusDistrictTree::Monitor *mtr_1){
    mtr_cache = mtr_1;
  }
    ...
    for (i = 0; i < Queue_depth; i++)
    {
      ...
      if(debug_flag==1){
        ...
        if( (leaf->count!=row_leaf_test->count) || 
            ...
            (leaf->plog_offset!=row_leaf_test->plog_offset) || 
...
class BPlusDistrictTree::InternalCache
{
  struct Node;
...
public:
  ...
  void SetMtr(BPlusDistrictTree::Monitor *mtr_2){
    mtr_internal = mtr_2;
  }
  ...
  T *Get(int fd, off_t offset)
  {
    ...
    if (block->offset != offset)
    {
      int x=1; ////--+//printf("error in retrieve leaf node!\n");
    }
    return static_cast<T *>(node->block);
  }
...
BPlusDistrictTree::BPlusDistrictTree(const char *path, const char *log_path, const char *chpt_path, const int bg_id_, const bool enable_backthd)
    : fd_(open(path, O_CREAT|O_DIRECT|O_RDWR)),log_fd_(open(log_path, O_CREAT|O_APPEND|O_RDWR)),chpt_fd_(open(chpt_path, O_CREAT|O_APPEND|O_RDWR)), block_cache_(new BlockCache(enable_backthd)), internal_cache_(new InternalCache())
{
  ...
  if (fd_ == -1)
    //Exit("open db");
    printf("open district db failed.\n");
  if (log_fd_ == -1)
    //Exit("open log");
    printf("open district log failed.\n");
  if (chpt_fd_ == -1)
    //Exit("open chpt");
    printf("open district chpt failed.\n");
  ...
  // meta_ = Map<Meta>(kMetaOffset_district_);
  meta_ = new Meta();
  ...
  {
    // Initialize B+tree;
    constexpr off_t of_root = kMetaOffset_district_ + kUnitNodeSize;
    meta_->block = of_root;
...
BPlusDistrictTree::~BPlusDistrictTree()
{
  UnMap(meta_, 0, 0);
  delete block_cache_;
  close(fd_);
...
void BPlusDistrictTree::SetPlog(){
  mtr_->plog_flag =1;
  mtr_->FLU_disabled =1;
  mtr_->warmup_flag =1;
  return;
...
void BPlusDistrictTree::GetStatic(int &hits_bp, int &hits_glog, int &hits_plog, int &disk_read, int &io_consume, int &error_cnt, int &dirtys, int &cleans){
...
off_t BPlusDistrictTree::GetLSN(){
  return mtr_->global_lsn;
}
void BPlusDistrictTree::IncreLSN(){
  mtr_->global_lsn++;
  return;
}

void BPlusDistrictTree::ResetStatic(){
    mtr_->hits_bp = 0; 
    mtr_->hits_glog = 0; 
    mtr_->hits_plog =0;
    mtr_->disk_read=0;
...
void BPlusDistrictTree::SetBaseline(){
  mtr_->FLU_disabled = 0;
  mtr_->plog_flag =-1;
  mtr_->warmup_flag =0;
}

void BPlusDistrictTree::SetIOStaic(){
    mtr_->plog_insert=0;
    mtr_->leaf_insert=0;
    mtr_->leaf_split=0;
...
off_t BPlusDistrictTree::GetChpt(){
  return mtr_->normal_chpt_now;
}
void BPlusDistrictTree::SetLoadFlag(int flag){
  mtr_->load_flag = flag;
}
void BPlusDistrictTree::DisableFLUOrNot(int flag){
  mtr_->FLU_disabled = flag;
}
void BPlusDistrictTree::LoadIOStatic(int &plog_insert,int &leaf_insert,int &leaf_split,int &phy_plog_write,int &phy_plog_read,int &phy_page_read,int &phy_page_write,int &logi_page_read,int &logi_page_write){
...
int BPlusDistrictTree::ApplyPendPages(std::map<off_t, std::pair<char*,char*>>* source, std::map<off_t, std::pair<char*,char*>>* target, std::map<off_t,std::pair<int,int>> *EmptyPages){
...
void BPlusDistrictTree::BackGround_GLog_Func()
{
  int total_KV=0;
  int final_KV=0;
  int total_Plog =0;
  ...
  for(std::map<off_t, std::pair<std::string,std::string>>::iterator it = pending_Seg->begin(); it!=pending_Seg->end();it++){    
      ...
      if(now_lsn > cur_lsn){
        ...
        std::string value = it->second.second;
          ...
          //hkc-modify-14
          std::string value1 = value.substr(0,10);
          std::string value2 = value.substr(10,20);
          std::string value3 = value.substr(30,20);
          std::string value4 = value.substr(50,20);
          std::string value5 = value.substr(70,2);
          std::string value6 = value.substr(72,9);
          std::string value7 = value.substr(81,6);
          std::string value8 = value.substr(87,14);
          std::string value9 = value.substr(101,16);
          
        if (InsertKVIntoPlog(plog, key.data(), 
                value1.data(),value2.data(),value3.data(),value4.data(),value5.data(),value6.data(),value7.data(),value8.data(),value9.data(),cur_lsn) <=
          GetMaxKeys(0)){
          mtr_->plog_insert += 1;
          /*
          if (Check(of_leaf) == 1){
          ...
          */
          ...
          for (int i = 0; i < kOrder_district_Plog; i++){
...
//hkc-modify-21
void BPlusDistrictTree::RowToColumn(char* initial_page, char* converted_page){
...
//hkc-modify-22
void BPlusDistrictTree::ColumnToRow(char* column_page, char* row_page){
  memset(row_page,0,kLeafPageSize);
  LeafNode* row_leaf = reinterpret_cast<LeafNode*>(row_page);
...
//hkc-gb-04
off_t BPlusDistrictTree::GetOnlyLeafOffset(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
  return of_leaf;
}
//hkc-gb-05
char* BPlusDistrictTree::GetLeafCopy(const std::string &key){
  off_t of_leaf = GetLeafOffset(key.data());
...
//hkc-gb-06
void BPlusDistrictTree::UpdateLeafCopy(const std::string &key, const std::string &value, off_t lsn, char* leaf_copy){
  ...
  if (InsertKVIntoLeafNode(leaf, key.data(),  value1.data(),value2.data(),value3.data(),value4.data(),value5.data(),value6.data(),
                          value7.data(),value8.data(),value9.data(),  lsn) <= GetMaxKeys(1)){
...
//hkc-gb-07
void BPlusDistrictTree::GetRowRecord(const std::string &key, std::string &value, char* leaf_copy) {
  LeafNode* leaf  = reinterpret_cast<LeafNode*>(&leaf_copy[0]);
  int index = GetIndexFromLeafNode(leaf, key.data());
  char* ptr = leaf->ValueALL(index);
  std::string tmp_value(ptr,ptr+kMaxValueSize_district_col_all);
  value = tmp_value;
  free(ptr);
  return ;
...
//hkc-gb-08
void BPlusDistrictTree::TriggerFlush(const off_t* offsets, const int size, char* evict_buf){
  int batch_size = 200;
...
void BPlusDistrictTree::Put(const std::string &key, const std::string &value, off_t lsn)
{
  ...
  //hkc-modify-16
          ...
          std::string value8 = value.substr(87,14);
          std::string value9 = value.substr(101,16);
  if (InsertKVIntoLeafNode(leaf_node, key.data(),  value1.data(),value2.data(),value3.data(),value4.data(),value5.data(),value6.data(),value7.data(),value8.data(),value9.data(),lsn) <=
      GetMaxKeys(1))
  {
    // 2.If records of leaf node less than or equals kOrder_district_ - 1 then finish.
  ...
  // until count <= kOrder_district_ - 1.
  size_t count;
  do
  {
...
/*
bool BPlusDistrictTree::Get(const std::string &key, std::string &value) const
{
...
bool BPlusDistrictTree::Scan(const std::string &start, const std::string &end, std::string &value) const
{
  if(start>end){
    int x=1;
  }
...
int BPlusDistrictTree::Check(off_t offset)
{
  return block_cache_->Check_Leaf(offset);
}

char *BPlusDistrictTree::GetMemCopy(off_t offset)
{
  return block_cache_->GetMemCopy_Leaf(offset);
}

template <typename T>
T *BPlusDistrictTree::Map(off_t offset, int isleaf, int is_plog, char *&plog_copy) const
{
  if (isleaf == 1)
  {
...
template <typename T>
void BPlusDistrictTree::InsertDirty(off_t offset) const
{
...
template <typename T>
void BPlusDistrictTree::RelocateDirty(off_t offset, off_t ori_oldset_lsn) const
{
...
template <typename T>
void BPlusDistrictTree::DirectInsertDirty(off_t offset_leaf, off_t offset_split) const
{
...
template <typename T>
void BPlusDistrictTree::UnMap(T *map_obj, int isleaf, int is_plog) const
{
  if (isleaf == 1)
  {
...
size_t BPlusDistrictTree::GetMinKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return (kOrder_district_Plog + 1) / 2 - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return (kOrder_district_ + 1) / 2 - 1;
  }
}
// 1: leaf node ; 0: plog
size_t BPlusDistrictTree::GetMaxKeys(int flag)
{
  if (flag == 0)
  {
    // plog page, i.e., plog
    return kOrder_district_Plog - 1;
  }
  else
  {
    // data page, i.e., leaf node
    return kOrder_district_ - 1;
  }
}
size_t BPlusDistrictTree::GetMinIndexKeys() { return (kOrder_district_Index + 1) / 2 - 1; }
size_t BPlusDistrictTree::GetMaxIndexKeys() { return kOrder_district_Index - 1; }

BPlusDistrictTree::IndexNode *BPlusDistrictTree::GetOrCreateParent(Node *node)
{
  if (node->parent == 0)
  {
    // Split root node.
...
template <typename T>
int BPlusDistrictTree::UpperBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
  ...
  {
    int mid = (l + r) >> 1;
    if (std::strncmp(arr[mid].key, key, kMaxKeySize_district_) <= 0)
    {
      l = mid + 1;
    }
    else
...
template <typename T>
int BPlusDistrictTree::UpperBoundIndex(T arr[], int n, const char *key) const
{
...
template <typename T>
int BPlusDistrictTree::LowerBoundIndex(T arr[], int n, const char *key) const
{
  ////assert(n <= GetMaxIndexKeys());
  int l = 0, r = n - 1;
  while (l <= r)
...
// flag: 1: data page; 0: plog
template <typename T>
int BPlusDistrictTree::LowerBound(T arr[], int n, const char *key, int flag) const
{
  if (flag == 0)
  {
    //assert(n <= GetMaxKeys(0));
...
template <typename T>
T *BPlusDistrictTree::AllocIndex()
{ // 1: leaf node, 0: internal node
...
template <typename T>
void BPlusDistrictTree::Dealloc(T *node)
{
  UnMap<T>(node);
}

void BPlusDistrictTree::WarmUpVirtualPageCache(int x) const
{
  return block_cache_->WarmUpVPCache(x);
}
off_t BPlusDistrictTree::GetLeafOffset(const char *key) const
{
...
inline size_t BPlusDistrictTree::InsertKeyIntoIndexNode(IndexNode *index_node,
                                                const char *key,
                                                Node *left_node,
...
//hkc-modify-13
size_t BPlusDistrictTree::InsertKVIntoLeafNode(LeafNode *leaf_node, const char *key,
                              ...
                              const char* value7, const char* value8,const char* value9,off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(leaf_node->Key(index - 1), key, kMaxKeySize_district_) == 0)
  {
    leaf_node->UpdateValue(index - 1, value1,value2,value3,value4,value5, value6,value7,value8,value9, cur_lsn);
    return leaf_node->count;
  }
  leaf_node->InsertKVAtIndex(index, key,  value1,value2,value3,value4,value5, value6,value7,value8,value9,cur_lsn);
  ++meta_->size;
  return leaf_node->count;
}
//hkc-modify-14
size_t BPlusDistrictTree::InsertKVIntoPlog(Plog *plog, const char *key,
                                   const char* value1, const char* value2,const char* value3,const char* value4,const char* value5,const char* value6,
                              const char* value7, const char* value8,const char* value9,off_t cur_lsn)
{
  ...
  if (index > 0 &&
      std::strncmp(plog->Key(index - 1), key, kMaxKeySize_district_) == 0)
  {
    plog->UpdateValue(index - 1, value1,value2,value3,value4,value5, value6,value7,value8,value9,cur_lsn);
    return plog->count;
  }
  plog->InsertKVAtIndex(index, key, value1,value2,value3,value4,value5, value6,value7,value8,value9,cur_lsn);
  ++meta_->size;
  return plog->count;
}

BPlusDistrictTree::LeafNode *BPlusDistrictTree::SplitLeafNode(LeafNode *leaf_node)
{
  //assert(leaf_node->count == kOrder_district_);
  off_t origian_lsn = leaf_node->GetSmallestLSN();
  constexpr int mid = (kOrder_district_ - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_district_ - mid;
...
BPlusDistrictTree::IndexNode *BPlusDistrictTree::SplitIndexNode(IndexNode *index_node)
{
  //assert(index_node->count == kOrder_district_Index);
  constexpr int mid = (kOrder_district_Index - 1) >> 1;
  constexpr int left_count = mid;
  constexpr int right_count = kOrder_district_Index - mid - 1;
  ...
  for (int i = mid + 1; i <= kOrder_district_Index; ++i)
  {
...
inline int BPlusDistrictTree::GetIndexFromLeafNode(LeafNode *leaf_node,
                                           const char *key) const
{
  int index = LowerBound(leaf_node->records, leaf_node->count, key, 1);
  return index < static_cast<int>(leaf_node->count) &&
                 std::strncmp(leaf_node->Key(index), key, kMaxKeySize_district_) == 0
             ? index
             : -1;
}

inline int BPlusDistrictTree::GetIndexFromPlog(Plog *plog,
                                       const char *key) const
{
  int index = LowerBound(plog->records, plog->count, key, 0);
  return index < static_cast<int>(plog->count) &&
                 std::strncmp(plog->Key(index), key, kMaxKeySize_district_) == 0
             ? index
             : -1;
}

std::vector<std::pair<std::string, std::string>> BPlusDistrictTree::GetRange(
    const std::string &left_key, const std::string &right_key) const
{
  ...
  while (of_leaf != 0 && !finish)
  {
    ...
    {
      if (strncmp(right_leaf_node->Key(i), right_key.data(), kMaxKeySize_district_) <=
          0)
      {
...
bool BPlusDistrictTree::Empty() const { return meta_->size == 0; }

size_t BPlusDistrictTree::Size() const { return meta_->size; }