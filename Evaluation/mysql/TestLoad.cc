...
namespace tpcch_kv_generator{
  ...
  class tpcc_gen{
    public:
      ...
      tpcc_gen(int par_id_, int w_start,int w_end){
      w = w_end;
      par_id = par_id_;
      cur_kvs_customer = cur_kvs_history = cur_kvs_stock = cur_kvs_orderline = 0;
      cur_w_id_customer = w_start;
      cur_d_id_customer = 1;
      cur_c_id_customer = 1;
      cur_w_id_neworders = w_start;
      cur_d_id_neworders = 1;
      cur_c_id_neworders = 1;
      cur_w_id_orders = w_start;
      cur_d_id_orders = 1;
      cur_c_id_orders = 1;
      cur_w_id_stock = w_start;
      cur_id_stock = 1;
      cur_w_id_orderline = w_start;
      cur_d_id_orderline = 1;
      cur_o_id_orderline = 1;
      cur_number_orderline = 1;

      cur_w_id_history = w_start;
      cur_d_id_history = 1;
      cur_c_id_history = 1;
      ...
      kvs_customer = 3000 * 10 * (w_end-w_start);
      kvs_orderline = 3000 * 10 * (w_end-w_start) * 20;
      
      kvs_item = 100000;
      cur_id_item =1;
      
      kvs_stock = kvs_item * (w_end-w_start);
      kvs_neworders = 3000 * 10 * (w_end-w_start) ;
      kvs_orders = 3000 * 10 * (w_end-w_start) ; 
      kvs_history = 3000 * 10 * (w_end-w_start);
    }
      ...
      std::string* generateLoad_BULK_Orderline(){
...
void Consumer_ItemTree_MVCC_TEST(ProAndCon* itemQ_, const int load_flag_){
  std::string skt= "/media/hkc/csd-7.7T/mysql-base/data/mysql.sock"; 
  MYSQL mysql;     
  mysql_init(&mysql);     
  mysql_options(&mysql, MYSQL_SET_CHARSET_NAME, "utf8");     
  mysql_options(&mysql, MYSQL_INIT_COMMAND, "SET NAMES utf8");       
  if(!mysql_real_connect(&mysql, "localhost", "root", "root", "pbtest", 3306, NULL, 0)) {         
...
void Consumer_OrderlineTree_MVCC_TEST(ProAndCon* itemQ_, const int load_flag_){
  ...
  mtx.lock();      
  if(!mysql_real_connect(&mysql, "localhost", "root", "root", "pbtest", 3306, NULL, 0)) {         
      std::cout<<"mysql connect error orderline init : "<<mysql_error(&mysql)<<" "<<mysql_errno(&mysql);     
  }  
  mtx.unlock();
  ...
  if(load_flag_ == 1){
    ...
    while(1){
      ...
      if(KeyWordExist(cmd,1)){
        ...
        str+=res[1];
        str+="(pk1, pk2, pk3, pk4, lsn, value1, value2, value3, value4, value5, value6) values(";
        str+=res[4]+",";
        str+=res[3]+",";
        str+=res[2]+",";
        str+=res[5];
    ...
    static_tree::orderline_put_cnt=0;
    while(1){
      std::string* cmd = itemQ_->consumption();
      if(cmd->size()==0){
        ErrorDetect(); 
...
void Consumer_CustomerTree_MVCC_TEST(ProAndCon* itemQ_, const int load_flag_){
  ...
  if(load_flag_ == 1){
    ...
    static_tree::customer_put_cnt=0;
    while(1){
      std::string* cmd = itemQ_->consumption();
      if(cmd->size()==0){
        ErrorDetect(); 
...
void Consumer_NewordersTree_MVCC_TEST(ProAndCon* itemQ_, const int load_flag_){
  ...
  if(load_flag_ == 1){
    ...
    static_tree::neworders_put_cnt=0;
    while(1){
      std::string* cmd = itemQ_->consumption();
      if(cmd->size()==0){
        ErrorDetect(); 
...
void dispatcher_TEST(int load_flag_,int w_start,int w_end){
    
    
  tpcch_kv_generator::tpcc_gen tpcc_gen_(0, w_start,w_end);
...
void dispatcher_TEST_LOAD_Orderline(ProAndCon* orderlineQ, int w_start,int w_end, int par_id){
    
    
  tpcch_kv_generator::tpcc_gen tpcc_gen_(par_id, w_start,w_end);
  ...
  mtx.unlock();
  //2. create tables
  std::string str = "create table pbtest.orderline";
  str+=std::to_string(par_id);
  str+="(pk1 int(16),pk2 int(16),pk3 int(16),pk4 int(16), lsn char(8), value1 char(16),value2 char(16),value3 char(20),value4 char(16),value5 char(8),value6 char(24),PRIMARY KEY(pk1,pk2,pk3,pk4))";
  mysql_real_query(&mysql, str.c_str(), str.size()); 
...
std::cout<<"finish load orderline, exe time: "<<tmp<<" ms"<<"w_start/w_end/par_id "<< w_start<<"/"<<w_end<<"/"<<par_id<<"\n";
return ;
...
int main(int argc, char const *argv[])
{
  ...
  if(load==5){
    std::vector<ProAndCon*> olQs;
    std::vector<std::thread*> olTs;
    std::vector<std::thread*> loaders;
    int start =0;
    int end=10;
    for(int i=0;i<20;i++){
      ProAndCon* olQ = new ProAndCon(50);
      olQs.push_back(olQ);
      std::thread* olT = new std::thread(Consumer_OrderlineTree_MVCC_TEST,olQ, 1);
      olTs.push_back(olT);
      olT->detach();
      std::thread* loader= new std::thread(dispatcher_TEST_LOAD_Orderline,olQ, start,end,i);
      loader->detach();
    ...
    std::string str = "create database test_bw;";       
    mysql_real_query(&mysql, str.c_str(),  str.size()); 
    str = "alter database test_bw charset=utf8;";      
    mysql_real_query(&mysql, str.c_str(), str.size());
    str = "create table test_bw.item(pk int(16) primary key, value1 TEXT,value2 TEXT);";
    mysql_real_query(&mysql, str.c_str(), str.size());
    ...
    for(int i=1;i<100000;i++){
      std::string str = "insert into test_bw.item(pk, value1, value2) values(";
      str+=std::to_string(i);
      str+=", \'";
      RandStrBig(buf,50);
      for(int i =0; i<50;i++) str+=buf[i];
      str+="\',\'"; //GMUUDXRMCZASEB,
      memset(buf,0,50);
      RandStrBig(buf,100);
      for(int i =0; i<100;i++) str+=buf[i];
      str+="\');";
      memset(buf,0,100);
      auto t1 = std::chrono::steady_clock::now();
      mysql_real_query(&mysql, str.c_str(), str.size());     
      auto t2 = std::chrono::steady_clock::now();
      time_ += std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
      cnt++;
    }
    std::cout<<"load time/cnt: "<<time_<<" / "<<cnt<<"\n";
    mysql_close(&mysql);
    return 1;
